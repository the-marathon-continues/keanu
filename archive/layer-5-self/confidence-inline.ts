...
import type { CalibrationReading } from "../layer-3-causal/calibrate.ts";
import { checkCalibration } from "../layer-3-causal/calibrate.ts";
...
function injectMarkers(
  ...
  const markedSentences = sentences.map((sentence) => {
    ...
    if (hasRecommendation || hasExternalState || hasAbsolute || hasVersion || hasPrediction) {
      ...
      if (hasExternalState) {
        sentenceConf = Math.min(sentenceConf, 0.5);
      }
      if (hasAbsolute) {
        sentenceConf = Math.min(sentenceConf, 0.5);
      }
      if (hasPrediction) {
        sentenceConf = Math.min(sentenceConf, 0.6);
      }
...
export function shouldInjectConfidence(state: ConfidenceInlineState, turn: number): boolean {
  ...
  if (turnsSinceInject > 10) {
    rate += 0.1;
  }
  if (turnsSinceInject > 20) {
    rate += 0.1;
  }
