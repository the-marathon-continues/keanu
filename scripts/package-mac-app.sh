...
# Build and bundle Keanu into a minimal .app we can open.
# Outputs to dist/Keanu.app

ROOT_DIR="$(cd "$(dirname "$0")/.." && pwd)"
APP_ROOT="$ROOT_DIR/dist/Keanu.app"
BUILD_ROOT="$ROOT_DIR/apps/macos/.build"
PRODUCT="Keanu"
BUNDLE_ID="${BUNDLE_ID:-ai.keanu.mac.debug}"
...
SPARKLE_FEED_URL="${SPARKLE_FEED_URL:-https://raw.githubusercontent.com/the-marathon-continues/keanu/main/appcast.xml}"
AUTO_CHECKS=true
...
INFO_PLIST_SRC="$ROOT_DIR/apps/macos/Sources/Keanu/Resources/Info.plist"
...
/usr/libexec/PlistBuddy -c "Set :KeanuBuildTimestamp ${BUILD_TS}" "$APP_ROOT/Contents/Info.plist" || true
/usr/libexec/PlistBuddy -c "Set :KeanuGitCommit ${GIT_COMMIT}" "$APP_ROOT/Contents/Info.plist" || true
...
echo "🚚 Copying binary"
cp "$BIN_PRIMARY" "$APP_ROOT/Contents/MacOS/Keanu"
if [[ "${#BUILD_ARCHS[@]}" -gt 1 ]]; then
  BIN_INPUTS=()
  for arch in "${BUILD_ARCHS[@]}"; do
    BIN_INPUTS+=("$(bin_for_arch "$arch")")
  done
  /usr/bin/lipo -create "${BIN_INPUTS[@]}" -output "$APP_ROOT/Contents/MacOS/Keanu"
fi
chmod +x "$APP_ROOT/Contents/MacOS/Keanu"
# SwiftPM outputs ad-hoc signed binaries; strip the signature before install_name_tool to avoid warnings.
/usr/bin/codesign --remove-signature "$APP_ROOT/Contents/MacOS/Keanu" 2>/dev/null || true
...
cp "$ROOT_DIR/apps/macos/Sources/Keanu/Resources/Keanu.icns" "$APP_ROOT/Contents/Resources/Keanu.icns"

echo "📦 Copying device model resources"
rm -rf "$APP_ROOT/Contents/Resources/DeviceModels"
cp -R "$ROOT_DIR/apps/macos/Sources/Keanu/Resources/DeviceModels" "$APP_ROOT/Contents/Resources/DeviceModels"
...
echo "📦 Copying KeanuKit resources"
KEANUKIT_BUNDLE="$(build_path_for_arch "$PRIMARY_ARCH")/$BUILD_CONFIG/KeanuKit_KeanuKit.bundle"
if [ -d "$KEANUKIT_BUNDLE" ]; then
  rm -rf "$APP_ROOT/Contents/Resources/KeanuKit_KeanuKit.bundle"
  cp -R "$KEANUKIT_BUNDLE" "$APP_ROOT/Contents/Resources/KeanuKit_KeanuKit.bundle"
else
  echo "WARN: KeanuKit resource bundle not found at $KEANUKIT_BUNDLE (continuing)" >&2
fi
...
echo "⏹  Stopping any running Keanu"
killall -q Keanu 2>/dev/null || true
