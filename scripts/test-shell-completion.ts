 ...
 * `keanu update`. Use it to verify the completion installation flow
...
\`keanu update\`, \`keanu doctor\`, and \`keanu onboard\`.
...
async function main() {
  ...
  if (status.profileInstalled && status.cacheExists && !options.force) {
    ...
    console.log(theme.muted("  1. Remove the '# Keanu Completion' block from your shell profile"));
  ...
  if (isCancel(shouldInstall) || !shouldInstall) {
    console.log(theme.muted(`Skipped. Run \`keanu completion --install\` later to enable.`));
    return;
  }
