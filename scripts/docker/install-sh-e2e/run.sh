...
MODELS_MODE="${KEANU_E2E_MODELS:-${KEANU_E2E_MODELS:-both}}" # both|openai|anthropic
INSTALL_TAG="${KEANU_INSTALL_TAG:-${KEANU_INSTALL_TAG:-latest}}"
E2E_PREVIOUS_VERSION="${KEANU_INSTALL_E2E_PREVIOUS:-${KEANU_INSTALL_E2E_PREVIOUS:-}}"
SKIP_PREVIOUS="${KEANU_INSTALL_E2E_SKIP_PREVIOUS:-${KEANU_INSTALL_E2E_SKIP_PREVIOUS:-0}}"
OPENAI_API_KEY="${OPENAI_API_KEY:-}"
ANTHROPIC_API_KEY="${ANTHROPIC_API_KEY:-}"
ANTHROPIC_API_TOKEN="${ANTHROPIC_API_TOKEN:-}"

if [[ "$MODELS_MODE" != "both" && "$MODELS_MODE" != "openai" && "$MODELS_MODE" != "anthropic" ]]; then
  echo "ERROR: KEANU_E2E_MODELS must be one of: both|openai|anthropic" >&2
  exit 2
fi

if [[ "$MODELS_MODE" == "both" ]]; then
  if [[ -z "$OPENAI_API_KEY" ]]; then
    echo "ERROR: KEANU_E2E_MODELS=both requires OPENAI_API_KEY." >&2
    exit 2
  fi
  if [[ -z "$ANTHROPIC_API_TOKEN" && -z "$ANTHROPIC_API_KEY" ]]; then
    echo "ERROR: KEANU_E2E_MODELS=both requires ANTHROPIC_API_TOKEN or ANTHROPIC_API_KEY." >&2
    exit 2
  fi
elif [[ "$MODELS_MODE" == "openai" && -z "$OPENAI_API_KEY" ]]; then
  echo "ERROR: KEANU_E2E_MODELS=openai requires OPENAI_API_KEY." >&2
  exit 2
elif [[ "$MODELS_MODE" == "anthropic" && -z "$ANTHROPIC_API_TOKEN" && -z "$ANTHROPIC_API_KEY" ]]; then
  echo "ERROR: KEANU_E2E_MODELS=anthropic requires ANTHROPIC_API_TOKEN or ANTHROPIC_API_KEY." >&2
  exit 2
fi
...
if [[ "$SKIP_PREVIOUS" == "1" ]]; then
  echo "==> Skip preinstall previous (KEANU_INSTALL_E2E_SKIP_PREVIOUS=1)"
else
...
if [[ "$INSTALL_TAG" == "beta" ]]; then
  KEANU_BETA=1 KEANU_BETA=1 curl -fsSL "$INSTALL_URL" | bash
elif [[ "$INSTALL_TAG" != "latest" ]]; then
  KEANU_VERSION="$INSTALL_TAG" KEANU_VERSION="$INSTALL_TAG" curl -fsSL "$INSTALL_URL" | bash
else
