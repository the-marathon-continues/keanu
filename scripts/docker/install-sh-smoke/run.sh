...
INSTALL_URL="${KEANU_INSTALL_URL:-https://openclaw.bot/install.sh}"
SMOKE_PREVIOUS_VERSION="${KEANU_INSTALL_SMOKE_PREVIOUS:-}"
SKIP_PREVIOUS="${KEANU_INSTALL_SMOKE_SKIP_PREVIOUS:-0}"
DEFAULT_PACKAGE="openclaw"
PACKAGE_NAME="${KEANU_INSTALL_PACKAGE:-$DEFAULT_PACKAGE}"
...
if [[ "$SKIP_PREVIOUS" == "1" ]]; then
  echo "==> Skip preinstall previous (KEANU_INSTALL_SMOKE_SKIP_PREVIOUS=1)"
else
...
if [[ -n "${KEANU_INSTALL_LATEST_OUT:-}" ]]; then
  printf "%s" "$LATEST_VERSION" > "${KEANU_INSTALL_LATEST_OUT:-}"
fi
if [[ -n "$CMD_PATH" ]]; then
