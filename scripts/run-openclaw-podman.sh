...
LAUNCH_SCRIPT="$KEANU_HOME/run-openclaw-podman.sh"
...
if [[ "${1:-}" == "launch" ]]; then
  shift
  if [[ -n "${KEANU_UID:-}" && "$(id -u)" -ne "$KEANU_UID" ]]; then
    # Exec as openclaw with cwd=/tmp so a nologin user never inherits an invalid cwd.
    exec sudo -u "$KEANU_USER" env HOME="$KEANU_HOME" PATH="$PATH" TERM="${TERM:-}" \
...
EFFECTIVE_HOME="${HOME:-}"
if [[ -n "${KEANU_UID:-}" && "$(id -u)" -eq "$KEANU_UID" ]]; then
  EFFECTIVE_HOME="$KEANU_HOME"
  export HOME="$KEANU_HOME"
fi
if [[ -z "${EFFECTIVE_HOME:-}" ]]; then
  EFFECTIVE_HOME="${KEANU_HOME:-/tmp}"
fi
CONFIG_DIR="${KEANU_CONFIG_DIR:-$EFFECTIVE_HOME/.openclaw}"
ENV_FILE="${KEANU_PODMAN_ENV:-$CONFIG_DIR/.env}"
WORKSPACE_DIR="${KEANU_WORKSPACE_DIR:-$CONFIG_DIR/workspace}"
CONTAINER_NAME="${KEANU_PODMAN_CONTAINER:-openclaw}"
KEANU_IMAGE="${KEANU_PODMAN_IMAGE:-openclaw:local}"
PODMAN_PULL="${KEANU_PODMAN_PULL:-never}"
HOST_GATEWAY_PORT="${KEANU_PODMAN_GATEWAY_HOST_PORT:-${KEANU_GATEWAY_PORT:-18789}}"
HOST_BRIDGE_PORT="${KEANU_PODMAN_BRIDGE_HOST_PORT:-${KEANU_BRIDGE_PORT:-18790}}"
GATEWAY_BIND="${KEANU_GATEWAY_BIND:-lan}"
...
if [[ -z "${KEANU_GATEWAY_TOKEN:-}" ]]; then
  export KEANU_GATEWAY_TOKEN="$(generate_token_hex_32)"
  mkdir -p "$(dirname "$ENV_FILE")"
  upsert_env_var "$ENV_FILE" "KEANU_GATEWAY_TOKEN" "$KEANU_GATEWAY_TOKEN"
  echo "Generated KEANU_GATEWAY_TOKEN and wrote it to $ENV_FILE." >&2
fi
...
PODMAN_USERNS="${KEANU_PODMAN_USERNS:-keep-id}"
USERNS_ARGS=()
RUN_USER_ARGS=()
case "$PODMAN_USERNS" in
  ...
  *)
    echo "Unsupported KEANU_PODMAN_USERNS=$PODMAN_USERNS (expected: keep-id, auto, host)." >&2
    exit 2
    ;;
esac
...
if [[ "$PODMAN_USERNS" == "keep-id" ]]; then
  ...
  echo "Starting container without --user (KEANU_PODMAN_USERNS=$PODMAN_USERNS), mounts may require ownership fixes." >&2
fi
...
if [[ "$RUN_SETUP" == true ]]; then
  exec podman run --pull="$PODMAN_PULL" --rm -it \
    ...
    -e KEANU_GATEWAY_TOKEN="$KEANU_GATEWAY_TOKEN" \
