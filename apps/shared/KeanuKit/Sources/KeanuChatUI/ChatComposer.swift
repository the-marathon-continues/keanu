...
@MainActor
struct KeanuChatComposer: View {
    @Bindable var viewModel: KeanuChatViewModel
        ...
        .background {
            ...
            if self.style == .standard {
                let shape = UnevenRoundedRectangle(
                    ...
                    style: .continuous)
                shape
                    .fill(KeanuChatTheme.composerBackground)
                    .overlay(shape.strokeBorder(KeanuChatTheme.composerBorder, lineWidth: 1))
        ...
        ScrollView(.horizontal, showsIndicators: false) {
            HStack(spacing: 6) {
                ForEach(
                    ...
                    id: \KeanuPendingAttachment.id)
                { (att: KeanuPendingAttachment) in
                    HStack(spacing: 6) {
                        if let img = att.preview {
                            KeanuPlatformImageFactory.image(img)
                                .resizable()
                                .scaledToFill()
    ...
    private var editor: some View {
        ...
        .background(
            RoundedRectangle(cornerRadius: 12, style: .continuous)
                .fill(KeanuChatTheme.composerField)
                .overlay(
                    RoundedRectangle(cornerRadius: 12, style: .continuous)
                        .strokeBorder(KeanuChatTheme.composerBorder)))
    ...
    private var connectionPill: some View {
        ...
        .padding(.horizontal, 8)
        .padding(.vertical, 4)
        .background(KeanuChatTheme.subtleCard)
        .clipShape(Capsule())
    }
