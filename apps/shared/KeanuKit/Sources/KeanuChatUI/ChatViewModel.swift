...
private let chatUILogger = Logger(subsystem: "ai.keanu", category: "KeanuChatUI")

@MainActor
@Observable
public final class KeanuChatViewModel {
    public private(set) var messages: [KeanuChatMessage] = []
    ...
    public var attachments: [KeanuPendingAttachment] = []
    ...
    public private(set) var pendingToolCalls: [KeanuChatPendingToolCall] = []
    public private(set) var sessions: [KeanuChatSessionEntry] = []
    private let transport: any KeanuChatTransport
    ...
    private var pendingToolCallsById: [String: KeanuChatPendingToolCall] = [:] {
        didSet {
    ...
    public init(sessionKey: String, transport: any KeanuChatTransport) {
    ...
    public var sessionChoices: [KeanuChatSessionEntry] {
        let now = Date().timeIntervalSince1970 * 1000
        let cutoff = now - (24 * 60 * 60 * 1000)
        let sorted = self.sessions.sorted { ($0.updatedAt ?? 0) > ($1.updatedAt ?? 0) }

        var result: [KeanuChatSessionEntry] = []
    ...
    public func removeAttachment(_ id: KeanuPendingAttachment.ID) {
    ...
    private static func decodeMessages(_ raw: [AnyCodable]) -> [KeanuChatMessage] {
        let decoded = raw.compactMap { item in
            (try? ChatPayloadDecoding.decode(item, as: KeanuChatMessage.self))
                .map { Self.stripInboundMetadata(from: $0) }
        }
        return Self.dedupeMessages(decoded)
    }

    private static func stripInboundMetadata(from message: KeanuChatMessage) -> KeanuChatMessage {
        guard message.role.lowercased() == "user" else {
            return message
        }

        let sanitizedContent = message.content.map { content -> KeanuChatMessageContent in
            guard let text = content.text else { return content }
            let cleaned = ChatMarkdownPreprocessor.preprocess(markdown: text).cleaned
            return KeanuChatMessageContent(
                type: content.type,
                text: cleaned,
    ...
    private static func messageIdentityKey(for message: KeanuChatMessage) -> String? {
    ...
    private static func reconcileMessageIDs(
        previous: [KeanuChatMessage],
        incoming: [KeanuChatMessage]) -> [KeanuChatMessage]
    {
    ...
    private static func dedupeMessages(_ messages: [KeanuChatMessage]) -> [KeanuChatMessage] {
        var result: [KeanuChatMessage] = []
    ...
    private static func dedupeKey(for message: KeanuChatMessage) -> String? {
    ...
    private func performSend() async {
        ...
        var userContent: [KeanuChatMessageContent] = [
            KeanuChatMessageContent(
                type: "text",
                text: messageText,
                thinking: nil,
        ...
        let encodedAttachments = self.attachments.map { att -> KeanuChatAttachmentPayload in
            KeanuChatAttachmentPayload(
                type: att.type,
    ...
    private func placeholderSession(key: String) -> KeanuChatSessionEntry {
        KeanuChatSessionEntry(
            key: key,
            kind: nil,
            displayName: nil,
            surface: nil,
    ...
    private func handleTransportEvent(_ evt: KeanuChatTransportEvent) {
        switch evt {
        case let .health(ok):
            self.healthOK = ok
        case .tick:
    ...
    private func handleChatEvent(_ chat: KeanuChatEventPayload) {
    ...
    private func handleAgentEvent(_ evt: KeanuAgentEventPayload) {
        ...
        case "tool":
            ...
            if phase == "start" {
                ...
                self.pendingToolCallsById[toolCallId] = KeanuChatPendingToolCall(
    ...
    private static func previewImage(data: Data) -> KeanuPlatformImage? {
        #if canImport(AppKit)
        NSImage(data: data)
        #elseif canImport(UIKit)
        UIImage(data: data)
