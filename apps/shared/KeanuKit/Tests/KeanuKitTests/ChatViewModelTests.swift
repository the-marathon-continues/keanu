...
private final class TestChatTransport: @unchecked Sendable, KeanuChatTransport {
    private let state = TestChatTransportState()
    private let historyResponses: [KeanuChatHistoryPayload]
    private let sessionsResponses: [KeanuChatSessionsListResponse]

    private let stream: AsyncStream<KeanuChatTransportEvent>
    private let continuation: AsyncStream<KeanuChatTransportEvent>.Continuation

    init(
        historyResponses: [KeanuChatHistoryPayload],
        sessionsResponses: [KeanuChatSessionsListResponse] = [])
    {
        self.historyResponses = historyResponses
        self.sessionsResponses = sessionsResponses
        var cont: AsyncStream<KeanuChatTransportEvent>.Continuation!
    ...
    func requestHistory(sessionKey: String) async throws -> KeanuChatHistoryPayload {
        ...
        return self.historyResponses.last ?? KeanuChatHistoryPayload(
        ...
        attachments _: [KeanuChatAttachmentPayload]) async throws -> KeanuChatSendResponse
    {
        ...
        return KeanuChatSendResponse(runId: idempotencyKey, status: "ok")
    }
    ...
    func listSessions(limit _: Int?) async throws -> KeanuChatSessionsListResponse {
        ...
        return self.sessionsResponses.last ?? KeanuChatSessionsListResponse(
            ts: nil,
            path: nil,
            count: 0,
            defaults: nil,
    ...
    func emit(_ evt: KeanuChatTransportEvent) {
...
@Suite struct ChatViewModelTests {
    @Test func streamsAssistantAndClearsOnFinal() async throws {
        ...
        let history1 = KeanuChatHistoryPayload(
            sessionKey: "main",
            sessionId: sessionId,
            messages: [],
            thinkingLevel: "off")
        let history2 = KeanuChatHistoryPayload(
            sessionKey: "main",
            sessionId: sessionId,
            messages: [
                AnyCodable([
        ...
        let vm = await MainActor.run { KeanuChatViewModel(sessionKey: "main", transport: transport) }
    ...
    @Test func clearsStreamingOnExternalFinalEvent() async throws {
        let sessionId = "sess-main"
        let history = KeanuChatHistoryPayload(
            sessionKey: "main",
            sessionId: sessionId,
            messages: [],
            thinkingLevel: "off")
    ...
    @Test func sessionChoicesPreferMainAndRecent() async throws {
        ...
        let history = KeanuChatHistoryPayload(
            ...
            messages: [],
            thinkingLevel: "off")
        let sessions = KeanuChatSessionsListResponse(
            ts: now,
            path: nil,
            count: 4,
            defaults: nil,
    ...
    @Test func sessionChoicesIncludeCurrentWhenMissing() async throws {
        ...
        let transport = TestChatTransport(
            historyResponses: [history],
            sessionsResponses: [sessions])
        let vm = await MainActor.run { KeanuChatViewModel(sessionKey: "custom", transport: transport) }
        await MainActor.run { vm.load() }
        try await waitUntil("sessions loaded") { await MainActor.run { !vm.sessions.isEmpty } }
