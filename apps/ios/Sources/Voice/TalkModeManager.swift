...
final class TalkModeManager: NSObject {
    ...
    private static let redactedConfigSentinel = "__KEANU_REDACTED__"
    ...
    private var pttCompletion: CheckedContinuation<KeanuTalkPTTStopPayload, Never>?
    ...
    func stop() {
        ...
        if pendingPTT {
            let payload = KeanuTalkPTTStopPayload(
    ...
    func beginPushToTalk() async throws -> KeanuTalkPTTStartPayload {
        guard self.gatewayConnected else {
            self.statusText = "Offline"
        ...
        if self.isPushToTalkActive, let captureId = self.activePTTCaptureId {
            return KeanuTalkPTTStartPayload(captureId: captureId)
        }
    ...
    func endPushToTalk() async -> KeanuTalkPTTStopPayload {
    ...
    func runPushToTalkOnce(maxDurationSeconds: TimeInterval = 12) async throws -> KeanuTalkPTTStopPayload {
        ...
        if self.isPushToTalkActive {
            let captureId = self.activePTTCaptureId ?? UUID().uuidString
            return KeanuTalkPTTStopPayload(
                captureId: captureId,
                transcript: nil,
                status: "busy")
        }
    ...
    func cancelPushToTalk() async -> KeanuTalkPTTStopPayload {
        let captureId = self.activePTTCaptureId ?? UUID().uuidString
        guard self.isPushToTalkActive else {
            let payload = KeanuTalkPTTStopPayload(
                captureId: captureId,
    ...
    private func finishPTTOnce(_ payload: KeanuTalkPTTStopPayload) {
    ...
    private func streamAssistant(runId: String, gateway: GatewayNodeSession) async {
        ...
        for await evt in stream {
            ...
            guard let agentEvent = try? GatewayPayloadDecoding.decode(payload, as: KeanuAgentEventPayload.self) else {
                continue
            }
