...
final class MotionService: MotionServicing {
    func activities(params: KeanuMotionActivityParams) async throws -> KeanuMotionActivityPayload {
        ...
        let mapped = try await withCheckedThrowingContinuation { (cont: CheckedContinuation<[KeanuMotionActivityEntry], Error>) in
        ...
        return KeanuMotionActivityPayload(activities: mapped)
    }
    ...
    func pedometer(params: KeanuPedometerParams) async throws -> KeanuPedometerPayload {
        ...
        let payload = try await withCheckedThrowingContinuation { (cont: CheckedContinuation<KeanuPedometerPayload, Error>) in
            pedometer.queryPedometerData(from: start, to: end) { data, error in
                if let error {
                    cont.resume(throwing: error)
                } else {
                    let formatter = ISO8601DateFormatter()
                    let payload = KeanuPedometerPayload(
