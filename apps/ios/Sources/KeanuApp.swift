...
@MainActor
final class KeanuAppDelegate: NSObject, UIApplicationDelegate, @preconcurrency UNUserNotificationCenterDelegate {
    private let logger = Logger(subsystem: "ai.keanu.ios", category: "Push")
    private let backgroundWakeLogger = Logger(subsystem: "ai.keanu.ios", category: "BackgroundWake")
    private static let wakeRefreshTaskIdentifier = "ai.keanu.ios.bgrefresh"
...
enum WatchPromptNotificationBridge {
    static let typeKey = "keanu.type"
    static let typeValue = "watch.prompt"
    static let promptIDKey = "keanu.watch.promptId"
    static let sessionKeyKey = "keanu.watch.sessionKey"
    static let actionPrimaryIDKey = "keanu.watch.action.primary.id"
    static let actionPrimaryLabelKey = "keanu.watch.action.primary.label"
    static let actionSecondaryIDKey = "keanu.watch.action.secondary.id"
    static let actionSecondaryLabelKey = "keanu.watch.action.secondary.label"
    static let actionPrimaryIdentifier = "keanu.watch.action.primary"
    static let actionSecondaryIdentifier = "keanu.watch.action.secondary"
    static let actionIdentifierPrefix = "keanu.watch.action."
    static let actionIDKeyPrefix = "keanu.watch.action.id."
    static let actionLabelKeyPrefix = "keanu.watch.action.label."
    static let categoryPrefix = "keanu.watch.prompt.category."

    @MainActor
    static func scheduleMirroredWatchPromptNotificationIfNeeded(
        invokeID: String,
        params: KeanuWatchNotifyParams,
        sendResult: WatchNotificationSendResult) async
    {
        ...
        let normalizedActions = (params.actions ?? []).compactMap { action -> KeanuWatchAction? in
            let id = action.id.trimmingCharacters(in: .whitespacesAndNewlines)
            let label = action.label.trimmingCharacters(in: .whitespacesAndNewlines)
            guard !id.isEmpty, !label.isEmpty else { return nil }
            return KeanuWatchAction(id: id, label: label, style: action.style)
        }
        ...
        content.title = title.isEmpty ? "Keanu" : title
        content.body = body
    ...
    private static func categoryActions(_ actions: [KeanuWatchAction]) -> [UNNotificationAction] {
...
struct KeanuApp: App {
    ...
    @UIApplicationDelegateAdaptor(KeanuAppDelegate.self) private var appDelegate
...
extension KeanuApp {
    private static func installUncaughtExceptionLogger() {
        NSLog("Keanu: installing uncaught exception handler")
