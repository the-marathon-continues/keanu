...
struct IOSGatewayChatTransport: KeanuChatTransport, Sendable {
    private static let logger = Logger(subsystem: "ai.keanu", category: "ios.chat.transport")
    ...
    func listSessions(limit: Int?) async throws -> KeanuChatSessionsListResponse {
        ...
        return try JSONDecoder().decode(KeanuChatSessionsListResponse.self, from: res)
    }
    ...
    func requestHistory(sessionKey: String) async throws -> KeanuChatHistoryPayload {
        ...
        return try JSONDecoder().decode(KeanuChatHistoryPayload.self, from: res)
    }
        ...
        attachments: [KeanuChatAttachmentPayload]) async throws -> KeanuChatSendResponse
    {
        ...
        struct Params: Codable {
            ...
            var message: String
            var thinking: String
            var attachments: [KeanuChatAttachmentPayload]?
            var timeoutMs: Int
        ...
        do {
            ...
            let decoded = try JSONDecoder().decode(KeanuChatSendResponse.self, from: res)
    ...
    func requestHealth(timeoutMs: Int) async throws -> Bool {
        ...
        return (try? JSONDecoder().decode(KeanuGatewayHealthOK.self, from: res))?.ok ?? true
    }
                ...
                for await evt in stream {
                    ...
                    case "health":
                        ...
                        let ok = (try? GatewayPayloadDecoding.decode(
                            payload,
                            as: KeanuGatewayHealthOK.self))?.ok ?? true
                        continuation.yield(.health(ok: ok))
                    case "chat":
                        guard let payload = evt.payload else { break }
                        if let chatPayload = try? GatewayPayloadDecoding.decode(
                            payload,
                            as: KeanuChatEventPayload.self)
                        {
                    ...
                    case "agent":
                        ...
                        if let agentPayload = try? GatewayPayloadDecoding.decode(
                            payload,
                            as: KeanuAgentEventPayload.self)
                        {
