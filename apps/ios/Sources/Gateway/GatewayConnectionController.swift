...
final class GatewayConnectionController {
    ...
    private func currentCaps() -> [String] {
        var caps = [KeanuCapability.canvas.rawValue, KeanuCapability.screen.rawValue]
        ...
        if cameraEnabled { caps.append(KeanuCapability.camera.rawValue) }

        let voiceWakeEnabled = UserDefaults.standard.bool(forKey: VoiceWakePreferences.enabledKey)
        if voiceWakeEnabled { caps.append(KeanuCapability.voiceWake.rawValue) }

        let locationModeRaw = UserDefaults.standard.string(forKey: "location.enabledMode") ?? "off"
        let locationMode = KeanuLocationMode(rawValue: locationModeRaw) ?? .off
        if locationMode != .off { caps.append(KeanuCapability.location.rawValue) }

        caps.append(KeanuCapability.device.rawValue)
        if WatchMessagingService.isSupportedOnDevice() {
            caps.append(KeanuCapability.watch.rawValue)
        }
        caps.append(KeanuCapability.photos.rawValue)
        caps.append(KeanuCapability.contacts.rawValue)
        caps.append(KeanuCapability.calendar.rawValue)
        caps.append(KeanuCapability.reminders.rawValue)
        if Self.motionAvailable() {
            caps.append(KeanuCapability.motion.rawValue)
        }
    ...
    private func currentCommands() -> [String] {
        var commands: [String] = [
            ...
            KeanuScreenCommand.record.rawValue,
            KeanuSystemCommand.notify.rawValue,
            KeanuChatCommand.push.rawValue,
        ...
        if caps.contains(KeanuCapability.camera.rawValue) {
            commands.append(KeanuCameraCommand.list.rawValue)
            commands.append(KeanuCameraCommand.snap.rawValue)
            commands.append(KeanuCameraCommand.clip.rawValue)
        }
        if caps.contains(KeanuCapability.location.rawValue) {
            commands.append(KeanuLocationCommand.get.rawValue)
        }
        if caps.contains(KeanuCapability.device.rawValue) {
            commands.append(KeanuDeviceCommand.status.rawValue)
            commands.append(KeanuDeviceCommand.info.rawValue)
        }
        if caps.contains(KeanuCapability.watch.rawValue) {
            commands.append(KeanuWatchCommand.status.rawValue)
            commands.append(KeanuWatchCommand.notify.rawValue)
        }
        if caps.contains(KeanuCapability.photos.rawValue) {
            commands.append(KeanuPhotosCommand.latest.rawValue)
        }
        if caps.contains(KeanuCapability.contacts.rawValue) {
            commands.append(KeanuContactsCommand.search.rawValue)
            commands.append(KeanuContactsCommand.add.rawValue)
        }
        if caps.contains(KeanuCapability.calendar.rawValue) {
            commands.append(KeanuCalendarCommand.events.rawValue)
            commands.append(KeanuCalendarCommand.add.rawValue)
        }
        if caps.contains(KeanuCapability.reminders.rawValue) {
            commands.append(KeanuRemindersCommand.list.rawValue)
            commands.append(KeanuRemindersCommand.add.rawValue)
        }
        if caps.contains(KeanuCapability.motion.rawValue) {
            commands.append(KeanuMotionCommand.activity.rawValue)
            commands.append(KeanuMotionCommand.pedometer.rawValue)
        }
