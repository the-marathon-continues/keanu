...
final class ContactsService: ContactsServicing {
    ...
    func search(params: KeanuContactsSearchParams) async throws -> KeanuContactsSearchPayload {
        ...
        return KeanuContactsSearchPayload(contacts: payload)
    }
    ...
    func add(params: KeanuContactsAddParams) async throws -> KeanuContactsAddPayload {
        ...
        if !phoneNumbers.isEmpty || !emails.isEmpty {
            if let existing = try Self.findExistingContact(
                ...
                emails: emails)
            {
                return KeanuContactsAddPayload(contact: Self.payload(from: existing))
            }
        }
        ...
        return KeanuContactsAddPayload(contact: Self.payload(from: persisted))
    }
    ...
    private static func payload(from contact: CNContact) -> KeanuContactPayload {
        KeanuContactPayload(
