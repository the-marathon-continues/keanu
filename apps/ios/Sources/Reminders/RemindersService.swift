...
final class RemindersService: RemindersServicing {
    func list(params: KeanuRemindersListParams) async throws -> KeanuRemindersListPayload {
        ...
        let payload = try await withCheckedThrowingContinuation { (cont: CheckedContinuation<[KeanuReminderPayload], Error>) in
        ...
        return KeanuRemindersListPayload(reminders: payload)
    }
    ...
    func add(params: KeanuRemindersAddParams) async throws -> KeanuRemindersAddPayload {
        ...
        let formatter = ISO8601DateFormatter()
        let due = reminder.dueDateComponents.flatMap { Calendar.current.date(from: $0) }
        let payload = KeanuReminderPayload(
            identifier: reminder.calendarItemIdentifier,
            title: reminder.title,
            dueISO: due.map { formatter.string(from: $0) },
            completed: reminder.isCompleted,
        ...
        return KeanuRemindersAddPayload(reminder: payload)
    }
