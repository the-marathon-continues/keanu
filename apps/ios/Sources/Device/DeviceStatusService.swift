...
final class DeviceStatusService: DeviceStatusServicing {
    ...
    func status() async throws -> KeanuDeviceStatusPayload {
        ...
        return KeanuDeviceStatusPayload(
            battery: battery,
            thermal: thermal,
            storage: storage,
            network: network,
    ...
    private func batteryStatus() -> KeanuBatteryStatusPayload {
        let device = UIDevice.current
        device.isBatteryMonitoringEnabled = true
        let level = device.batteryLevel >= 0 ? Double(device.batteryLevel) : nil
        let state: KeanuBatteryState = switch device.batteryState {
        ...
        return KeanuBatteryStatusPayload(
            level: level,
            state: state,
            lowPowerModeEnabled: ProcessInfo.processInfo.isLowPowerModeEnabled)
    }

    private func thermalStatus() -> KeanuThermalStatusPayload {
        let state: KeanuThermalState = switch ProcessInfo.processInfo.thermalState {
        ...
        return KeanuThermalStatusPayload(state: state)
    }

    private func storageStatus() -> KeanuStorageStatusPayload {
        let attrs = (try? FileManager.default.attributesOfFileSystem(forPath: NSHomeDirectory())) ?? [:]
        let total = (attrs[.systemSize] as? NSNumber)?.int64Value ?? 0
        let free = (attrs[.systemFreeSize] as? NSNumber)?.int64Value ?? 0
        let used = max(0, total - free)
        return KeanuStorageStatusPayload(totalBytes: total, freeBytes: free, usedBytes: used)
    }
