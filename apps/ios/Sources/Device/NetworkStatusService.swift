...
final class NetworkStatusService: @unchecked Sendable {
    func currentStatus(timeoutMs: Int = 1500) async -> KeanuNetworkStatusPayload {
    ...
    private static func payload(from path: NWPath) -> KeanuNetworkStatusPayload {
        let status: KeanuNetworkPathStatus = switch path.status {
    ...
    private static func fallbackPayload() -> KeanuNetworkStatusPayload {
        KeanuNetworkStatusPayload(
            status: .unsatisfied,
            isExpensive: false,
            isConstrained: false,
            interfaces: [.other])
