...
final class ShareViewController: UIViewController {
    ...
    private let logger = Logger(subsystem: "ai.keanu.ios", category: "ShareExtension")
    ...
    private func setupUI() {
        ...
        self.sendButton.setTitle("Send to Keanu", for: .normal)
    ...
    private func sendCurrentDraft() async {
        ...
        self.showStatus("Sending to Keanu gateway…")
    ...
    private func sendMessageToGateway(_ message: String, attachments: [ShareAttachment]) async throws {
        guard let config = ShareGatewayRelaySettings.loadConfig() else {
            throw NSError(
                domain: "KeanuShare",
                code: 10,
                userInfo: [NSLocalizedDescriptionKey: "Keanu is not connected to a gateway yet."])
        }
        ...
        do {
            try await gateway.connect(
                ...
                connectOptions: makeOptions("keanu-ios"),
                sessionBox: nil,
                onConnected: {},
                onDisconnected: { _ in },
                onInvoke: { req in
