...
struct ControlAgentEvent: Codable, Sendable, Identifiable {
    ...
    let stream: String
    let ts: Double
    let data: [String: KeanuProtocol.AnyCodable]
    let summary: String?
}
...
final class ControlChannel {
    ...
    private let logger = Logger(subsystem: "ai.keanu", category: "control")
    ...
    {
        do {
            let rawParams = params?.reduce(into: [String: KeanuKit.AnyCodable]()) {
                $0[$1.key] = KeanuKit.AnyCodable($1.value.base)
            }
    ...
    private static func bridgeToProtocolArgs(
        _ value: KeanuProtocol.AnyCodable?) -> [String: KeanuProtocol.AnyCodable]?
    {
        guard let value else { return nil }
        if let dict = value.value as? [String: KeanuProtocol.AnyCodable] {
            return dict
        }
        if let dict = value.value as? [String: KeanuKit.AnyCodable],
           let data = try? JSONEncoder().encode(dict),
           let decoded = try? JSONDecoder().decode([String: KeanuProtocol.AnyCodable].self, from: data)
        {
            return decoded
        }
