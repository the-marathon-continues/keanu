...
enum SessionMenuPreviewLoader {
    private static let logger = Logger(subsystem: "ai.keanu", category: "SessionPreview")
    ...
    private static func requestPreview(
        keys: [String],
        maxItems: Int) async throws -> KeanuSessionsPreviewPayload
    {
    ...
    private static func snapshot(
        from entry: KeanuSessionPreviewEntry,
    ...
    private static func cache(payload: KeanuSessionsPreviewPayload, maxItems: Int) async {
    ...
    private static func previewItems(
        from payload: KeanuChatHistoryPayload,
        maxItems: Int) -> [SessionPreviewItem]
    {
        let boundedItems = self.normalizeMaxItems(maxItems)
        let raw: [KeanuKit.AnyCodable] = payload.messages ?? []
    ...
    private static func decodeMessages(_ raw: [KeanuKit.AnyCodable]) -> [KeanuChatMessage] {
        raw.compactMap { item in
            guard let data = try? JSONEncoder().encode(item) else { return nil }
            return try? JSONDecoder().decode(KeanuChatMessage.self, from: data)
        }
    }
    ...
    private static func previewText(for message: KeanuChatMessage) -> String? {
    ...
    private static func isToolCall(_ message: KeanuChatMessage) -> Bool {
    ...
    private static func toolNames(for message: KeanuChatMessage) -> [String] {
        var names: [String] = []
    ...
    private static func mediaSummary(for message: KeanuChatMessage) -> String? {
