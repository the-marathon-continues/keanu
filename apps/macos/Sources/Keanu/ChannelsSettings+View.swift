...
extension ChannelsSettings {
    var body: some View {
        NavigationSplitView(columnVisibility: .constant(.all)) {
            ...
            IntegrationsGridView(
                ...
                showSetupWizard: self.$showSetupWizard,
                setupWizardChannelId: self.$setupWizardChannelId
            )
        ...
        .sheet(isPresented: self.$showSetupWizard) {
            if let channelId = self.setupWizardChannelId {
                IntegrationSetupWizard(
                    store: self.store,
                    channelId: channelId,
                    isPresented: self.$showSetupWizard
                )
            }
        }
    }

    // MARK: - Legacy sidebar (kept for reference, not used in new UI)
    ...
    private var emptyDetail: some View {
        VStack(spacing: 16) {
            ...
            Text("Click a card to view status and configure settings.")
                .font(.callout)
    ...
    func channelDetail(_ channel: ChannelItem) -> some View {
        ScrollView(.vertical) {
    ...
    func detailHeader(for channel: ChannelItem) -> some View {
