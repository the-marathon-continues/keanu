...
private let webChatSwiftLogger = Logger(subsystem: "ai.keanu", category: "WebChatSwiftUI")
...
struct MacGatewayChatTransport: KeanuChatTransport, Sendable {
    ...
    func listSessions(limit: Int?) async throws -> KeanuChatSessionsListResponse {
        ...
        let data = try await GatewayConnection.shared.request(
            ...
            params: params,
            timeoutMs: 15000)
        return try JSONDecoder().decode(KeanuChatSessionsListResponse.self, from: data)
    }
    ...
    static func mapPushToTransportEvent(_ push: GatewayPush) -> KeanuChatTransportEvent? {
        switch push {
        case let .snapshot(hello):
            let ok = (try? JSONDecoder().decode(
                KeanuGatewayHealthOK.self,
...
final class WebChatSwiftUIWindowController {
    ...
    private let hosting: NSHostingController<KeanuChatView>
    ...
    init(sessionKey: String, presentation: WebChatPresentation, transport: any KeanuChatTransport) {
        self.sessionKey = sessionKey
        self.presentation = presentation
        let vm = KeanuChatViewModel(sessionKey: sessionKey, transport: transport)
        let accent = Self.color(fromHex: AppStateStore.shared.seamColorHex)
        self.hosting = NSHostingController(rootView: KeanuChatView(
            viewModel: vm,
    ...
    private static func makeContentController(
        for presentation: WebChatPresentation,
        hosting: NSHostingController<KeanuChatView>) -> NSViewController
    {
