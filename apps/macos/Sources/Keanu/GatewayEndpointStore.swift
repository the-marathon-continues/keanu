...
actor GatewayEndpointStore {
    ...
    private static let staticLogger = Logger(subsystem: "ai.keanu", category: "gateway-endpoint")
    ...
    private let logger = Logger(subsystem: "ai.keanu", category: "gateway-endpoint")
    ...
    init(deps: Deps = .live) {
        ...
        let bind = GatewayEndpointStore.resolveGatewayBindMode(
            root: KeanuConfigFile.loadDict(),
            env: ProcessInfo.processInfo.environment)
        let customBindHost = GatewayEndpointStore.resolveGatewayCustomBindHost(root: KeanuConfigFile.loadDict())
