...
enum CommandResolver {
    private static let projectRootDefaultsKey = "keanu.gatewayProjectRootPath"
    private static let helperName = "keanu"

    static func gatewayEntrypoint(in root: URL) -> String? {
        let distEntry = root.appendingPathComponent("dist/index.js").path
        if FileManager().isReadableFile(atPath: distEntry) { return distEntry }
        let keanuEntry = root.appendingPathComponent("keanu.mjs").path
        if FileManager().isReadableFile(atPath: keanuEntry) { return keanuEntry }
        let binEntry = root.appendingPathComponent("bin/keanu.js").path
    ...
    static func projectRoot() -> URL {
        ...
        let fallback = FileManager().homeDirectoryForCurrentUser
            .appendingPathComponent("Projects/keanu")
    ...
    static func preferredPaths(home: URL, current: [String], projectRoot: URL) -> [String] {
        ...
        #endif
        let keanuPaths = self.keanuManagedPaths(home: home)
        if !keanuPaths.isEmpty {
            extras.insert(contentsOf: keanuPaths, at: 1)
        }
        extras.insert(contentsOf: self.nodeManagerBinPaths(home: home), at: 1 + keanuPaths.count)
        var seen = Set<String>()
        // Preserve order while stripping duplicates so PATH lookups remain deterministic.
        return (extras + current).filter { seen.insert($0).inserted }
    }

    private static func keanuManagedPaths(home: URL) -> [String] {
        let bases = [
            home.appendingPathComponent(".keanu"),
        ]
        var paths: [String] = []
        for base in bases {
    ...
    static func keanuExecutable(searchPaths: [String]? = nil) -> String? {
        self.findExecutable(named: self.helperName, searchPaths: searchPaths)
    }

    static func projectKeanuExecutable(projectRoot: URL? = nil) -> String? {
        #if DEBUG
    ...
    static func nodeCliPath() -> String? {
        ...
        let candidates = [
            root.appendingPathComponent("keanu.mjs").path,
            root.appendingPathComponent("bin/keanu.js").path,
        ]
    ...
    static func hasAnyKeanuInvoker(searchPaths: [String]? = nil) -> Bool {
        if self.keanuExecutable(searchPaths: searchPaths) != nil { return true }
        ...
        searchPaths: [String]? = nil) -> [String]
    {
        ...
        let root = self.projectRoot()
        if let keanuPath = self.projectKeanuExecutable(projectRoot: root) {
            return [keanuPath, subcommand] + extraArgs
        }
        if let keanuPath = self.keanuExecutable(searchPaths: searchPaths) {
            return [keanuPath, subcommand] + extraArgs
        }
        ...
        if let pnpm = self.findExecutable(named: "pnpm", searchPaths: searchPaths) {
            ...
            return [pnpm, "--silent", "keanu", subcommand] + extraArgs
        }

        switch runtimeResult {
        case .success:
            let missingEntry = """
            keanu entrypoint missing (looked for dist/index.js or keanu.mjs); run pnpm build.
            """
    ...
    private static func sshNodeCommand(subcommand: String, extraArgs: [String], settings: RemoteSettings) -> [String]? {
        ...
        // Run the real keanu CLI on the remote host.
        let exportedPath = [
            "/opt/homebrew/bin",
            "/usr/local/bin",
            "/usr/bin",
        ...
        let projectSection = if userPRJ.isEmpty {
            """
            DEFAULT_PRJ="$HOME/Projects/keanu"
        ...
        elif [ -n "${PRJ:-}" ] && [ -f "$PRJ/keanu.mjs" ]; then
          if command -v node >/dev/null 2>&1; then
            CLI="node $PRJ/keanu.mjs"
            node "$PRJ/keanu.mjs" \(quotedArgs);
          else
            echo "Node >=22 required on remote host"; exit 127;
          fi
        elif [ -n "${PRJ:-}" ] && [ -f "$PRJ/bin/keanu.js" ]; then
          if command -v node >/dev/null 2>&1; then
            CLI="node $PRJ/bin/keanu.js"
            node "$PRJ/bin/keanu.js" \(quotedArgs);
          else
            echo "Node >=22 required on remote host"; exit 127;
          fi
        elif command -v pnpm >/dev/null 2>&1; then
          CLI="pnpm --silent keanu"
          pnpm --silent keanu \(quotedArgs);
        else
          echo "keanu CLI missing on remote host"; exit 127;
        fi
        """
        let options: [String] = [
            "-o", "BatchMode=yes",
    ...
    {
        let root = configRoot ?? KeanuConfigFile.loadDict()
