...
struct TailscaleIntegrationSection: View {
    ...
    private var authFields: some View {
        ...
        Text("Stored in ~/.keanu/keanu.json. Prefer KEANU_GATEWAY_PASSWORD for production.")
            .font(.caption)
            .foregroundStyle(.secondary)
    ...
    private func applySettings() async {
        ...
        if self.connectionMode == .local, !self.isPaused {
            self.statusMessage = "Saved to ~/.keanu/keanu.json. Restarting gateway…"
        } else {
            self.statusMessage = "Saved to ~/.keanu/keanu.json. Restart the gateway to apply."
        }
