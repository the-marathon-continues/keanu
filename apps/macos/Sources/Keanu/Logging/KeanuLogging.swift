...
enum KeanuLogging {
    ...
    private static let didBootstrap: Void = {
        LoggingSystem.bootstrap { label in
            ...
            let osHandler = KeanuOSLogHandler(subsystem: subsystem, category: category)
            let fileHandler = KeanuFileLogHandler(label: label)
...
extension Logging.Logger {
    init(subsystem: String, category: String) {
        KeanuLogging.bootstrapIfNeeded()
        let label = KeanuLogging.makeLabel(subsystem: subsystem, category: category)
...
struct KeanuOSLogHandler: LogHandler {
...
struct KeanuFileLogHandler: LogHandler {
    let label: String
    var metadata: Logger.Metadata = [:]
        ...
        line: UInt)
    {
        ...
        let (subsystem, category) = KeanuLogging.parseLabel(self.label)
