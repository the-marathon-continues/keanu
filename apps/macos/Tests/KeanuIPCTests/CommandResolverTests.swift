...
@Suite(.serialized) struct CommandResolverTests {
    ...
    @Test func prefersKeanuBinary() async throws {
    ...
    @Test func prefersKeanuBinaryOverPnpm() async throws {
        let defaults = self.makeDefaults()
        defaults.set(AppState.ConnectionMode.local.rawValue, forKey: connectionModeKey)
    ...
    @Test func usesKeanuBinaryWithoutNodeRuntime() async throws {
        let defaults = self.makeDefaults()
        defaults.set(AppState.ConnectionMode.local.rawValue, forKey: connectionModeKey)
    ...
    @Test func fallsBackToPnpm() async throws {
        ...
        #expect(cmd.prefix(4).elementsEqual([pnpmPath.path, "--silent", "keanu", "rpc"]))
    }
    ...
    @Test func pnpmKeepsExtraArgsAfterSubcommand() async throws {
        ...
        #expect(cmd.prefix(5).elementsEqual([pnpmPath.path, "--silent", "keanu", "health", "--json"]))
