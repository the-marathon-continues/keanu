...
@Suite struct InstancesStoreTests {
    ...
    func presenceEventPayloadDecodesViaJSONEncoder() {
        ...
        let entry: [String: KeanuProtocol.AnyCodable] = [
            "host": .init("gw"),
            "ip": .init("10.0.0.1"),
            "version": .init("2.0.0"),
            "mode": .init("gateway"),
        ...
        let payloadMap: [String: KeanuProtocol.AnyCodable] = [
            "presence": .init([KeanuProtocol.AnyCodable(entry)]),
        ]
        let payload = KeanuProtocol.AnyCodable(payloadMap)
