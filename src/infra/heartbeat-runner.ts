...
export type HeartbeatRunner = {
  stop: () => void;
  updateConfig: (cfg: KeanuConfig) => void;
};

function hasExplicitHeartbeatAgents(cfg: KeanuConfig) {
  const list = cfg.agents?.list ?? [];
  return list.some((entry) => Boolean(entry?.heartbeat));
}

export function isHeartbeatEnabledForAgent(cfg: KeanuConfig, agentId?: string): boolean {
...
function resolveHeartbeatConfig(cfg: KeanuConfig, agentId?: string): HeartbeatConfig | undefined {
...
function resolveHeartbeatAgents(cfg: KeanuConfig): HeartbeatAgent[] {
  const list = cfg.agents?.list ?? [];
...
export function resolveHeartbeatPrompt(cfg: KeanuConfig, heartbeat?: HeartbeatConfig) {
  return resolveHeartbeatPromptText(heartbeat?.prompt ?? cfg.agents?.defaults?.heartbeat?.prompt);
}

function resolveHeartbeatAckMaxChars(cfg: KeanuConfig, heartbeat?: HeartbeatConfig) {
  return Math.max(
    0,
...
export function startHeartbeatRunner(opts: {
  ...
  const updateConfig = (cfg: KeanuConfig) => {
    if (state.stopped) {
      return;
    }
    const now = Date.now();
