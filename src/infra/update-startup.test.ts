...
describe("update-startup", () => {
  ...
  beforeAll(async () => {
    suiteRoot = await fs.mkdtemp(path.join(os.tmpdir(), "keanu-update-check-suite-"));
  });
  ...
  beforeEach(async () => {
    ...
    envSnapshot = captureEnv(["KEANU_STATE_DIR", "NODE_ENV", "VITEST"]);
    process.env.KEANU_STATE_DIR = tempDir;
    ...
    if (!loaded) {
      ({ resolveKeanuPackageRoot } = await import("./keanu-root.js"));
  ...
  function mockPackageUpdateStatus(tag = "latest", version = "2.0.0") {
    vi.mocked(resolveKeanuPackageRoot).mockResolvedValue("/opt/keanu");
  ...
  it("uses current runtime + entrypoint for default auto-update command execution", async () => {
    ...
    process.argv = [process.execPath, "/opt/keanu/dist/entry.js"];
    try {
      await runGatewayUpdateCheck({
        cfg: {
          update: {
