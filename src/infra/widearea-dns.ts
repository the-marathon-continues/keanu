...
export function resolveWideAreaDiscoveryDomain(params?: {
  ...
  const candidate = params?.configDomain ?? env.KEANU_WIDE_AREA_DOMAIN ?? null;
...
function extractContentHash(zoneText: string): string | null {
  const match = zoneText.match(/^\s*;\s*keanu-content-hash:\s*(\S+)\s*$/m);
  return match?.[1] ?? null;
}
...
function renderZone(opts: WideAreaGatewayZoneOpts & { serial: number }): string {
  const hostname = os.hostname().split(".")[0] ?? "keanu";
  const hostLabel = dnsLabel(opts.hostLabel ?? hostname, "keanu");
  const instanceLabel = dnsLabel(opts.instanceLabel ?? `${hostname}-gateway`, "keanu-gw");
  ...
  records.push(`_keanu-gw._tcp IN PTR ${instanceLabel}._keanu-gw._tcp`);
  records.push(`${instanceLabel}._keanu-gw._tcp IN SRV 0 0 ${opts.gatewayPort} ${hostLabel}`);
  records.push(`${instanceLabel}._keanu-gw._tcp IN TXT ${txt.map(txtQuote).join(" ")}`);
  ...
  return `; keanu-content-hash: ${contentHash}\n${contentBody}`;
}
