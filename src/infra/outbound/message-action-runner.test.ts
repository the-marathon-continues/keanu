...
describe("runMessageAction sandboxed media validation", () => {
  ...
  it("allows media paths under preferred Keanu tmp root", async () => {
    const tmpRoot = resolvePreferredKeanuTmpDir();
    ...
    try {
      ...
      const hostTmpOutsideKeanu = path.join(os.tmpdir(), "outside-keanu", "test-media.png");
      await expect(
        runMessageAction({
          cfg: slackConfig,
          action: "send",
