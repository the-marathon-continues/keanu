...
export class DirectoryCache<T> {
  ...
  private lastConfigRef: KeanuConfig | null = null;
  ...
  get(key: string, cfg: KeanuConfig): T | undefined {
  ...
  set(key: string, value: T, cfg: KeanuConfig): void {
    this.resetIfConfigChanged(cfg);
    const now = Date.now();
  ...
  private resetIfConfigChanged(cfg: KeanuConfig): void {
