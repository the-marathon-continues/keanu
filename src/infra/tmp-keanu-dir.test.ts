...
import { POSIX_KEANU_TMP_DIR, resolvePreferredKeanuTmpDir } from "./tmp-keanu-dir.js";

type TmpDirOptions = NonNullable<Parameters<typeof resolvePreferredKeanuTmpDir>[0]>;

function fallbackTmp(uid = 501) {
  return path.join("/var/fallback", `keanu-${uid}`);
}
...
function resolveWithMocks(params: {
  ...
  const resolved = resolvePreferredKeanuTmpDir({
    accessSync,
...
describe("resolvePreferredKeanuTmpDir", () => {
  it("prefers /tmp/keanu when it already exists and is writable", () => {
    ...
    expect(resolved).toBe(POSIX_KEANU_TMP_DIR);
    expect(tmpdir).not.toHaveBeenCalled();
  });

  it("prefers /tmp/keanu when it does not exist but /tmp is writable", () => {
    ...
    expect(mkdirSync).toHaveBeenCalledWith(POSIX_KEANU_TMP_DIR, expect.any(Object));
    expect(tmpdir).not.toHaveBeenCalled();
  });

  it("falls back to os.tmpdir()/keanu when /tmp/keanu is not a directory", () => {
  ...
  it("falls back to os.tmpdir()/keanu when /tmp is not writable", () => {
  ...
  it("falls back when /tmp/keanu is a symlink", () => {
    const lstatSync = vi.fn(() => ({
      isDirectory: () => true,
      isSymbolicLink: () => true,
      uid: 501,
  ...
  it("falls back when /tmp/keanu is not owned by the current user", () => {
    const lstatSync = vi.fn(() => ({
      isDirectory: () => true,
      isSymbolicLink: () => false,
      uid: 0,
  ...
  it("falls back when /tmp/keanu is group/other writable", () => {
    const lstatSync = vi.fn(() => ({
      isDirectory: () => true,
      isSymbolicLink: () => false,
      uid: 501,
