...
function candidateBinDirs(opts: EnsureKeanuPathOpts): { prepend: string[]; append: string[] } {
  ...
  // Bundled macOS app: `keanu` lives next to the executable (process.execPath).
  try {
    const execDir = path.dirname(execPath);
    const siblingCli = path.join(execDir, "keanu");
  ...
  const allowProjectLocalBin =
    ...
    isTruthyEnvValue(process.env.KEANU_ALLOW_PROJECT_LOCAL_BIN);
  if (allowProjectLocalBin) {
    const localBinDir = path.join(cwd, "node_modules", ".bin");
    if (isExecutable(path.join(localBinDir, "keanu"))) {
...
/**
 * Best-effort PATH bootstrap so skills that require the `keanu` CLI can run
 * under launchd/minimal environments (and inside the macOS app bundle).
 */
export function ensureKeanuCliOnPath(opts: EnsureKeanuPathOpts = {}) {
  if (isTruthyEnvValue(process.env.KEANU_PATH_BOOTSTRAPPED)) {
    return;
  }
