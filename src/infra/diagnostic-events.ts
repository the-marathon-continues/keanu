...
function getDiagnosticEventsState(): DiagnosticEventsGlobalState {
  const globalStore = globalThis as typeof globalThis & {
    __keanuDiagnosticEventsState?: DiagnosticEventsGlobalState;
  };
  if (!globalStore.__keanuDiagnosticEventsState) {
    globalStore.__keanuDiagnosticEventsState = {
      seq: 0,
      listeners: new Set<(evt: DiagnosticEventPayload) => void>(),
      dispatchDepth: 0,
    };
  }
  return globalStore.__keanuDiagnosticEventsState;
}

export function isDiagnosticsEnabled(config?: KeanuConfig): boolean {
