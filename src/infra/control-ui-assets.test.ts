...
vi.mock("./keanu-root.js", () => ({
  resolveKeanuPackageRoot: vi.fn(async () => null),
  resolveKeanuPackageRootSync: vi.fn(() => null),
}));
...
let keanuRoot: typeof import("./keanu-root.js");
...
describe("control UI assets helpers (fs-mocked)", () => {
  beforeAll(async () => {
    ...
    keanuRoot = await import("./keanu-root.js");
  });
  ...
  it("uses resolveKeanuPackageRoot when available", async () => {
    const pkgRoot = abs("fixtures/keanu");
    (
      keanuRoot.resolveKeanuPackageRoot as unknown as ReturnType<typeof vi.fn>
    ).mockResolvedValueOnce(pkgRoot);

    await expect(resolveControlUiDistIndexPath(abs("fixtures/bin/keanu"))).resolves.toBe(
  ...
  it("falls back to package.json name matching when root resolution fails", async () => {
    ...
    setFile(path.join(root, "package.json"), JSON.stringify({ name: "keanu" }));
    setFile(path.join(root, "dist", "control-ui", "index.html"), "<html></html>\n");

    await expect(resolveControlUiDistIndexPath(path.join(root, "keanu.mjs"))).resolves.toBe(
      path.join(root, "dist", "control-ui", "index.html"),
    );
  });

  it("returns null when fallback package name does not match", async () => {
    const root = abs("fixtures/not-keanu");
  ...
  it("resolves control-ui root for dist bundle argv1 and moduleUrl candidates", async () => {
    const pkgRoot = abs("fixtures/keanu-bundle");
    (
      keanuRoot.resolveKeanuPackageRootSync as unknown as ReturnType<typeof vi.fn>
