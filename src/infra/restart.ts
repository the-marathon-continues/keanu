...
export function triggerKeanuRestart(): RestartAttempt {
  ...
  if (process.platform !== "darwin") {
    if (process.platform === "linux") {
      const unit = normalizeSystemdUnit(process.env.KEANU_SYSTEMD_UNIT, process.env.KEANU_PROFILE);
  ...
  const label =
    process.env.KEANU_LAUNCHD_LABEL || resolveGatewayLaunchAgentLabel(process.env.KEANU_PROFILE);
