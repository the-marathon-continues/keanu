...
export const POSIX_KEANU_TMP_DIR = "/tmp/keanu";

type ResolvePreferredKeanuTmpDirOptions = {
...
export function resolvePreferredKeanuTmpDir(
  options: ResolvePreferredKeanuTmpDirOptions = {},
): string {
  ...
  const fallback = (): string => {
    const base = tmpdir();
    const suffix = uid === undefined ? "keanu" : `keanu-${uid}`;
  ...
  const resolvePreferredState = (
    ...
    try {
      const preferred = lstatSync(POSIX_KEANU_TMP_DIR);
      if (!isTrustedPreferredDir(preferred)) {
        return "invalid";
      }
      if (requireWritableAccess) {
        accessSync(POSIX_KEANU_TMP_DIR, fs.constants.W_OK | fs.constants.X_OK);
      }
      return "available";
    } catch (err) {
  ...
  try {
    ...
    mkdirSync(POSIX_KEANU_TMP_DIR, { recursive: true, mode: 0o700 });
