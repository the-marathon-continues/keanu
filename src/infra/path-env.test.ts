...
let ensureKeanuCliOnPath: typeof import("./path-env.js").ensureKeanuCliOnPath;

describe("ensureKeanuCliOnPath", () => {
  const envKeys = [
    "PATH",
    "KEANU_PATH_BOOTSTRAPPED",
    "KEANU_ALLOW_PROJECT_LOCAL_BIN",
    "MISE_DATA_DIR",
    "HOMEBREW_PREFIX",
    "HOMEBREW_BREW_FILE",
    "XDG_BIN_HOME",
  ...
  beforeAll(async () => {
    ({ ensureKeanuCliOnPath } = await import("./path-env.js"));
  });
  ...
  it("prepends the bundled app bin dir when a sibling keanu exists", () => {
    const tmp = abs("/tmp/keanu-path/case-bundled");
    const appBinDir = path.join(tmp, "AppBin");
    const cliPath = path.join(appBinDir, "keanu");
    setDir(tmp);
    setDir(appBinDir);
    setExe(cliPath);

    process.env.PATH = "/usr/bin";
    delete process.env.KEANU_PATH_BOOTSTRAPPED;
  ...
  it("is idempotent", () => {
    ...
    process.env.KEANU_PATH_BOOTSTRAPPED = "1";
    ensureKeanuCliOnPath({
  ...
  it("prepends mise shims when available", () => {
    const tmp = abs("/tmp/keanu-path/case-mise");
    const appBinDir = path.join(tmp, "AppBin");
    const appCli = path.join(appBinDir, "keanu");
    setDir(tmp);
    setDir(appBinDir);
    setExe(appCli);
  ...
  it("only appends project-local node_modules/.bin when explicitly enabled", () => {
    const tmp = abs("/tmp/keanu-path/case-project-local");
    const appBinDir = path.join(tmp, "AppBin");
    const appCli = path.join(appBinDir, "keanu");
    setDir(tmp);
    setDir(appBinDir);
    ...
    const localCli = path.join(localBinDir, "keanu");
  ...
  it("prepends Linuxbrew dirs when present", () => {
    const tmp = abs("/tmp/keanu-path/case-linuxbrew");
