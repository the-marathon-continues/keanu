...
const DEFAULT_PACKAGE_NAME = "keanu";
...
export async function runGatewayUpdate(opts: UpdateRunnerOptions = {}): Promise<UpdateRunResult> {
  ...
  if (gitRoot && pkgRoot && path.resolve(gitRoot) === path.resolve(pkgRoot)) {
    ...
    if (channel === "dev") {
      ...
      const preflightRoot = await fs.mkdtemp(path.join(os.tmpdir(), "keanu-update-preflight-"));
    ...
    const doctorEntry = path.join(gitRoot, "keanu.mjs");
    ...
    const doctorStep = await runStep(
      step("keanu doctor", doctorArgv, gitRoot, { KEANU_UPDATE_IN_PROGRESS: "1" }),
    );
