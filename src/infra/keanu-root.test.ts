...
const VITEST_FS_BASE = path.join(path.parse(process.cwd()).root, "__keanu_vitest__");
const FIXTURE_BASE = path.join(VITEST_FS_BASE, "keanu-root");
...
describe("resolveKeanuPackageRoot", () => {
  let resolveKeanuPackageRoot: typeof import("./keanu-root.js").resolveKeanuPackageRoot;
  let resolveKeanuPackageRootSync: typeof import("./keanu-root.js").resolveKeanuPackageRootSync;

  beforeAll(async () => {
    ({ resolveKeanuPackageRoot, resolveKeanuPackageRootSync } = await import("./keanu-root.js"));
  });
  ...
  it("resolves package root from .bin argv1", async () => {
    ...
    const argv1 = path.join(project, "node_modules", ".bin", "keanu");
    const pkgRoot = path.join(project, "node_modules", "keanu");
    setFile(path.join(pkgRoot, "package.json"), JSON.stringify({ name: "keanu" }));

    expect(resolveKeanuPackageRootSync({ argv1 })).toBe(pkgRoot);
  });

  it("resolves package root via symlinked argv1", async () => {
    const project = fx("symlink-scenario");
    const bin = path.join(project, "bin", "keanu");
    const realPkg = path.join(project, "real-pkg");
    state.realpaths.set(abs(bin), abs(path.join(realPkg, "keanu.mjs")));
    setFile(path.join(realPkg, "package.json"), JSON.stringify({ name: "keanu" }));

    expect(resolveKeanuPackageRootSync({ argv1: bin })).toBe(realPkg);
  });
  ...
  it("prefers moduleUrl candidates", async () => {
    ...
    expect(resolveKeanuPackageRootSync({ moduleUrl })).toBe(pkgRoot);
  });

  it("returns null for non-keanu package roots", async () => {
    const pkgRoot = fx("not-keanu");
    setFile(path.join(pkgRoot, "package.json"), JSON.stringify({ name: "not-keanu" }));

    expect(resolveKeanuPackageRootSync({ cwd: pkgRoot })).toBeNull();
  });
  ...
  it("async resolver matches sync behavior", async () => {
    ...
    await expect(resolveKeanuPackageRoot({ cwd: pkgRoot })).resolves.toBe(pkgRoot);
  });

  it("async resolver returns null when no package roots exist", async () => {
    await expect(resolveKeanuPackageRoot({ cwd: fx("missing") })).resolves.toBeNull();
  });
});
