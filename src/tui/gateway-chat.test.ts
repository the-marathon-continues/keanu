...
describe("resolveGatewayConnection", () => {
  ...
  it("uses KEANU_GATEWAY_TOKEN for local mode", () => {
    loadConfig.mockReturnValue({ gateway: { mode: "local" } });

    withEnv({ KEANU_GATEWAY_TOKEN: "env-token" }, () => {
      const result = resolveGatewayConnection({});
  ...
  it("prefers KEANU_GATEWAY_PASSWORD over remote password fallback", () => {
    ...
    withEnv({ KEANU_GATEWAY_PASSWORD: "env-pass" }, () => {
      const result = resolveGatewayConnection({});
