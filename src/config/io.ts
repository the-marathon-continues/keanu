...
import type { KeanuConfig, ConfigFileSnapshot, LegacyConfigIssue } from "./types.js";
import {
  validateConfigObjectRawWithPlugins,
  validateConfigObjectWithPlugins,
} from "./validation.js";
import { compareKeanuVersions } from "./version.js";
...
function formatConfigValidationFailure(pathLabel: string, issueMessage: string): string {
  ...
  return [
    ...
    "",
    "Fix with:",
    `  keanu config set ${allowPath} '["*"]'`,
    "",
    "Or switch policy:",
    `  keanu config set ${policyPath} "pairing"`,
  ].join("\n");
}
...
function unsetPathForWrite(
  root: KeanuConfig,
  pathSegments: string[],
): { changed: boolean; next: KeanuConfig } {
...
function coerceConfig(value: unknown): KeanuConfig {
  if (!value || typeof value !== "object" || Array.isArray(value)) {
    return {};
  }
  return value as KeanuConfig;
}
...
function stampConfigVersion(cfg: KeanuConfig): KeanuConfig {
...
function warnIfConfigFromFuture(cfg: KeanuConfig, logger: Pick<typeof console, "warn">): void {
  const touched = cfg.meta?.lastTouchedVersion;
  if (!touched) {
    return;
  }
  const cmp = compareKeanuVersions(VERSION, touched);
  if (cmp === null) {
    return;
  }
  if (cmp < 0) {
    logger.warn(
      `Config was last written by a newer Keanu (${touched}); current version is ${VERSION}.`,
    );
  }
}
...
function resolveConfigForRead(
  ...
  if (resolvedIncludes && typeof resolvedIncludes === "object" && "env" in resolvedIncludes) {
    applyConfigEnvVars(resolvedIncludes as KeanuConfig, env);
  }
...
export function createConfigIO(overrides: ConfigIoDeps = {}) {
  ...
  function loadConfig(): KeanuConfig {
    try {
      ...
      const preValidationDuplicates = findDuplicateAgentDirs(resolvedConfig as KeanuConfig, {
        env: deps.env,
  ...
  async function writeConfigFile(cfg: KeanuConfig, options: ConfigWriteOptions = {}) {
    clearConfigCache();
    ...
    const outputConfigBase =
      envRefMap && changedPaths
        ? (restoreEnvRefsFromMap(cfgToWrite, "", envRefMap, changedPaths) as KeanuConfig)
        : cfgToWrite;
    ...
    const logConfigOverwrite = () => {
      ...
      const shouldLogInVitest = deps.env.KEANU_TEST_CONFIG_OVERWRITE_LOG === "1";
    ...
    const logConfigWriteAnomalies = () => {
      ...
      const isVitest = deps.env.VITEST === "true";
      const shouldLogInVitest = deps.env.KEANU_TEST_CONFIG_WRITE_ANOMALY_LOG === "1";
      if (isVitest && !shouldLogInVitest) {
        return;
      }
    ...
    const auditRecordBase = {
      ...
      watchMode: deps.env.KEANU_WATCH_MODE === "1",
      watchSession:
        typeof deps.env.KEANU_WATCH_SESSION === "string" &&
        deps.env.KEANU_WATCH_SESSION.trim().length > 0
          ? deps.env.KEANU_WATCH_SESSION.trim()
          : null,
      watchCommand:
        typeof deps.env.KEANU_WATCH_COMMAND === "string" &&
        deps.env.KEANU_WATCH_COMMAND.trim().length > 0
          ? deps.env.KEANU_WATCH_COMMAND.trim()
          : null,
      existsBefore: snapshot.exists,
...
// module scope. `KEANU_CONFIG_PATH` (and friends) are expected to work even
...
function resolveConfigCacheMs(env: NodeJS.ProcessEnv): number {
  const raw = env.KEANU_CONFIG_CACHE_MS?.trim();
  if (raw === "" || raw === "0") {
    return 0;
  }
  if (!raw) {
...
function shouldUseConfigCache(env: NodeJS.ProcessEnv): boolean {
  if (env.KEANU_DISABLE_CONFIG_CACHE?.trim()) {
    return false;
  }
...
export function loadConfig(): KeanuConfig {
