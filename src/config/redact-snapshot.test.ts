...
import type { ConfigFileSnapshot } from "./types.keanu.js";
...
function makeSnapshot<TConfig extends Record<string, unknown>>(
  ...
  return {
    path: "/home/user/.keanu/config.json5",
    exists: true,
...
describe("redactConfigSnapshot", () => {
  ...
  it("does not redact passwordFile path fields", () => {
    const snapshot = makeSnapshot({
      channels: {
        irc: {
          passwordFile: "/etc/keanu/irc-password.txt",
          nickserv: {
            passwordFile: "/etc/keanu/nickserv-password.txt",
    ...
    expect(irc.passwordFile).toBe("/etc/keanu/irc-password.txt");
    expect(nickserv.passwordFile).toBe("/etc/keanu/nickserv-password.txt");
