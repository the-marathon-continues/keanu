...
export const resolveKeanuPackageRoot = vi.fn().mockResolvedValue(null) as unknown as MockFn;
...
vi.mock("../plugins/loader.js", () => ({
  loadKeanuPlugins: () => ({ plugins: [], diagnostics: [] }),
}));
...
beforeEach(() => {
  ...
  resolveKeanuPackageRoot.mockReset().mockResolvedValue(null);
  ...
  setStdinTty(true);
  originalStateDir = process.env.KEANU_STATE_DIR;
  originalUpdateInProgress = process.env.KEANU_UPDATE_IN_PROGRESS;
  process.env.KEANU_UPDATE_IN_PROGRESS = "1";
  tempStateDir = fs.mkdtempSync(path.join(os.tmpdir(), "keanu-doctor-state-"));
...
afterEach(() => {
  ...
  if (originalStateDir === undefined) {
    ...
    process.env.KEANU_STATE_DIR = originalStateDir;
  }
  if (originalUpdateInProgress === undefined) {
    delete process.env.KEANU_UPDATE_IN_PROGRESS;
  } else {
    process.env.KEANU_UPDATE_IN_PROGRESS = originalUpdateInProgress;
  }
  if (tempStateDir) {
