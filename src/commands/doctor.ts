...
function resolveMode(cfg: KeanuConfig): "local" | "remote" {
...
export async function doctorCommand(
  ...
  const root = await resolveKeanuPackageRoot({
  ...
  let cfg: KeanuConfig = configResult.cfg;
  ...
  if (!cfg.gateway?.mode) {
    const lines = [
      ...
      `Fix: run ${formatCliCommand("keanu configure")} and set Gateway mode (local/remote).`,
      `Or set directly: ${formatCliCommand("keanu config set gateway.mode local")}`,
    ];
    if (!fs.existsSync(configPath)) {
      lines.push(`Missing config: run ${formatCliCommand("keanu setup")} first.`);
    }
    note(lines.join("\n"), "Gateway");
  }
  ...
  if (shouldWriteConfig) {
    ...
    runtime.log(`Run "${formatCliCommand("keanu doctor --fix")}" to apply changes.`);
  }
