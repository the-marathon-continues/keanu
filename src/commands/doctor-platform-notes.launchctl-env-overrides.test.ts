...
describe("noteMacLaunchctlGatewayEnvOverrides", () => {
  it("prints clear unsetenv instructions for token override", async () => {
    ...
    const getenv = vi.fn(async (name: string) =>
      name === "KEANU_GATEWAY_TOKEN" ? "launchctl-token" : undefined,
    );
    const cfg = {
      gateway: {
        auth: {
    ...
    expect(message).toContain("KEANU_GATEWAY_TOKEN");
    expect(message).toContain("launchctl unsetenv KEANU_GATEWAY_TOKEN");
    expect(message).not.toContain("KEANU_GATEWAY_PASSWORD");
  });
