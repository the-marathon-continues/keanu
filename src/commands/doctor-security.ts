...
import type { KeanuConfig, GatewayBindMode } from "../config/config.js";
...
export async function noteSecurityWarnings(cfg: KeanuConfig) {
  const warnings: string[] = [];
  const auditHint = `- Run: ${formatCliCommand("keanu security audit --deep")}`;

  if (cfg.approvals?.exec?.enabled === false) {
    warnings.push(
      "- Note: approvals.exec.enabled=false disables approval forwarding only.",
      "  Host exec gating still comes from ~/.keanu/exec-approvals.json.",
      `  Check local policy with: ${formatCliCommand("keanu approvals get --gateway")}`,
    );
  }
    ...
    if (!hasSharedSecret) {
      const authFixLines =
        resolvedAuth.mode === "password"
          ? [
              `  Fix: ${formatCliCommand("keanu configure")} to set a password`,
              `  Or switch to token: ${formatCliCommand("keanu config set gateway.auth.mode token")}`,
            ]
          : [
              `  Fix: ${formatCliCommand("keanu doctor --fix")} to generate a token`,
      ...
      warnings.push(
        ...
        `  Fix: ${formatCliCommand("keanu config set gateway.bind loopback")}`,
    ...
    if (dmScope === "main" && isMultiUserDm) {
      warnings.push(
        `- ${params.label} DMs: multiple senders share the main session; run: ` +
          formatCliCommand('keanu config set session.dmScope "per-channel-peer"') +
