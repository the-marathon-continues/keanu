...
export async function modelsStatusCommand(
  ...
  const agentDir = agentId ? resolveAgentDir(cfg, agentId) : resolveKeanuAgentDir();
  ...
  if (missingProvidersInUse.length > 0) {
    ...
    for (const provider of missingProvidersInUse) {
      const hint =
        provider === "anthropic"
          ? `Run \`claude setup-token\`, then \`${formatCliCommand("keanu models auth setup-token")}\` or \`${formatCliCommand("keanu configure")}\`.`
          : `Run \`${formatCliCommand("keanu configure")}\` or set an API key env var.`;
