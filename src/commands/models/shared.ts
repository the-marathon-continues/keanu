...
import { type KeanuConfig, readConfigFileSnapshot, writeConfigFile } from "../../config/config.js";
...
export async function loadValidConfigOrThrow(): Promise<KeanuConfig> {
...
export async function updateConfig(
  mutator: (cfg: KeanuConfig) => KeanuConfig,
): Promise<KeanuConfig> {
...
export function resolveModelTarget(params: { raw: string; cfg: KeanuConfig }): {
  provider: string;
  model: string;
} {
...
export function buildAllowlistSet(cfg: KeanuConfig): Set<string> {
...
export function resolveKnownAgentId(params: {
  ...
  if (!knownAgents.includes(agentId)) {
    throw new Error(
      `Unknown agent id "${raw}". Use "${formatCliCommand("keanu agents list")}" to see configured agents.`,
    );
  }
  return agentId;
}
