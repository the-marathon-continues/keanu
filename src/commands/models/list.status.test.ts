...
const mocks = vi.hoisted(() => {
  ...
  return {
    store,
    resolveKeanuAgentDir: vi.fn().mockReturnValue("/tmp/keanu-agent"),
    resolveAgentDir: vi.fn().mockReturnValue("/tmp/keanu-agent"),
    ...
    resolveAuthStorePathForDisplay: vi.fn().mockReturnValue("/tmp/keanu-agent/auth-profiles.json"),
...
vi.mock("../../agents/agent-paths.js", () => ({
  resolveKeanuAgentDir: mocks.resolveKeanuAgentDir,
}));
...
async function withAgentScopeOverrides<T>(
  ...
  try {
    ...
    if (originalAgentDir) {
      ...
      mocks.resolveAgentDir.mockReturnValue("/tmp/keanu-agent");
    }
  }
}
...
describe("modelsStatusCommand auth overview", () => {
  it("includes masked auth sources in JSON output", async () => {
    ...
    expect(mocks.resolveKeanuAgentDir).toHaveBeenCalled();
    expect(payload.defaultModel).toBe("anthropic/claude-opus-4-5");
    expect(payload.auth.storePath).toBe("/tmp/keanu-agent/auth-profiles.json");
    ...
    await withAgentScopeOverrides(
      {
        ...
        agentDir: "/tmp/keanu-agent-custom",
      },
      async () => {
        ...
        expect(payload.agentDir).toBe("/tmp/keanu-agent-custom");
