...
const SEARCH_TOOL = "https://docs.keanu.ai/mcp.SearchKeanu";
...
export async function docsSearchCommand(queryParts: string[], runtime: RuntimeEnv) {
  ...
  if (!query) {
    const docs = formatDocsLink("/", "docs.keanu.ai");
    if (isRich()) {
      runtime.log(`${theme.muted("Docs:")} ${docs}`);
      runtime.log(`${theme.muted("Search:")} ${formatCliCommand('keanu docs "your query"')}`);
    } else {
      runtime.log("Docs: https://docs.keanu.ai/");
      runtime.log(`Search: ${formatCliCommand('keanu docs "your query"')}`);
    }
    return;
  }
