...
beforeAll(() => {
  envSnapshot = captureEnv(["KEANU_PROFILE"]);
  process.env.KEANU_PROFILE = "isolated";
});
...
vi.mock("../infra/keanu-root.js", () => ({
  resolveKeanuPackageRoot: vi.fn().mockResolvedValue("/tmp/keanu"),
}));
...
vi.mock("../infra/update-check.js", () => ({
  checkUpdateStatus: vi.fn().mockResolvedValue({
    ...
    deps: {
      manager: "pnpm",
      status: "ok",
      lockfilePath: "/tmp/keanu/pnpm-lock.yaml",
      markerPath: "/tmp/keanu/node_modules/.modules.yaml",
    },
...
describe("statusCommand", () => {
  ...
  it("prints formatted lines otherwise", async () => {
    ...
    await statusCommand({}, runtime as never);
    const logs = runtimeLogMock.mock.calls.map((c: unknown[]) => String(c[0]));
    expect(logs.some((l: string) => l.includes("Keanu status"))).toBe(true);
    ...
    expect(
      logs.some(
        (l: string) =>
          l.includes("keanu status --all") ||
          l.includes("keanu --profile isolated status --all") ||
          l.includes("keanu status --all") ||
          l.includes("keanu --profile isolated status --all"),
      ),
    ).toBe(true);
  ...
  it("shows gateway auth when reachable", async () => {
    ...
    process.env.KEANU_GATEWAY_TOKEN = "abcd1234";
    try {
