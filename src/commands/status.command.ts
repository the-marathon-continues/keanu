...
export async function statusCommand(
  ...
  runtime.log(theme.heading("Keanu status"));
  runtime.log("");
  ...
  if (pairingRecovery) {
    ...
    if (pairingRecovery.requestId) {
      runtime.log(
        theme.muted(
          `Recovery: ${formatCliCommand(`keanu devices approve ${pairingRecovery.requestId}`)}`,
        ),
      );
    }
    runtime.log(theme.muted(`Fallback: ${formatCliCommand("keanu devices approve --latest")}`));
    runtime.log(theme.muted(`Inspect: ${formatCliCommand("keanu devices list")}`));
  }
  ...
  runtime.log(theme.muted(`Full report: ${formatCliCommand("keanu security audit")}`));
  runtime.log(theme.muted(`Deep probe: ${formatCliCommand("keanu security audit --deep")}`));
  ...
  runtime.log("");
  runtime.log("FAQ: https://docs.keanu.ai/faq");
  runtime.log("Troubleshooting: https://docs.keanu.ai/troubleshooting");
  runtime.log("");
  ...
  runtime.log(`  Need to share?      ${formatCliCommand("keanu status --all")}`);
  runtime.log(`  Need to debug live? ${formatCliCommand("keanu logs --follow")}`);
  if (gatewayReachable) {
    runtime.log(`  Need to test channels? ${formatCliCommand("keanu status --deep")}`);
  } else {
    runtime.log(`  Fix reachability first: ${formatCliCommand("keanu gateway probe")}`);
  }
}
