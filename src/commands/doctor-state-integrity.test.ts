...
type EnvSnapshot = {
  HOME?: string;
  KEANU_HOME?: string;
  KEANU_STATE_DIR?: string;
  KEANU_OAUTH_DIR?: string;
};

function captureEnv(): EnvSnapshot {
  return {
    HOME: process.env.HOME,
    KEANU_HOME: process.env.KEANU_HOME,
    KEANU_STATE_DIR: process.env.KEANU_STATE_DIR,
    KEANU_OAUTH_DIR: process.env.KEANU_OAUTH_DIR,
  };
}
...
function setupSessionState(cfg: KeanuConfig, env: NodeJS.ProcessEnv, homeDir: string) {
  const agentId = "main";
...
async function runStateIntegrity(cfg: KeanuConfig) {
...
describe("doctor state integrity oauth dir checks", () => {
  ...
  beforeEach(() => {
    ...
    tempHome = fs.mkdtempSync(path.join(os.tmpdir(), "keanu-doctor-state-integrity-"));
    process.env.HOME = tempHome;
    process.env.KEANU_HOME = tempHome;
    process.env.KEANU_STATE_DIR = path.join(tempHome, ".keanu");
    delete process.env.KEANU_OAUTH_DIR;
    fs.mkdirSync(process.env.KEANU_STATE_DIR, { recursive: true, mode: 0o700 });
  ...
  it("prompts for oauth dir when KEANU_OAUTH_DIR is explicitly configured", async () => {
    process.env.KEANU_OAUTH_DIR = path.join(tempHome, ".oauth");
    const cfg: KeanuConfig = {};
    const confirmSkipInNonInteractive = await runStateIntegrity(cfg);
    expect(confirmSkipInNonInteractive).toHaveBeenCalledWith(OAUTH_PROMPT_MATCHER);
    expect(stateIntegrityText()).toContain("CRITICAL: OAuth dir missing");
  ...
  it("prints keanu-only verification hints when recent sessions are missing transcripts", async () => {
    const cfg: KeanuConfig = {};
    setupSessionState(cfg, process.env, process.env.HOME ?? "");
    ...
    expect(text).toMatch(/keanu sessions --store ".*sessions\.json"/);
    expect(text).toMatch(/keanu sessions cleanup --store ".*sessions\.json" --dry-run/);
