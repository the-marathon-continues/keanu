...
describe("writeOAuthCredentials", () => {
  ...
  it("writes auth-profiles.json under KEANU_AGENT_DIR when set", async () => {
    const env = await setupAuthTestEnv("keanu-oauth-");
  ...
  it("writes OAuth credentials to all sibling agent dirs when syncSiblingAgents=true", async () => {
    tempStateDir = await fs.mkdtemp(path.join(os.tmpdir(), "keanu-oauth-sync-"));
    ...
    process.env.KEANU_AGENT_DIR = kidAgentDir;
  ...
  it("writes OAuth credentials only to target dir by default", async () => {
    tempStateDir = await fs.mkdtemp(path.join(os.tmpdir(), "keanu-oauth-nosync-"));
    process.env.KEANU_STATE_DIR = tempStateDir;
  ...
  it("syncs siblings from explicit agentDir outside KEANU_STATE_DIR", async () => {
    tempStateDir = await fs.mkdtemp(path.join(os.tmpdir(), "keanu-oauth-external-"));
    process.env.KEANU_STATE_DIR = tempStateDir;

    // Create standard-layout agents tree *outside* KEANU_STATE_DIR
...
describe("setMinimaxApiKey", () => {
  ...
  it("writes to KEANU_AGENT_DIR when set", async () => {
    const env = await setupAuthTestEnv("keanu-minimax-", { agentSubdir: "custom-agent" });
    lifecycle.setStateDir(env.stateDir);
