...
function expectPrimaryModelChanged(
  applied: { changed: boolean; next: KeanuConfig },
  primary: string,
) {
...
function expectConfigUnchanged(applied: { changed: boolean; next: KeanuConfig }, cfg: KeanuConfig) {
  expect(applied.changed).toBe(false);
  expect(applied.next).toEqual(cfg);
}

type SharedDefaultModelCase = {
  apply: (cfg: KeanuConfig) => { changed: boolean; next: KeanuConfig };
  defaultModel: string;
  overrideConfig: KeanuConfig;
  alreadyDefaultConfig: KeanuConfig;
};
...
describe("applyDefaultModelChoice", () => {
  it("ensures allowlist entry exists when returning an agent override", async () => {
    ...
    const applied = await applyDefaultModelChoice({
      ...
      applyProviderConfig: (config: KeanuConfig) => config,
      applyDefaultConfig: (config: KeanuConfig) => config,
      noteAgentModel,
...
describe("shared default model behavior", () => {
  it("sets defaults when model is unset", () => {
    for (const testCase of SHARED_DEFAULT_MODEL_CASES) {
      const cfg: KeanuConfig = { agents: { defaults: {} } };
