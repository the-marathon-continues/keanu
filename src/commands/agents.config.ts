...
function resolveAgentName(cfg: KeanuConfig, agentId: string) {
...
function resolveAgentModel(cfg: KeanuConfig, agentId: string) {
  const entry = listAgentEntries(cfg).find(
    (agent) => normalizeAgentId(agent.id) === normalizeAgentId(agentId),
  );
  if (entry?.model) {
...
export function buildAgentSummaries(cfg: KeanuConfig): AgentSummary[] {
