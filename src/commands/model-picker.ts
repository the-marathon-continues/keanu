...
type PromptDefaultModelResult = { model?: string; config?: KeanuConfig };
...
function resolveConfiguredModelRaw(cfg: KeanuConfig): string {
  return resolveAgentModelPrimaryValue(cfg.agents?.defaults?.model) ?? "";
}

function resolveConfiguredModelKeys(cfg: KeanuConfig): string[] {
...
export function applyPrimaryModel(cfg: KeanuConfig, model: string): KeanuConfig {
...
export function applyModelAllowlist(cfg: KeanuConfig, models: string[]): KeanuConfig {
  const defaults = cfg.agents?.defaults;
