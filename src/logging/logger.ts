...
import type { KeanuConfig } from "../config/types.js";
...
export const DEFAULT_LOG_DIR = resolvePreferredKeanuTmpDir();
export const DEFAULT_LOG_FILE = path.join(DEFAULT_LOG_DIR, "keanu.log"); // legacy single-file path

const LOG_PREFIX = "keanu";
...
function resolveSettings(): ResolvedSettings {
  let cfg: KeanuConfig["logging"] | undefined =
    (loggingState.overrideSettings as LoggerSettings | null) ?? readLoggingConfig();
  if (!cfg) {
    try {
      const loaded = requireConfig?.("../config/config.js") as
        | {
            loadConfig?: () => KeanuConfig;
          }
        | undefined;
  ...
  const defaultLevel =
    process.env.VITEST === "true" && process.env.KEANU_TEST_FILE_LOG !== "1" ? "silent" : "info";
      ...
      if (nextBytes > settings.maxFileBytes) {
        if (!warnedAboutSizeCap) {
          ...
          process.stderr.write(
            `[keanu] log file size cap reached; suppressing writes file=${settings.file} maxFileBytes=${settings.maxFileBytes}\n`,
          );
        }
        return;
      }
