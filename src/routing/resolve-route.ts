...
function listAgents(cfg: KeanuConfig) {
  const agents = cfg.agents?.list;
  return Array.isArray(agents) ? agents : [];
}

function pickFirstExistingAgentId(cfg: KeanuConfig, agentId: string): string {
...
type EvaluatedBindingsCache = {
  bindingsRef: KeanuConfig["bindings"];
  byChannelAccount: Map<string, EvaluatedBinding[]>;
};

const evaluatedBindingsCacheByCfg = new WeakMap<KeanuConfig, EvaluatedBindingsCache>();
