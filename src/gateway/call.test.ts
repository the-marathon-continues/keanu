...
describe("buildGatewayConnectionDetails", () => {
  ...
  it("throws for insecure ws:// remote URLs (CWE-319)", () => {
    ...
    expect((thrown as Error).message).toContain("keanu doctor --fix");
  });
...
describe("callGateway url override auth requirements", () => {
  ...
  beforeEach(() => {
    envSnapshot = captureEnv(["KEANU_GATEWAY_TOKEN", "KEANU_GATEWAY_PASSWORD"]);
    resetGatewayCallMocks();
  ...
  it("throws when url override is set without explicit credentials", async () => {
    ...
    process.env.KEANU_GATEWAY_PASSWORD = "env-password";
    loadConfig.mockReturnValue({
      gateway: {
        mode: "local",
...
describe("callGateway password resolution", () => {
  ...
  beforeEach(() => {
    envSnapshot = captureEnv(["KEANU_GATEWAY_PASSWORD", "KEANU_GATEWAY_TOKEN"]);
    resetGatewayCallMocks();
  ...
  it.each([
    ...
    if (envPassword !== undefined) {
      process.env.KEANU_GATEWAY_PASSWORD = envPassword;
    }
