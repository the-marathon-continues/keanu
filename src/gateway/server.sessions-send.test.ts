...
const { createKeanuTools } = await import("../agents/keanu-tools.js");
...
type SessionSendTool = ReturnType<typeof createKeanuTools>[number];
...
function getSessionsSendTool(): SessionSendTool {
  ...
  const tool = createKeanuTools().find((candidate) => candidate.name === "sessions_send");
  if (!tool) {
...
beforeAll(async () => {
  envSnapshot = captureEnv(["KEANU_GATEWAY_PORT", "KEANU_GATEWAY_TOKEN"]);
  gatewayPort = await getFreePort();
  testState.gatewayAuth = { mode: "token", token: gatewayToken };
  process.env.KEANU_GATEWAY_PORT = String(gatewayPort);
  process.env.KEANU_GATEWAY_TOKEN = gatewayToken;
...
describe("sessions_send label lookup", () => {
  it(
    ...
    async () => {
      ...
      const configPath = process.env.KEANU_CONFIG_PATH;
      if (!configPath) {
        throw new Error("KEANU_CONFIG_PATH missing in gateway test environment");
      }
