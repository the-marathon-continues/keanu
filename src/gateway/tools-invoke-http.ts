...
import { createKeanuTools } from "../agents/keanu-tools.js";
import {
...
export async function handleToolsInvokeHttpRequest(
  ...
  const messageChannel = normalizeMessageChannel(getHeader(req, "x-keanu-message-channel") ?? "");
  const accountId = getHeader(req, "x-keanu-account-id")?.trim() || undefined;
  const agentTo = getHeader(req, "x-keanu-message-to")?.trim() || undefined;
  const agentThreadId = getHeader(req, "x-keanu-thread-id")?.trim() || undefined;
  ...
  const allTools = createKeanuTools({
