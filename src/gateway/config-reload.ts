...
import type { KeanuConfig, ConfigFileSnapshot, GatewayReloadMode } from "../config/config.js";
...
export function resolveGatewayReloadSettings(cfg: KeanuConfig): GatewayReloadSettings {
...
export function startGatewayConfigReloader(opts: {
  initialConfig: KeanuConfig;
  readSnapshot: () => Promise<ConfigFileSnapshot>;
  onHotReload: (plan: GatewayReloadPlan, nextConfig: KeanuConfig) => Promise<void>;
  onRestart: (plan: GatewayReloadPlan, nextConfig: KeanuConfig) => void;
  log: {
  ...
  const queueRestart = (plan: GatewayReloadPlan, nextConfig: KeanuConfig) => {
    if (restartQueued) {
      return;
    }
    restartQueued = true;
  ...
  const applySnapshot = async (nextConfig: KeanuConfig) => {
