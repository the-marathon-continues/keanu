...
function readGatewayTokenEnv(
  ...
  const primary = trimToUndefined(env.KEANU_GATEWAY_TOKEN);
  if (primary) {
    return primary;
  }
  if (!includeLegacyEnv) {
  ...
  return trimToUndefined(env.KEANU_GATEWAY_TOKEN);
}
...
function readGatewayPasswordEnv(
  ...
  const primary = trimToUndefined(env.KEANU_GATEWAY_PASSWORD);
  if (primary) {
    return primary;
  }
  if (!includeLegacyEnv) {
  ...
  return trimToUndefined(env.KEANU_GATEWAY_PASSWORD);
}
