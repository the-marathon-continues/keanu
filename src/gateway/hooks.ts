...
export function resolveHooksConfig(cfg: KeanuConfig): HooksConfigResolved | null {
...
function resolveKnownAgentIds(cfg: KeanuConfig, defaultAgentId: string): Set<string> {
...
export function extractHookToken(req: IncomingMessage): string | undefined {
  ...
  const headerToken =
    typeof req.headers["x-keanu-token"] === "string" ? req.headers["x-keanu-token"].trim() : "";
  if (headerToken) {
    return headerToken;
  }
  return undefined;
