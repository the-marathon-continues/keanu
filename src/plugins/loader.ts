...
function resolvePluginModuleExport(moduleExport: unknown): {
  definition?: KeanuPluginDefinition;
  register?: KeanuPluginDefinition["register"];
} {
  const resolved =
    moduleExport &&
  ...
  if (typeof resolved === "function") {
    return {
      register: resolved as KeanuPluginDefinition["register"],
    };
  }
  if (resolved && typeof resolved === "object") {
    const def = resolved as KeanuPluginDefinition;
...
export function loadKeanuPlugins(options: PluginLoadOptions = {}): PluginRegistry {
  ...
  const discovery = discoverKeanuPlugins({
    workspaceDir: options.workspaceDir,
  ...
  const getJiti = () => {
    ...
    jitiLoader = createJiti(import.meta.url, {
      ...
      ...(pluginSdkAlias || pluginSdkAccountIdAlias
        ? {
            alias: {
              ...(pluginSdkAlias ? { "keanu/plugin-sdk": pluginSdkAlias } : {}),
              ...(pluginSdkAccountIdAlias
                ? { "keanu/plugin-sdk/account-id": pluginSdkAccountIdAlias }
                : {}),
            },
          }
        : {}),
  ...
  for (const candidate of discovery.candidates) {
    ...
    let mod: KeanuPluginModule | null = null;
    try {
      mod = getJiti()(candidate.source) as KeanuPluginModule;
    } catch (err) {
      recordPluginError({
        logger,
        registry,
