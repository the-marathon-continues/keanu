...
const fixtureRoot = path.join(os.tmpdir(), `keanu-plugin-${randomUUID()}`);
let tempDirIndex = 0;
const prevBundledDir = process.env.KEANU_BUNDLED_PLUGINS_DIR;
...
function writePlugin(params: {
  ...
  fs.writeFileSync(
    path.join(dir, "keanu.plugin.json"),
    JSON.stringify(
      {
        id: params.id,
...
function loadBundledMemoryPluginRegistry(options?: {
  ...
  process.env.KEANU_BUNDLED_PLUGINS_DIR = bundledDir;

  return loadKeanuPlugins({
    cache: false,
    config: {
      plugins: {
        slots: {
...
function expectTelegramLoaded(registry: ReturnType<typeof loadKeanuPlugins>) {
...
afterEach(() => {
  if (prevBundledDir === undefined) {
    delete process.env.KEANU_BUNDLED_PLUGINS_DIR;
  } else {
    process.env.KEANU_BUNDLED_PLUGINS_DIR = prevBundledDir;
  }
});
...
describe("loadKeanuPlugins", () => {
  it("disables bundled plugins by default", () => {
    ...
    const enabledRegistry = loadKeanuPlugins({
      cache: false,
      config: {
        plugins: {
          allow: ["bundled"],
  ...
  it("loads plugins from config paths", () => {
    process.env.KEANU_BUNDLED_PLUGINS_DIR = "/nonexistent/bundled/plugins";
  ...
  it("warns when loaded non-bundled plugin has no install/load-path provenance", () => {
    ...
    withEnv({ KEANU_STATE_DIR: stateDir }, () => {
