...
function createSinglePluginWithEmptySlotsConfig(): KeanuConfig {
  return {
    plugins: {
...
function createSingleNpmInstallConfig(installPath: string): KeanuConfig {
  return {
    plugins: {
      entries: createSinglePluginEntries(),
      installs: {
...
describe("resolveUninstallDirectoryTarget", () => {
  ...
  it("falls back to default path when configured installPath is untrusted", () => {
    const extensionsDir = path.join(os.tmpdir(), "keanu-uninstall-safe");
    const target = resolveUninstallDirectoryTarget({
      ...
      installRecord: {
        source: "npm",
        spec: "my-plugin@1.0.0",
        installPath: "/tmp/not-keanu-extensions/my-plugin",
      },
      extensionsDir,
    });
