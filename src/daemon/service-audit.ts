...
function auditGatewayToken(
  ...
  const serviceToken = command?.environment?.KEANU_GATEWAY_TOKEN?.trim();
  if (serviceToken === expectedToken) {
    return;
  }
  issues.push({
    code: SERVICE_AUDIT_CODES.gatewayTokenMismatch,
    message: "Gateway service KEANU_GATEWAY_TOKEN does not match gateway.auth.token in keanu.json",
...
export function checkTokenDrift(params: {
  ...
  if (configToken && serviceToken !== configToken) {
    return {
      ...
      detail: "Run `keanu gateway install --force` to sync the token.",
      level: "recommended",
    };
  }
