    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          ["keanu node run --host 127.0.0.1 --port 18789", "Run the node host in the foreground."],
          ["keanu node status", "Check node host service status."],
          ["keanu node install", "Install the node host service."],
          ["keanu node restart", "Restart the installed node host service."],
        ])}\n\n${theme.muted("Docs:")} ${formatDocsLink("/cli/node", "docs.keanu.ai/cli/node")}\n`,
    );
