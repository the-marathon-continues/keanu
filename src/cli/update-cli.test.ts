...
import type { KeanuConfig, ConfigFileSnapshot } from "../config/types.keanu.js";
...
vi.mock("../infra/keanu-root.js", () => ({
  resolveKeanuPackageRoot: vi.fn(),
}));
...
const { resolveKeanuPackageRoot } = await import("../infra/keanu-root.js");
...
describe("update-cli", () => {
  const fixtureRoot = "/tmp/keanu-update-tests";
  let fixtureCount = 0;
  ...
  const baseConfig = {} as KeanuConfig;
  const baseSnapshot: ConfigFileSnapshot = {
    path: "/tmp/keanu-config.json",
    exists: true,
    raw: "{}",
    parsed: {},
    resolved: baseConfig,
  ...
  const mockPackageInstallStatus = (root: string) => {
    vi.mocked(resolveKeanuPackageRoot).mockResolvedValue(root);
  ...
  const setupNonInteractiveDowngrade = async () => {
    const tempDir = createCaseDir("keanu-update");
    setTty(false);
  ...
  beforeEach(() => {
    ...
    vi.mocked(resolveKeanuPackageRoot).mockClear();
    ...
    vi.mocked(resolveKeanuPackageRoot).mockResolvedValue(process.cwd());
    ...
    readPackageName.mockResolvedValue("keanu");
    ...
    prepareRestartScript.mockResolvedValue("/tmp/keanu-restart-test.sh");
    runRestartScript.mockResolvedValue(undefined);
    inspectPortUsage.mockResolvedValue({
      port: 18789,
      status: "busy",
      listeners: [{ pid: 4242, command: "keanu-gateway" }],
      hints: [],
    });
    ...
    },
    {
      ...
      prepare: async () => {
        vi.mocked(readConfigFileSnapshot).mockResolvedValue({
          ...baseSnapshot,
          config: { update: { channel: "beta" } } as KeanuConfig,
        });
      },
  ...
  it("honors --tag override", async () => {
    ...
    vi.mocked(resolveKeanuPackageRoot).mockResolvedValue(tempDir);
    vi.mocked(runGatewayUpdate).mockResolvedValue(
      makeOkUpdateResult({
        mode: "npm",
      }),
  ...
  it("updateCommand refreshes service env from updated install root when available", async () => {
    const root = createCaseDir("keanu-updated-root");
  ...
  it("updateCommand continues after doctor sub-step and clears update flag", async () => {
    ...
    try {
      await withEnvAsync({ KEANU_UPDATE_IN_PROGRESS: undefined }, async () => {
        vi.mocked(runGatewayUpdate).mockResolvedValue(makeOkUpdateResult());
        vi.mocked(runDaemonRestart).mockResolvedValue(true);
        vi.mocked(doctorCommand).mockResolvedValue(undefined);
        vi.mocked(defaultRuntime.log).mockClear();
        ...
        expect(process.env.KEANU_UPDATE_IN_PROGRESS).toBeUndefined();
  ...
  it("updateWizardCommand offers dev checkout and forwards selections", async () => {
    const tempDir = createCaseDir("keanu-update-wizard");
    await withEnvAsync({ KEANU_GIT_DIR: tempDir }, async () => {
      setTty(true);
