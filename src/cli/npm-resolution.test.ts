...
describe("npm-resolution helpers", () => {
  it("keeps original spec when pin is disabled", () => {
    const result = resolvePinnedNpmSpec({
      rawSpec: "@keanu/plugin-alpha@latest",
      pin: false,
      resolvedSpec: "@keanu/plugin-alpha@1.2.3",
    });
    expect(result).toEqual({
      recordSpec: "@keanu/plugin-alpha@latest",
    });
  });
  ...
  it("returns pinned spec notice when resolved spec is available", () => {
    ...
    expect(result).toEqual({
      recordSpec: "@keanu/plugin-alpha@1.2.3",
      pinNotice: "Pinned npm install record to @keanu/plugin-alpha@1.2.3.",
    });
  });
  ...
  it("maps npm resolution metadata to install fields", () => {
    expect(
      ...
      resolvedName: "@keanu/plugin-alpha",
  ...
  it("builds common npm install record fields", () => {
    expect(
      buildNpmInstallRecordFields({
        spec: "@keanu/plugin-alpha@1.2.3",
        installPath: "/tmp/keanu/extensions/alpha",
        version: "1.2.3",
        resolution: {
          name: "@keanu/plugin-alpha",
          version: "1.2.3",
  ...
  it("resolves pinned install record and emits pin notice", () => {
    ...
    expect(logs).toEqual(["Pinned npm install record to @keanu/plugin-alpha@1.2.3."]);
  ...
  it("resolves pinned install record for CLI and formats warning output", () => {
    ...
    const record = resolvePinnedNpmInstallRecordForCli(
      "@keanu/plugin-alpha@latest",
      true,
      "/tmp/keanu/extensions/alpha",
      "1.2.3",
      undefined,
      (message) => logs.push(message),
    ...
    expect(record).toEqual({
      source: "npm",
      spec: "@keanu/plugin-alpha@latest",
      installPath: "/tmp/keanu/extensions/alpha",
      version: "1.2.3",
