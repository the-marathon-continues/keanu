    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          ["keanu memory status", "Show index and provider status."],
          ["keanu memory index --force", "Force a full reindex."],
          ['keanu memory search --query "deployment notes"', "Search indexed memory entries."],
          ["keanu memory status --json", "Output machine-readable JSON."],
        ])}\n\n${theme.muted("Docs:")} ${formatDocsLink("/cli/memory", "docs.keanu.ai/cli/memory")}\n`,
    );
