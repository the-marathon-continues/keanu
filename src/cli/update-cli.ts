...
export function registerUpdateCli(program: Command) {
  const update = program
    .command("update")
    .description("Update Keanu and inspect update channel status")
    ...
    .addHelpText("after", () => {
      const examples = [
        ["keanu update", "Update a source checkout (git)"],
        ["keanu update --channel beta", "Switch to beta channel (git + npm)"],
        ["keanu update --channel dev", "Switch to dev channel (git + npm)"],
        ["keanu update --tag beta", "One-off update to a dist-tag or version"],
        ["keanu update --dry-run", "Preview actions without changing anything"],
        ["keanu update --no-restart", "Update without restarting the service"],
        ["keanu update --json", "Output result as JSON"],
        ["keanu update --yes", "Non-interactive (accept downgrade prompts)"],
        ["keanu update wizard", "Interactive update wizard"],
        ["keanu --update", "Shorthand for keanu update"],
      ] as const;
...
${theme.heading("Switch channels:")}
  ...
  - Run keanu update status to see the active channel and source
...
${theme.muted("Docs:")} ${formatDocsLink("/cli/update", "docs.keanu.ai/cli/update")}`;
    })
    .action(async (opts) => {
      try {
        await updateCommand({
  ...
  update
    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          ["keanu update status", "Show channel + version status."],
          ["keanu update status --json", "JSON output."],
          ["keanu update status --timeout 10", "Custom timeout."],
        ])}\n\n${theme.heading("Notes:")}\n${theme.muted(
          "- Shows current update channel (stable/beta/dev) and source",
        )}\n${theme.muted("- Includes git tag/branch/SHA for source checkouts")}\n\n${theme.muted(
          "Docs:",
        )} ${formatDocsLink("/cli/update", "docs.keanu.ai/cli/update")}`,
    )
    .action(async (opts, command) => {
      try {
