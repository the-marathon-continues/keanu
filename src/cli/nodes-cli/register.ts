    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          ["keanu nodes status", "List known nodes with live status."],
          ["keanu nodes pairing pending", "Show pending node pairing requests."],
          ['keanu nodes run --node <id> --raw "uname -a"', "Run a shell command on a node."],
          ["keanu nodes camera snap --node <id>", "Capture a photo from a node camera."],
        ])}\n\n${theme.muted("Docs:")} ${formatDocsLink("/cli/nodes", "docs.keanu.ai/cli/nodes")}\n`,
    );
