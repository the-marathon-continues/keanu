...
${formatHelpExamples([
  ['keanu agent --to +15555550123 --message "status update"', "Start a new session."],
  ['keanu agent --agent ops --message "Summarize logs"', "Use a specific agent."],
  [
    'keanu agent --session-id 1234 --message "Summarize inbox" --thinking medium',
    "Target a session with explicit thinking level.",
  ],
  [
    'keanu agent --to +15555550123 --message "Trace logs" --verbose on --json',
    "Enable verbose logging and JSON output.",
  ],
  ['keanu agent --to +15555550123 --message "Summon reply" --deliver', "Deliver reply."],
  [
    'keanu agent --agent ops --message "Generate report" --deliver --reply-channel slack --reply-to "#reports"',
    "Send reply to a different channel/target.",
  ],
])}

${theme.muted("Docs:")} ${formatDocsLink("/cli/agent", "docs.keanu.ai/cli/agent")}`,
    )
    .action(async (opts) => {
  ...
  const agents = program
    ...
    .addHelpText(
      "after",
      () =>
        `\n${theme.muted("Docs:")} ${formatDocsLink("/cli/agents", "docs.keanu.ai/cli/agents")}\n`,
    );
...
${theme.heading("Examples:")}
${formatHelpExamples([
  ['keanu agents set-identity --agent main --name "Keanu" --emoji "🦞"', "Set name + emoji."],
  ["keanu agents set-identity --agent main --avatar avatars/keanu.png", "Set avatar path."],
  [
    "keanu agents set-identity --workspace ~/.keanu/workspace --from-identity",
    "Load from IDENTITY.md.",
  ],
  [
    "keanu agents set-identity --identity-file ~/.keanu/workspace/IDENTITY.md --agent main",
