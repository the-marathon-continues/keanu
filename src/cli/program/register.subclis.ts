...
const shouldRegisterPrimaryOnly = (argv: string[]) => {
  if (isTruthyEnvValue(process.env.KEANU_DISABLE_LAZY_SUBCOMMANDS)) {
    return false;
  }
...
const shouldEagerRegisterSubcommands = (_argv: string[]) => {
  return isTruthyEnvValue(process.env.KEANU_DISABLE_LAZY_SUBCOMMANDS);
};

const loadConfig = async (): Promise<KeanuConfig> => {
...
const entries: SubCliEntry[] = [
  ...
  {
    name: "docs",
    description: "Search the live Keanu docs",
    hasSubcommands: false,
    register: async (program) => {
