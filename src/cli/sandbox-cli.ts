...
const SANDBOX_EXAMPLES = {
  main: [
    ["keanu sandbox list", "List all sandbox containers."],
    ["keanu sandbox list --browser", "List only browser containers."],
    ["keanu sandbox recreate --all", "Recreate all containers."],
    ["keanu sandbox recreate --session main", "Recreate a specific session."],
    ["keanu sandbox recreate --agent mybot", "Recreate agent containers."],
    ["keanu sandbox explain", "Explain effective sandbox config."],
  ],
  list: [
    ["keanu sandbox list", "List all sandbox containers."],
    ["keanu sandbox list --browser", "List only browser containers."],
  ...
  recreate: [
    ["keanu sandbox recreate --all", "Recreate all containers."],
    ["keanu sandbox recreate --session main", "Recreate a specific session."],
    ["keanu sandbox recreate --agent mybot", "Recreate a specific agent (includes sub-agents)."],
    ["keanu sandbox recreate --browser --all", "Recreate only browser containers."],
    ["keanu sandbox recreate --all --force", "Skip confirmation."],
  ],
  explain: [
    ["keanu sandbox explain", "Show effective sandbox config."],
    ["keanu sandbox explain --session agent:main:main", "Explain a specific session."],
    ["keanu sandbox explain --agent work", "Explain an agent sandbox."],
    ["keanu sandbox explain --json", "JSON output."],
  ],
} as const;
...
export function registerSandboxCli(program: Command) {
  const sandbox = program
    ...
    .addHelpText(
      "after",
      () =>
        `\n${theme.muted("Docs:")} ${formatDocsLink("/cli/sandbox", "docs.keanu.ai/cli/sandbox")}\n`,
    )
    .action(() => {
