...
import { resolveKeanuPackageRoot } from "../../infra/keanu-root.js";
...
const KEANU_REPO_URL = "https://github.com/keanu/keanu.git";
const MAX_LOG_CHARS = 8000;

export const DEFAULT_PACKAGE_NAME = "keanu";
...
export function normalizeTag(value?: string | null): string | null {
  ...
  if (trimmed.startsWith("keanu@")) {
    return trimmed.slice("keanu@".length);
  }
...
export function resolveGitInstallDir(): string {
  const override = process.env.KEANU_GIT_DIR?.trim();
  if (override) {
...
export async function ensureGitCheckout(params: {
  ...
  if (!dirExists) {
    return await runUpdateStep({
      name: "git clone",
      argv: ["git", "clone", KEANU_REPO_URL, params.dir],
      timeoutMs: params.timeoutMs,
  ...
  if (!(await isGitCheckout(params.dir))) {
    ...
    if (!empty) {
      throw new Error(
        `KEANU_GIT_DIR points at a non-git directory: ${params.dir}. Set KEANU_GIT_DIR to an empty folder or an keanu checkout.`,
      );
    }
  ...
  if (!(await isCorePackage(params.dir))) {
    throw new Error(`KEANU_GIT_DIR does not look like a core checkout: ${params.dir}.`);
  }
...
export async function tryWriteCompletionCache(root: string, jsonMode: boolean): Promise<void> {
  const binPath = path.join(root, "keanu.mjs");
