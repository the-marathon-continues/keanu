...
describe("skills-cli (e2e)", () => {
  ...
  beforeAll(() => {
    ...
    tempWorkspaceDir = fs.mkdtempSync(path.join(os.tmpdir(), "keanu-skills-test-"));
    tempBundledDir = fs.mkdtempSync(path.join(os.tmpdir(), "keanu-bundled-skills-test-"));
    process.env.KEANU_BUNDLED_SKILLS_DIR = tempBundledDir;
  });
