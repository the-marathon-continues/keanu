...
function resolveGatewayPort(cfg: KeanuConfig, env: NodeJS.ProcessEnv): number {
...
function resolveAuth(cfg: KeanuConfig, env: NodeJS.ProcessEnv): ResolveAuthResult {
  ...
  const password =
    env.KEANU_GATEWAY_PASSWORD?.trim() ||
    env.KEANU_GATEWAY_PASSWORD?.trim() ||
