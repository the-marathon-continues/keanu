...
async function createDockerSetupSandbox(): Promise<DockerSetupSandbox> {
  const rootDir = await mkdtemp(join(tmpdir(), "keanu-docker-setup-"));
...
function createEnv(
  ...
  const env: NodeJS.ProcessEnv = {
    ...
    KEANU_GATEWAY_TOKEN: "test-token",
    KEANU_CONFIG_DIR: join(sandbox.rootDir, "config"),
    KEANU_WORKSPACE_DIR: join(sandbox.rootDir, "keanu"),
  };
...
describe("docker-setup.sh", () => {
  ...
  it("handles env defaults, home-volume mounts, and apt build args", async () => {
    ...
    const result = runDockerSetup(activeSandbox, {
      KEANU_DOCKER_APT_PACKAGES: "ffmpeg build-essential",
      KEANU_EXTRA_MOUNTS: undefined,
      KEANU_HOME_VOLUME: "keanu-home",
    });
    expect(result.status).toBe(0);
    const envFile = await readFile(join(activeSandbox.rootDir, ".env"), "utf8");
    expect(envFile).toContain("KEANU_DOCKER_APT_PACKAGES=ffmpeg build-essential");
    expect(envFile).toContain("KEANU_EXTRA_MOUNTS=");
    expect(envFile).toContain("KEANU_HOME_VOLUME=keanu-home");
    const extraCompose = await readFile(
      join(activeSandbox.rootDir, "docker-compose.extra.yml"),
      "utf8",
    );
    expect(extraCompose).toContain("keanu-home:/home/node");
    expect(extraCompose).toContain("volumes:");
    expect(extraCompose).toContain("keanu-home:");
    const log = await readFile(activeSandbox.logPath, "utf8");
    expect(log).toContain("--build-arg KEANU_DOCKER_APT_PACKAGES=ffmpeg build-essential");
  });
  ...
  it("precreates config identity dir for CLI device auth writes", async () => {
    ...
    const result = runDockerSetup(activeSandbox, {
      KEANU_CONFIG_DIR: configDir,
      KEANU_WORKSPACE_DIR: workspaceDir,
    });
  ...
  it("rejects injected multiline KEANU_EXTRA_MOUNTS values", async () => {
    const activeSandbox = requireSandbox(sandbox);

    const result = runDockerSetup(activeSandbox, {
      KEANU_EXTRA_MOUNTS: "/tmp:/tmp\n  evil-service:\n    image: alpine",
    });

    expect(result.status).not.toBe(0);
    expect(result.stderr).toContain("KEANU_EXTRA_MOUNTS cannot contain control characters");
  });

  it("rejects invalid KEANU_EXTRA_MOUNTS mount format", async () => {
    const activeSandbox = requireSandbox(sandbox);

    const result = runDockerSetup(activeSandbox, {
      KEANU_EXTRA_MOUNTS: "bad mount spec",
    });
  ...
  it("rejects invalid KEANU_HOME_VOLUME names", async () => {
    const activeSandbox = requireSandbox(sandbox);

    const result = runDockerSetup(activeSandbox, {
      KEANU_HOME_VOLUME: "bad name",
    });

    expect(result.status).not.toBe(0);
    expect(result.stderr).toContain("KEANU_HOME_VOLUME must match");
  });
