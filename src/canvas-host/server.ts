...
(() => {
  ...
  const hasIOS = () =>
    !!(
      ...
      window.webkit.messageHandlers.keanuCanvasA2UIAction
    );
  const hasAndroid = () =>
    !!(
      (window.keanuCanvasA2UIAction &&
        typeof window.keanuCanvasA2UIAction.postMessage === "function")
    );
  const hasHelper = () => typeof window.keanuSendUserAction === "function";
  statusEl.innerHTML =
    "Bridge: " +
  ...
  window.addEventListener("keanu:a2ui-action-status", onStatus);

  function send(name, sourceComponentId) {
    if (!hasHelper()) {
      log("No action bridge found. Ensure you're viewing this on an iOS/Android Keanu node canvas.");
      return;
    }
    const sendUserAction =
      typeof window.keanuSendUserAction === "function"
        ? window.keanuSendUserAction
        : undefined;
...
function isDisabledByEnv() {
  if (isTruthyEnvValue(process.env.KEANU_SKIP_CANVAS_HOST)) {
    return true;
  }
  if (isTruthyEnvValue(process.env.KEANU_SKIP_CANVAS_HOST)) {
    return true;
    ...
    try {
      ...
      if (!opened) {
        if (urlPath === "/" || urlPath.endsWith("/")) {
          ...
          res.end(
            `<!doctype html><meta charset="utf-8" /><title>Keanu Canvas</title><pre>Missing file.\nCreate ${rootDir}/index.html</pre>`,
          );
          return true;
        }
        res.statusCode = 404;
