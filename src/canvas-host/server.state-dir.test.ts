...
describe("canvas host state dir defaults", () => {
  it("uses KEANU_STATE_DIR for the default canvas root", async () => {
    await withStateDirEnv("keanu-canvas-state-", async ({ stateDir }) => {
      ...
      try {
        ...
        expect(indexContents).toContain("Keanu Canvas");
      } finally {
        await handler.close();
      }
    });
