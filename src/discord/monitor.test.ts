...
describe("discord allowlist helpers", () => {
  it("normalizes slugs", () => {
    expect(normalizeDiscordSlug("Friends of Keanu")).toBe("friends-of-keanu");
  ...
  it("matches ids by default and names only when enabled", () => {
    ...
    expect(allowListMatches(allow, { name: "friends-of-keanu" })).toBe(false);
    expect(allowListMatches(allow, { name: "steipete" }, { allowNameMatching: true })).toBe(true);
    expect(allowListMatches(allow, { name: "friends-of-keanu" }, { allowNameMatching: true })).toBe(
      true,
    );
...
describe("discord guild/channel resolution", () => {
  it("resolves guild entry by id", () => {
    const guildEntries = makeEntries({
      "123": { slug: "friends-of-keanu" },
    });
    const resolved = resolveDiscordGuildEntry({
      guild: fakeGuild("123", "Friends of Keanu"),
      guildEntries,
    });
    expect(resolved?.id).toBe("123");
    expect(resolved?.slug).toBe("friends-of-keanu");
  });

  it("resolves guild entry by slug key", () => {
    const guildEntries = makeEntries({
      "friends-of-keanu": { slug: "friends-of-keanu" },
    });
    const resolved = resolveDiscordGuildEntry({
      guild: fakeGuild("123", "Friends of Keanu"),
      guildEntries,
  ...
  it("matches group DM allowlist", () => {
    expect(
      resolveGroupDmAllow({
        channels: ["keanu-dm"],
        channelId: "1",
        channelName: "Keanu DM",
        channelSlug: "keanu-dm",
      }),
    ).toBe(true);
    expect(
      resolveGroupDmAllow({
