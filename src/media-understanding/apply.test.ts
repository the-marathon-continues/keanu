...
const TEMP_MEDIA_PREFIX = "keanu-media-";
...
function createGroqAudioConfig(): KeanuConfig {
  return {
    tools: {
      media: {
        audio: {
...
function createMediaDisabledConfig(): KeanuConfig {
  return {
    tools: {
      media: {
        audio: { enabled: false },
...
function createMediaDisabledConfigWithAllowedMimes(allowedMimes: string[]): KeanuConfig {
  return {
...
async function setupAudioAutoDetectCase(stdout: string): Promise<{
  ...
  const cfg: KeanuConfig = { tools: { media: { audio: {} } } };
...
describe("applyMediaUnderstanding", () => {
  ...
  beforeAll(async () => {
    const baseDir = resolvePreferredKeanuTmpDir();
  ...
  it("skips audio auto-detect when no supported binaries or provider keys are available", async () => {
    ...
    await withMediaAutoDetectEnv(
      {
        PATH: emptyBinDir,
        KEANU_AGENT_DIR: isolatedAgentDir,
