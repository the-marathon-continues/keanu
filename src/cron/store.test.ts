...
async function makeStorePath() {
  const dir = await fs.mkdtemp(path.join(os.tmpdir(), "keanu-cron-store-"));
  return {
    dir,
...
describe("resolveCronStorePath", () => {
  ...
  it("uses KEANU_HOME for tilde expansion", () => {
    vi.stubEnv("KEANU_HOME", "/srv/keanu-home");
    vi.stubEnv("HOME", "/home/other");

    const result = resolveCronStorePath("~/cron/jobs.json");
    expect(result).toBe(path.resolve("/srv/keanu-home", "cron", "jobs.json"));
  });
});
