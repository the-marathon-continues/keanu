...
import { scheduleGatewaySigusr1Restart, triggerKeanuRestart } from "../../infra/restart.js";
...
export const handleRestartCommand: CommandHandler = async (params, allowTextCommands) => {
  ...
  if (hasSigusr1Listener) {
    ...
    return {
      shouldContinue: false,
      reply: {
        text: "⚙️ Restarting Keanu in-process (SIGUSR1); back in a few seconds.",
      },
    };
  }
  const restartMethod = triggerKeanuRestart();
  if (!restartMethod.ok) {
  ...
  return {
    shouldContinue: false,
    reply: {
      text: `⚙️ Restarting Keanu via ${restartMethod.method}; give me a few seconds to come back online.`,
    },
  };
};
