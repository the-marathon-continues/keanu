...
describe("abort detection", () => {
  ...
  async function createAbortConfig(params?: {
    ...
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "keanu-abort-"));
  ...
  it("isAbortRequestText aligns abort command semantics", () => {
    ...
    expect(isAbortRequestText("stop keanu!!!")).toBe(true);
    ...
    expect(isAbortRequestText("/stop@keanu_bot", { botUsername: "keanu_bot" })).toBe(true);
    expect(isAbortRequestText("/Stop@keanu_bot", { botUsername: "keanu_bot" })).toBe(true);
