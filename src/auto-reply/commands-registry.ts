...
export function isCommandEnabled(cfg: KeanuConfig, commandKey: string): boolean {
...
function resolveDefaultCommandContext(cfg?: KeanuConfig): {
  provider: string;
  model: string;
} {
...
export function getCommandDetection(_cfg?: KeanuConfig): CommandDetection {
  const commands = getChatCommands();
...
export function maybeResolveTextAlias(raw: string, cfg?: KeanuConfig) {
