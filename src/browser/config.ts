import type { BrowserConfig, BrowserProfileConfig, KeanuConfig } from "../config/config.js";
...
function normalizeHexColor(raw: string | undefined) {
  ...
  if (!value) {
    return DEFAULT_KEANU_BROWSER_COLOR;
  }
...
/**
 * Ensure the default "keanu" profile exists in the profiles map.
...
function ensureDefaultProfile(
  ...
  if (!result[DEFAULT_KEANU_BROWSER_PROFILE_NAME]) {
    result[DEFAULT_KEANU_BROWSER_PROFILE_NAME] = {
 ...
 *
 * Note: this is a Keanu browser profile (routing config), not a Chrome user profile.
...
function ensureDefaultChromeExtensionProfile(
  ...
  // (legacy single-profile configs may use controlPort+1 for keanu/keanu CDP).
...
export function resolveProfile(
  ...
  let cdpUrl = "";
  const driver = profile.driver === "extension" ? "extension" : "keanu";
