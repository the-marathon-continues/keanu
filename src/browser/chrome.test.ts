...
describe("browser chrome profile decoration", () => {
  ...
  beforeAll(async () => {
    fixtureRoot = await fsp.mkdtemp(path.join(os.tmpdir(), "keanu-chrome-suite-"));
  });
  ...
  it("writes expected name + signed ARGB seed to Chrome prefs", async () => {
    ...
    decorateKeanuProfile(userDataDir, { color: DEFAULT_KEANU_BROWSER_COLOR });

    const expectedSignedArgb = ((0xff << 24) | 0x00a86b) >> 0;

    const def = await readDefaultProfileFromLocalState(userDataDir);

    expect(def.name).toBe(DEFAULT_KEANU_BROWSER_PROFILE_NAME);
    expect(def.shortcut_name).toBe(DEFAULT_KEANU_BROWSER_PROFILE_NAME);
    ...
    const marker = await fsp.readFile(path.join(userDataDir, ".keanu-profile-decorated"), "utf-8");
    expect(marker.trim()).toMatch(/^\d+$/);
  });

  it("best-effort writes name when color is invalid", async () => {
    const userDataDir = await createUserDataDir();
    decorateKeanuProfile(userDataDir, { color: "palm-green" });
    const def = await readDefaultProfileFromLocalState(userDataDir);
  ...
  it("is idempotent when rerun on an existing profile", async () => {
    ...
    expect(profile.name).toBe(DEFAULT_KEANU_BROWSER_PROFILE_NAME);
  });
});
...
describe("browser chrome helpers", () => {
  ...
  it("stopKeanuChrome no-ops when process is already killed", async () => {
    const proc = makeProc({ killed: true });
    await stopKeanuChrome(
      {
        proc,
        cdpPort: 12345,
      } as unknown as Parameters<typeof stopKeanuChrome>[0],
      10,
    );
    expect(proc.kill).not.toHaveBeenCalled();
  });

  it("stopKeanuChrome sends SIGTERM and returns once CDP is down", async () => {
  ...
  it("stopKeanuChrome escalates to SIGKILL when CDP stays reachable", async () => {
    vi.stubGlobal(
      "fetch",
      vi.fn().mockResolvedValue({
        ok: true,
