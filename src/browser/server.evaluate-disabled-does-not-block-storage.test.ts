  ...
  return {
    ...
    loadConfig: () => ({
      browser: {
        ...
        profiles: {
          keanu: { cdpPort: testPort + 1, color: "#FF4500" },
        },
      },
    }),
...
describe("browser control evaluate gating", () => {
  beforeEach(async () => {
    ...
    prevGatewayPort = process.env.KEANU_GATEWAY_PORT;
    process.env.KEANU_GATEWAY_PORT = String(testPort - 2);
    prevGatewayToken = process.env.KEANU_GATEWAY_TOKEN;
    prevGatewayPassword = process.env.KEANU_GATEWAY_PASSWORD;
