...
const RELAY_TOKEN_CONTEXT = "keanu-extension-relay-v1";
const DEFAULT_RELAY_PROBE_TIMEOUT_MS = 500;
const KEANU_RELAY_BROWSER = "Keanu/extension-relay";

function resolveGatewayAuthToken(): string | null {
  const envToken =
    process.env.KEANU_GATEWAY_TOKEN?.trim() || process.env.KEANU_GATEWAY_TOKEN?.trim();
  if (envToken) {
    return envToken;
  }
  try {
...
export function resolveRelayAcceptedTokensForPort(port: number): string[] {
  ...
  if (!gatewayToken) {
    throw new Error(
      "extension relay requires gateway auth token (set gateway.auth.token or KEANU_GATEWAY_TOKEN)",
    );
  }
...
export async function probeAuthenticatedKeanuRelay(params: {
  baseUrl: string;
  ...
  try {
    ...
    return browserName === KEANU_RELAY_BROWSER;
  } catch {
    return false;
  } finally {
    clearTimeout(timer);
