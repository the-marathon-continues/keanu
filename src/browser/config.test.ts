...
describe("browser config", () => {
  it("defaults to enabled with loopback defaults and lobster-orange color", () => {
    ...
    expect(keanu?.driver).toBe("keanu");
    expect(keanu?.cdpPort).toBe(18800);
    expect(keanu?.cdpUrl).toBe("http://127.0.0.1:18800");
    expect(resolved.remoteCdpTimeoutMs).toBe(1500);
    expect(resolved.remoteCdpHandshakeTimeoutMs).toBe(3000);
  });

  it("derives default ports from KEANU_GATEWAY_PORT when unset", () => {
    withEnv({ KEANU_GATEWAY_PORT: "19001" }, () => {
      const resolved = resolveBrowserConfig(undefined);
      ...
      const keanu = resolveProfile(resolved, "keanu");
      expect(keanu?.cdpPort).toBe(19012);
      expect(keanu?.cdpUrl).toBe("http://127.0.0.1:19012");
    });
  });

  it("derives default ports from gateway.port when env is unset", () => {
    withEnv({ KEANU_GATEWAY_PORT: undefined }, () => {
      ...
      const keanu = resolveProfile(resolved, "keanu");
      expect(keanu?.cdpPort).toBe(19022);
      expect(keanu?.cdpUrl).toBe("http://127.0.0.1:19022");
    });
  });
  ...
  it("treats non-loopback cdpUrl as remote", () => {
    ...
    const profile = resolveProfile(resolved, "keanu");
  ...
  it("does not add the built-in chrome extension profile if the derived relay port is already used", () => {
    const resolved = resolveBrowserConfig({
      profiles: {
        keanu: { cdpPort: 18792, color: "#FF4500" },
      },
    });
