...
describe("extension-relay-auth", () => {
  ...
  beforeEach(() => {
    prevGatewayToken = process.env.KEANU_GATEWAY_TOKEN;
  ...
  it("accepts authenticated keanu relay probe responses", async () => {
    ...
    await withRelayServer(
      ...
      async ({ port }) => {
        ...
        const ok = await probeAuthenticatedKeanuRelay({
          baseUrl: `http://127.0.0.1:${port}`,
          relayAuthHeader: "x-keanu-relay-token",
