...
const tmpDirMocks = vi.hoisted(() => ({
  resolvePreferredKeanuTmpDir: vi.fn(() => "/tmp/keanu"),
}));
vi.mock("../infra/tmp-keanu-dir.js", () => tmpDirMocks);
...
describe("pw-tools-core", () => {
  beforeEach(() => {
    ...
    tmpDirMocks.resolvePreferredKeanuTmpDir.mockReturnValue("/tmp/keanu");
  });
  ...
  it("uses preferred tmp dir when waiting for download without explicit path", async () => {
    tmpDirMocks.resolvePreferredKeanuTmpDir.mockReturnValue("/tmp/keanu-preferred");
    const { res, outPath } = await waitForImplicitDownloadOutput({
      downloadUrl: "https://example.com/file.bin",
      suggestedFilename: "file.bin",
    });
    expect(typeof outPath).toBe("string");
    const expectedRootedDownloadsDir = path.join(path.sep, "tmp", "keanu-preferred", "downloads");
    const expectedDownloadsTail = `${path.join("tmp", "keanu-preferred", "downloads")}${path.sep}`;
    ...
    expect(tmpDirMocks.resolvePreferredKeanuTmpDir).toHaveBeenCalled();
  });
  ...
  it("sanitizes suggested download filenames to prevent traversal escapes", async () => {
    ...
    expect(path.dirname(String(outPath))).toBe(
      path.join(path.sep, "tmp", "keanu-preferred", "downloads"),
    );
    expect(path.basename(String(outPath))).toMatch(/-passwd$/);
    expect(path.normalize(res.path)).toContain(
      path.normalize(`${path.join("tmp", "keanu-preferred", "downloads")}${path.sep}`),
    );
  });
