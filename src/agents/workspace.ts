...
export function resolveDefaultAgentWorkspaceDir(
  ...
  const profile = env.KEANU_PROFILE?.trim();
  if (profile && profile.toLowerCase() !== "default") {
    return path.join(home, ".keanu", `workspace-${profile}`);
  }
  return path.join(home, ".keanu", "workspace");
}
...
const WORKSPACE_STATE_DIRNAME = ".keanu";
