...
export function buildSandboxCreateArgs(params: {
  ...
  args.push("--label", "keanu.sandbox=1");
  args.push("--label", `keanu.sessionKey=${params.scopeKey}`);
  args.push("--label", `keanu.createdAtMs=${createdAtMs}`);
  if (params.configHash) {
    args.push("--label", `keanu.configHash=${params.configHash}`);
  }
...
async function readContainerConfigHash(containerName: string): Promise<string | null> {
  return await readDockerContainerLabel(containerName, "keanu.configHash");
}

function formatSandboxRecreateHint(params: { scope: SandboxConfig["scope"]; sessionKey: string }) {
  if (params.scope === "session") {
    return formatCliCommand(`keanu sandbox recreate --session ${params.sessionKey}`);
  }
  if (params.scope === "agent") {
    const agentId = resolveSandboxAgentId(params.sessionKey) ?? "main";
    return formatCliCommand(`keanu sandbox recreate --agent ${agentId}`);
  }
  return formatCliCommand("keanu sandbox recreate --all");
}
