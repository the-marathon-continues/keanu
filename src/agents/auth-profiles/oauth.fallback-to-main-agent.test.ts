...
describe("resolveApiKeyForProfile fallback to main agent", () => {
  const envSnapshot = captureEnv(["KEANU_STATE_DIR", "KEANU_AGENT_DIR", "PI_CODING_AGENT_DIR"]);
  let tmpDir: string;
  ...
  beforeEach(async () => {
    ...
    // Set environment variables so resolveKeanuAgentDir() returns mainAgentDir
    process.env.KEANU_STATE_DIR = tmpDir;
    process.env.KEANU_AGENT_DIR = mainAgentDir;
