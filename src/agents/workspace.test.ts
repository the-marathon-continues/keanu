...
describe("resolveDefaultAgentWorkspaceDir", () => {
  it("uses KEANU_HOME for default workspace resolution", () => {
    const dir = resolveDefaultAgentWorkspaceDir({
      KEANU_HOME: "/srv/keanu-home",
      HOME: "/home/other",
    } as NodeJS.ProcessEnv);

    expect(dir).toBe(path.join(path.resolve("/srv/keanu-home"), ".keanu", "workspace"));
  });
});

const WORKSPACE_STATE_PATH_SEGMENTS = [".keanu", "workspace-state.json"] as const;
...
describe("ensureAgentWorkspace", () => {
  it("creates BOOTSTRAP.md and records a seeded marker for brand new workspaces", async () => {
    const tempDir = await makeTempWorkspace("keanu-workspace-");
