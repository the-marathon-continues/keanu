...
describe("resolveAgentConfig", () => {
  ...
  it("resolves explicit and effective model primary separately", () => {
    ...
    const cfgWithObjectDefault: KeanuConfig = {
      agents: {
        defaults: {
          model: {
    ...
    const cfgNoDefaults: KeanuConfig = {
      agents: {
        list: [{ id: "main" }],
      },
    };
  ...
  it("supports per-agent model primary+fallbacks", () => {
    ...
    const cfgNoOverride: KeanuConfig = {
      agents: {
        list: [
          {
            id: "linus",
    ...
    const cfgInheritDefaults: KeanuConfig = {
      agents: {
        defaults: {
          model: {
            fallbacks: ["openai/gpt-4.1"],
  ...
  it("should normalize agent id", () => {
    ...
    expect(result?.workspace).toBe("~/keanu");
  });

  it("uses KEANU_HOME for default agent workspace", () => {
    const home = path.join(path.sep, "srv", "keanu-home");
    vi.stubEnv("KEANU_HOME", home);

    const workspace = resolveAgentWorkspaceDir({} as KeanuConfig, "main");
    expect(workspace).toBe(path.join(path.resolve(home), ".keanu", "workspace"));
  });

  it("uses KEANU_HOME for default agentDir", () => {
    const home = path.join(path.sep, "srv", "keanu-home");
    vi.stubEnv("KEANU_HOME", home);
    // Clear state dir so it falls back to KEANU_HOME
    vi.stubEnv("KEANU_STATE_DIR", "");

    const agentDir = resolveAgentDir({} as KeanuConfig, "main");
    expect(agentDir).toBe(path.join(path.resolve(home), ".keanu", "agents", "main", "agent"));
  });
});
