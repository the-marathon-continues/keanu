...
const defaultTools = createKeanuCodingTools({ senderIsOwner: true });
...
describe("createKeanuCodingTools", () => {
  ...
  it("enforces apply_patch availability and canonical names across model/provider constraints", () => {
    ...
    const enabledConfig: KeanuConfig = {
      tools: {
        exec: {
    ...
    const openAiTools = createKeanuCodingTools({
    ...
    const anthropicTools = createKeanuCodingTools({
      config: enabledConfig,
    ...
    const allowModelsConfig: KeanuConfig = {
      tools: {
        exec: {
    ...
    const allowed = createKeanuCodingTools({
    ...
    const denied = createKeanuCodingTools({
      config: allowModelsConfig,
      modelProvider: "openai",
      modelId: "gpt-5-mini",
    });
    ...
    const oauthTools = createKeanuCodingTools({
      modelProvider: "anthropic",
      modelAuthMode: "oauth",
    });
