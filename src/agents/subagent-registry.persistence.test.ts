...
describe("subagent registry persistence", () => {
  const envSnapshot = captureEnv(["KEANU_STATE_DIR"]);
  ...
  const writePersistedRegistry = async (
    ...
    tempStateDir = await fs.mkdtemp(path.join(os.tmpdir(), "keanu-subagent-"));
  ...
  it("uses isolated temp state when KEANU_STATE_DIR is unset in tests", async () => {
    ...
    expect(registryPath).toContain(path.join(os.tmpdir(), "keanu-test-state"));
  });
});
