...
export function listChannelSupportedActions(params: {
  ...
  const cfg = params.cfg ?? ({} as KeanuConfig);
...
export function listChannelAgentTools(params: { cfg?: KeanuConfig }): ChannelAgentTool[] {
...
function runPluginListActions(plugin: ChannelPlugin, cfg: KeanuConfig): ChannelMessageActionName[] {
  if (!plugin.actions?.listActions) {
    return [];
  }
  try {
