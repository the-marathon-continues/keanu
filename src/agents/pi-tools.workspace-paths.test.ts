...
import { createKeanuCodingTools } from "./pi-tools.js";
...
describe("workspace path resolution", () => {
  it("resolves relative read/write/edit paths against workspaceDir even after cwd changes", async () => {
    await withTempDir("keanu-ws-", async (workspaceDir) => {
      await withTempDir("keanu-cwd-", async (otherDir) => {
        const cwdSpy = vi.spyOn(process, "cwd").mockReturnValue(otherDir);
        try {
          const tools = createKeanuCodingTools({ workspaceDir });
          ...
          expect(await fs.readFile(path.join(workspaceDir, editFile), "utf8")).toBe("hello keanu");
        } finally {
          cwdSpy.mockRestore();
        }
      });
  ...
  it("lets exec workdir override the workspace default", async () => {
    await withTempDir("keanu-ws-", async (workspaceDir) => {
      await withTempDir("keanu-override-", async (overrideDir) => {
        const tools = createKeanuCodingTools({
          workspaceDir,
          exec: { host: "gateway", ask: "off", security: "full" },
        });
  ...
  it("rejects @-prefixed absolute paths outside workspace when workspaceOnly is enabled", async () => {
    await withTempDir("keanu-ws-", async (workspaceDir) => {
      const cfg: KeanuConfig = { tools: { fs: { workspaceOnly: true } } };
      const tools = createKeanuCodingTools({ workspaceDir, config: cfg });
      const { readTool } = expectReadWriteEditTools(tools);

      const outsideAbsolute = path.resolve(path.parse(workspaceDir).root, "outside-keanu.txt");
      await expect(
...
describe("sandboxed workspace paths", () => {
  it("uses sandbox workspace for relative read/write/edit", async () => {
    await withTempDir("keanu-sandbox-", async (sandboxDir) => {
      await withTempDir("keanu-workspace-", async (workspaceDir) => {
        ...
        const tools = createKeanuCodingTools({ workspaceDir, sandbox });
        const { readTool, writeTool, editTool } = expectReadWriteEditTools(tools);
