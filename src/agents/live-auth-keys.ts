...
const GOOGLE_LIVE_SINGLE_KEY = "KEANU_LIVE_GEMINI_KEY";
...
const PROVIDER_API_KEY_CONFIG: Record<string, Omit<ProviderApiKeyConfig, "fallbackVars">> = {
  anthropic: {
    liveSingle: "KEANU_LIVE_ANTHROPIC_KEY",
    listVar: "KEANU_LIVE_ANTHROPIC_KEYS",
  ...
  openai: {
    liveSingle: "KEANU_LIVE_OPENAI_KEY",
...
function resolveProviderApiKeyConfig(provider: string): ProviderApiKeyConfig {
  ...
  const liveSingle = custom?.liveSingle ?? `KEANU_LIVE_${base}_KEY`;
