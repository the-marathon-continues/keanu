...
type WebSearchConfig = NonNullable<KeanuConfig["tools"]>["web"] extends infer Web
...
function resolveSearchConfig(cfg?: KeanuConfig): WebSearchConfig {
...
function missingSearchKeyPayload(provider: (typeof SEARCH_PROVIDERS)[number]) {
  ...
  return {
    ...
    message: `web_search needs a Brave Search API key. Run \`${formatCliCommand("keanu configure --section web")}\` to store it, or set BRAVE_API_KEY in the Gateway environment.`,
    docs: "https://docs.keanu.ai/tools/web",
  };
}
...
async function runPerplexitySearch(params: {
  ...
  const res = await fetch(endpoint, {
    ...
    headers: {
      ...
      "HTTP-Referer": "https://keanu.ai",
      "X-Title": "Keanu Web Search",
    },
