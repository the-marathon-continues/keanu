...
describe("handleTelegramAction", () => {
  ...
  function reactionConfig(reactionLevel: "minimal" | "extensive" | "off" | "ack"): KeanuConfig {
    return {
      channels: { telegram: { botToken: "tok", reactionLevel } },
    } as KeanuConfig;
  }

  function telegramConfig(overrides?: Record<string, unknown>): KeanuConfig {
    return {
      channels: {
        telegram: {
          botToken: "tok",
  ...
  it("rejects sticker actions when disabled by default", async () => {
    const cfg = { channels: { telegram: { botToken: "tok" } } } as KeanuConfig;
    await expect(
...
describe("handleTelegramAction per-account gating", () => {
  async function expectAccountStickerSend(cfg: KeanuConfig, accountId = "media") {
    await handleTelegramAction(
