...
describe("gateway tool defaults", () => {
  const envSnapshot = {
    keanu: process.env.KEANU_GATEWAY_TOKEN,
    clawdbot: process.env.KEANU_GATEWAY_TOKEN,
  };
  ...
  afterAll(() => {
    if (envSnapshot.keanu === undefined) {
      delete process.env.KEANU_GATEWAY_TOKEN;
    } else {
      process.env.KEANU_GATEWAY_TOKEN = envSnapshot.keanu;
    }
    if (envSnapshot.clawdbot === undefined) {
      delete process.env.KEANU_GATEWAY_TOKEN;
    } else {
      process.env.KEANU_GATEWAY_TOKEN = envSnapshot.clawdbot;
    }
  });
  ...
  it("uses KEANU_GATEWAY_TOKEN for allowlisted local overrides", () => {
    process.env.KEANU_GATEWAY_TOKEN = "env-token";
  ...
  it("does not leak local env/config tokens to remote overrides", () => {
    process.env.KEANU_GATEWAY_TOKEN = "local-env-token";
    process.env.KEANU_GATEWAY_TOKEN = "legacy-env-token";
    configState.value = {
      gateway: {
