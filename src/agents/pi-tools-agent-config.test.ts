...
describe("Agent-specific tool filtering", () => {
  ...
  async function withApplyPatchEscapeCase(
    ...
    const workspaceDir = await fs.mkdtemp(path.join(os.tmpdir(), "keanu-pi-tools-"));
...
*** End Patch`;
  ...
  function createMainSessionTools(cfg: KeanuConfig) {
    return createKeanuCodingTools({
      config: cfg,
      sessionKey: "agent:main:main",
  ...
  function createMainAgentConfig(params: {
    tools: NonNullable<KeanuConfig["tools"]>;
    agentTools?: NonNullable<NonNullable<KeanuConfig["agents"]>["list"]>[number]["tools"];
  }): KeanuConfig {
    return {
      tools: params.tools,
      agents: {
  ...
  it("should allow different tool policies for different agents", () => {
    ...
    const mainTools = createKeanuCodingTools({
      config: cfg,
      sessionKey: "agent:main:main",
    ...
    const familyTools = createKeanuCodingTools({
      config: cfg,
  ...
  it("should apply group tool policy overrides (group-specific beats wildcard)", () => {
    ...
    const trustedTools = createKeanuCodingTools({
      config: cfg,
  ...
  it("should apply per-sender tool policies for group tools", () => {
    ...
    const aliceTools = createKeanuCodingTools({
      config: cfg,
    ...
    const bobTools = createKeanuCodingTools({
      config: cfg,
      sessionKey: "agent:main:whatsapp:group:family",
      senderId: "bob",
  ...
  it("should not let default sender policy override group tools", () => {
    ...
    const adminTools = createKeanuCodingTools({
      config: cfg,
  ...
  it("should apply agent-specific exec host defaults over global defaults", async () => {
    ...
    const helperTools = createKeanuCodingTools({
      config: cfg,
