...
export function createKeanuTools(options?: {
  ...
  /**
   * If true, include tool_search meta-tool and exclude deferred tools from base context.
  ...
  // Build tool index for search (includes all tools)
  let toolIndexCache: ToolSearchEntry[] | null = null;
  const getToolIndex = (): ToolSearchEntry[] => {
    if (!toolIndexCache) {
      toolIndexCache = buildToolIndex(allTools, {
        getDeferredFlag: (tool) => getPluginToolMeta(tool)?.deferred ?? false,
  ...
  // Filter out deferred plugin tools from base context
  const baseTools = allTools.filter((tool) => {
    const meta = getPluginToolMeta(tool);
    return !meta?.deferred;
  });

  // Add tool_search as always-available
  const toolSearchTool = createToolSearchTool({ getToolIndex });

  return [toolSearchTool, ...baseTools];
}
