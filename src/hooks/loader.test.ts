...
describe("loader", () => {
  ...
  beforeAll(async () => {
    fixtureRoot = await fs.mkdtemp(path.join(os.tmpdir(), "keanu-hooks-loader-"));
  });
  ...
  beforeEach(async () => {
    ...
    envSnapshot = captureEnv(["KEANU_BUNDLED_HOOKS_DIR"]);
    process.env.KEANU_BUNDLED_HOOKS_DIR = "/nonexistent/bundled/hooks";
  });
