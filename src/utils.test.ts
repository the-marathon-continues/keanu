...
describe("ensureDir", () => {
  it("creates nested directory", async () => {
    await withTempDirSync("keanu-test-", async (tmp) => {
...
describe("jidToE164", () => {
  ...
  it("maps @lid from authDir mapping files", () => {
    withTempDirSync("keanu-auth-", (authDir) => {
  ...
  it("falls back through lidMappingDirs in order", () => {
    withTempDirSync("keanu-lid-a-", (first) => {
      withTempDirSync("keanu-lid-b-", (second) => {
...
describe("resolveConfigDir", () => {
  it("prefers ~/.keanu when legacy dir is missing", async () => {
    const root = await fs.promises.mkdtemp(path.join(os.tmpdir(), "keanu-config-dir-"));
    try {
...
describe("resolveHomeDir", () => {
  it("prefers KEANU_HOME over HOME", () => {
    vi.stubEnv("KEANU_HOME", "/srv/keanu-home");
    vi.stubEnv("HOME", "/home/other");

    expect(resolveHomeDir()).toBe(path.resolve("/srv/keanu-home"));
...
describe("shortenHomePath", () => {
  it("uses $KEANU_HOME prefix when KEANU_HOME is set", () => {
    vi.stubEnv("KEANU_HOME", "/srv/keanu-home");
    vi.stubEnv("HOME", "/home/other");

    expect(shortenHomePath(`${path.resolve("/srv/keanu-home")}/.keanu/keanu.json`)).toBe(
      "$KEANU_HOME/.keanu/keanu.json",
    );
...
describe("shortenHomeInString", () => {
  it("uses $KEANU_HOME replacement when KEANU_HOME is set", () => {
    vi.stubEnv("KEANU_HOME", "/srv/keanu-home");
    vi.stubEnv("HOME", "/home/other");

    expect(
      shortenHomeInString(`config: ${path.resolve("/srv/keanu-home")}/.keanu/keanu.json`),
    ).toBe("config: $KEANU_HOME/.keanu/keanu.json");
...
describe("resolveUserPath", () => {
  ...
  it("expands ~/ to home dir", () => {
    expect(resolveUserPath("~/keanu")).toBe(path.resolve(os.homedir(), "keanu"));
  });
  ...
  it("prefers KEANU_HOME for tilde expansion", () => {
    vi.stubEnv("KEANU_HOME", "/srv/keanu-home");
    vi.stubEnv("HOME", "/home/other");

    expect(resolveUserPath("~/keanu")).toBe(path.resolve("/srv/keanu-home", "keanu"));
