...
async function runConfigureChannelAccess<TResolved>(params: {
  ...
  label?: string;
  placeholder?: string;
  setPolicy: (cfg: KeanuConfig, policy: ChannelAccessPolicy) => KeanuConfig;
  resolveAllowlist: (params: { cfg: KeanuConfig; entries: string[] }) => Promise<TResolved>;
  applyAllowlist: (params: { cfg: KeanuConfig; resolved: TResolved }) => KeanuConfig;
}) {
...
describe("configureChannelAccessWithAllowlist", () => {
  it("returns input config when user skips access configuration", async () => {
    ...
    const setPolicy = vi.fn((next: KeanuConfig) => next);
    const resolveAllowlist = vi.fn(async () => [] as string[]);
    const applyAllowlist = vi.fn((params: { cfg: KeanuConfig }) => params.cfg);
  ...
  it("applies non-allowlist policy directly", async () => {
    ...
    const setPolicy = vi.fn(
      (next: KeanuConfig, policy: ChannelAccessPolicy): KeanuConfig => ({
        ...next,
  ...
  it("resolves allowlist entries and applies them after forcing allowlist policy", async () => {
    ...
    const calls: string[] = [];
    const setPolicy = vi.fn((next: KeanuConfig, policy: ChannelAccessPolicy): KeanuConfig => {
    ...
    const resolveAllowlist = vi.fn(async (params: { cfg: KeanuConfig; entries: string[] }) => {
      calls.push("resolve");
      expect(params.cfg).toBe(cfg);
      expect(params.entries).toEqual(["#general", "#support"]);
      return ["C1", "C2"];
    });
    const applyAllowlist = vi.fn((params: { cfg: KeanuConfig; resolved: string[] }) => {
      calls.push("apply");
