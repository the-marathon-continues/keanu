...
function cfg(accounts?: Record<string, unknown> | null): KeanuConfig {
  if (accounts === null) {
    return { channels: { testchannel: {} } } as unknown as KeanuConfig;
  }
  if (accounts === undefined) {
    return {} as unknown as KeanuConfig;
  }
  return { channels: { testchannel: { accounts } } } as unknown as KeanuConfig;
}

describe("createAccountListHelpers", () => {
  describe("listConfiguredAccountIds", () => {
    it("returns empty for missing config", () => {
      expect(listConfiguredAccountIds({} as KeanuConfig)).toEqual([]);
    });
  ...
  describe("listAccountIds", () => {
    it('returns ["default"] for empty config', () => {
      expect(listAccountIds({} as KeanuConfig)).toEqual(["default"]);
    });
  ...
  describe("resolveDefaultAccountId", () => {
    ...
    it('returns "default" for empty config', () => {
      expect(resolveDefaultAccountId({} as KeanuConfig)).toBe("default");
    });
  });
});
