...
function telegramCfg(): KeanuConfig {
  return { channels: { telegram: { botToken: "tok" } } } as KeanuConfig;
}
...
async function runTelegramAction(
  ...
  options?: { cfg?: KeanuConfig; accountId?: string },
) {
...
async function runSignalAction(
  ...
  const cfg =
    options?.cfg ?? ({ channels: { signal: { account: "+15550001111" } } } as KeanuConfig);
...
function slackHarness() {
  const cfg = { channels: { slack: { botToken: "tok" } } } as KeanuConfig;
...
function createSignalAccountOverrideCfg(): KeanuConfig {
  return {
    channels: {
      signal: {
...
function createDiscordModerationOverrideCfg(params?: { channelsEnabled?: boolean }): KeanuConfig {
...
describe("discord message actions", () => {
  it("lists channel and upload actions by default", async () => {
    const cfg = { channels: { discord: { token: "d0" } } } as KeanuConfig;
  ...
  it("lists moderation when at least one account enables it", () => {
    ...
    for (const channelConfig of cases) {
      const cfg = channelConfig as unknown as KeanuConfig;
      const actions = discordMessageActions.listActions?.({ cfg }) ?? [];
  ...
  it("maps reaction targets into signal sendReaction calls", async () => {
    const cases = [
      ...
      },
      {
        ...
        cfg: { channels: { signal: { account: "+15550001111" } } } as KeanuConfig,
        accountId: undefined,
        params: {
