...
import { isPostBreatheRecovery } from "../layer-5-self/breathe.ts";
...
export function spring(userMessage: string): SpringReading {
  ...
  if (words > 50 || questionMarks >= 2) {
    complexity = "high";
  } else if (words > 20 || questionMarks >= 1) {
    complexity = "mid";
  }
...
export function summer(
  ...
  const risks: string[] = [];
  if (springReading.complexity === "high") {
    risks.push("high complexity, easy to miss something");
  }
  if (springReading.taskType === "correction") {
    risks.push("drew is correcting, listen before acting");
  }
  if (springReading.taskType === "deployment") {
    risks.push("deployment has real consequences");
  }
