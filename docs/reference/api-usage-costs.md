...
Keanu features that can generate provider usage or paid API calls.
...
- `keanu status --usage` and `keanu channels list` show provider **usage windows**
...
Keanu can pick up credentials from:
...
`keanu models scan` can probe OpenRouter models and uses `OPENROUTER_API_KEY` when
probing is enabled.
