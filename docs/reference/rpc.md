...
Keanu integrates external CLIs via JSON-RPC. Two patterns are used today.
...
- Keanu owns lifecycle when `channels.signal.autoStart=true`.
...
- Keanu spawns `imsg rpc` as a child process (legacy iMessage integration).
