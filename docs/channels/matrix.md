...
Matrix is an open, decentralized messaging protocol. Keanu connects as a Matrix **user**
...
```bash
keanu plugins install @keanu/matrix
```

Local checkout (when running from a git repo):

```bash
keanu plugins install ./extensions/matrix
```
...
1. Install the Matrix plugin:
   - From npm: `keanu plugins install @keanu/matrix`
   - From a local checkout: `keanu plugins install ./extensions/matrix`
...
3. Get an access token for the bot account:
   ...
   - Or set `channels.matrix.userId` + `channels.matrix.password`: Keanu calls the same
     login endpoint, stores the access token in `~/.keanu/credentials/matrix/credentials.json`,
...
- On first connection, Keanu requests device verification from your other sessions.
- Verify the device in another Matrix client (Element, etc.) to enable key sharing.
- If the crypto module cannot be loaded, E2EE is disabled and encrypted rooms will not decrypt;
  Keanu logs a warning.
...
`~/.keanu/matrix/accounts/<account>/<homeserver>__<user>/<token-hash>/crypto/`
...
- Approve via:
  - `keanu pairing list matrix`
  - `keanu pairing approve matrix <CODE>`
...
- On startup, Keanu resolves room/user names in allowlists to IDs and logs the mapping; unresolved entries are ignored for allowlist matching.
