...
Feishu (Lark) is a team chat platform used by companies for messaging and collaboration. This plugin connects Keanu to a Feishu/Lark bot using the platform’s WebSocket event subscription so messages can be received without exposing a public webhook URL.
...
```bash
keanu plugins install @keanu/feishu
```

Local checkout (when running from a git repo):

```bash
keanu plugins install ./extensions/feishu
```
...
If you just installed Keanu, run the wizard:
...
2. Configuring app credentials in Keanu
...
1. You already ran `keanu channels add` for Feishu
2. The gateway is running (`keanu gateway status`)
...
```bash
keanu pairing approve feishu <CODE>
```
...
2. Run `keanu logs --follow` and look for `chat_id`
...
2. Run `keanu logs --follow` and look for `open_id`
...
| Command                 | Description                   |
| ----------------------- | ----------------------------- |
| `keanu gateway status`  | Show gateway status           |
| `keanu gateway install` | Install/start gateway service |
| `keanu gateway stop`    | Stop gateway service          |
| `keanu gateway restart` | Restart gateway service       |
| `keanu logs --follow`   | Tail gateway logs             |
...
4. Check logs: `keanu logs --follow`
...
5. Ensure the gateway is running: `keanu gateway status`
6. Check logs: `keanu logs --follow`
  ...
  agents: {
    list: [
      { id: "main" },
      {
        ...
        agentDir: "/home/user/.keanu/agents/clawd-fan/agent",
      },
      {
        id: "clawd-xi",
        workspace: "/home/user/clawd-xi",
        agentDir: "/home/user/.keanu/agents/clawd-xi/agent",
      },
    ],
  },
  bindings: [
