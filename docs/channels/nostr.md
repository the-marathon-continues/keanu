...
read_when:
  - You want Keanu to receive DMs via Nostr
...
Nostr is a decentralized protocol for social networking. This channel enables Keanu to receive and respond to encrypted direct messages (DMs) via NIP-04.

## Install (on demand)

### Onboarding (recommended)

- The onboarding wizard (`keanu onboard`) and `keanu channels add` list optional channel plugins.
...
```bash
keanu plugins install @keanu/nostr
```

Use a local checkout (dev workflows):

```bash
keanu plugins install --link <path-to-keanu>/extensions/nostr
```
  ...
  "channels": {
    "nostr": {
      ...
      "profile": {
        ...
        "nip05": "keanu@example.com",
        "lud16": "keanu@example.com"
      }
    }
  }
}
