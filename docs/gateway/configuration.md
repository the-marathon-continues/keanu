...
read_when:
  - Setting up Keanu for the first time
...
Keanu reads an optional <Tooltip tip="JSON5 supports comments and trailing commas">**JSON5**</Tooltip> config from `~/.keanu/keanu.json`.

If the file is missing, Keanu uses safe defaults. Common reasons to add a config:
...
<Tip>
**New to configuration?** Start with `keanu onboard` for interactive setup, or check out the [Configuration Examples](/gateway/configuration-examples) guide for complete copy-paste configs.
</Tip>
...
<Tabs>
  <Tab title="Interactive wizard">
    ```bash
    keanu onboard       # full setup wizard
    keanu configure     # config wizard
    ```
  </Tab>
  ...
  <Tab title="Direct edit">
    Edit `~/.keanu/keanu.json` directly. The Gateway watches the file and applies changes automatically (see [hot reload](#config-hot-reload)).
  </Tab>
</Tabs>
...
<Warning>
Keanu only accepts configurations that fully match the schema. Unknown keys, malformed types, or invalid values cause the Gateway to **refuse to start**. The only root-level exception is `$schema` (string), so editors can attach JSON Schema metadata.
</Warning>

When validation fails:

- The Gateway does not boot
- Only diagnostic commands work (`keanu doctor`, `keanu logs`, `keanu health`, `keanu status`)
- Run `keanu doctor` to see exact issues
- Run `keanu doctor --fix` (or `--yes`) to apply repairs
...
The Gateway watches `~/.keanu/keanu.json` and applies changes automatically — no manual restart needed for most settings.
...
<AccordionGroup>
  <Accordion title="config.apply (full replace)">
    ...
    <Warning>
    `config.apply` replaces the **entire config**. Use `config.patch` for partial updates, or `keanu config set` for single keys.
    </Warning>
    ...
    keanu gateway call config.apply --params '{
      "raw": "{ agents: { defaults: { workspace: \"~/.keanu/workspace\" } } }",
      "baseHash": "<hash>",
...
Keanu reads env vars from the parent process plus:

- `.env` from the current working directory (if present)
- `~/.keanu/.env` (global fallback)
...
<Accordion title="Shell env import (optional)">
  If enabled and expected keys aren't set, Keanu runs your login shell and imports only the missing keys:
...
Env var equivalent: `KEANU_LOAD_SHELL_ENV=1`
</Accordion>
...
```json5
{
  gateway: { auth: { token: "${KEANU_GATEWAY_TOKEN}" } },
