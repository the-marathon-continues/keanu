---
summary: "Use MiniMax M2.1 in Keanu"
read_when:
  - You want MiniMax models in Keanu
...
```bash
keanu plugins enable minimax-portal-auth  # skip if already loaded.
keanu gateway restart  # restart if gateway is already running
keanu onboard --auth-choice minimax-portal
```
...
See [MiniMax OAuth plugin README](https://github.com/keanu/keanu/tree/main/extensions/minimax-portal-auth) for details.
...
Configure manually via `keanu.json`:
...
- Use `keanu models list` and `keanu models set minimax/MiniMax-M2.1` to switch.
...
- Running `keanu configure` and selecting **MiniMax M2.1**, or
