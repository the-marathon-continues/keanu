...
Keanu can convert outbound replies into audio using ElevenLabs, OpenAI, or Edge TTS.
It works anywhere Keanu can send audio; Telegram gets a round voice-note bubble.
...
Edge TTS does **not** require an API key. If no API keys are found, Keanu defaults
...
TTS config lives under `messages.tts` in `keanu.json`.
...
- If `provider` is **unset**, Keanu prefers `openai` (if key), then `elevenlabs` (if key),
  otherwise `edge`.
...
`~/.keanu/settings/tts.json`, override with `KEANU_TTS_PREFS` or
...
- **Edge TTS**: uses `edge.outputFormat` (default `audio-24khz-48kbitrate-mono-mp3`).
  ...
  - If the configured Edge output format fails, Keanu retries with MP3.
...
Discord note: `/tts` is a built-in Discord command, so Keanu registers
