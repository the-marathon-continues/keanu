...
Deploy Keanu on Render using Infrastructure as Code. The included `render.yaml` Blueprint defines your entire stack declaratively, service, disk, environment variables, so you can deploy with a single click and version your infrastructure alongside your code.
...
[Deploy to Render](https://render.com/deploy?repo=https://github.com/keanu/keanu)
...
repository configures everything needed to run Keanu:
...
services:
  - type: web
    ...
    envVars:
      ...
      - key: KEANU_STATE_DIR
        value: /data/.keanu
      - key: KEANU_WORKSPACE_DIR
        value: /data/workspace
      - key: KEANU_GATEWAY_TOKEN
...
The web dashboard is available at `https://<your-service>.onrender.com/keanu`.
...
If you use the original Keanu repository, Render will not auto-deploy your Keanu. To update it, run a manual Blueprint sync from the dashboard.
...
For Keanu, vertical scaling is usually sufficient. Horizontal scaling requires sticky sessions or external state management.
...
This downloads a portable backup you can restore on any Keanu host.
