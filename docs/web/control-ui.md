...
- optional prefix: set `gateway.controlUi.basePath` (e.g. `/keanu`)
...
If the page fails to load, start the Gateway first: `keanu gateway`.
...
you revoke it with `keanu devices revoke --device <id> --role <role>`. See
...
- Config: view/edit `~/.keanu/keanu.json` (`config.get`, `config.set`)
...
- Stop:
  ...
  - Type `/stop` (or standalone abort phrases like `stop`, `stop action`, `stop run`, `stop keanu`, `please stop`) to abort out-of-band
...
(`tailscale-user-login`) when `gateway.auth.allowTailscale` is `true`. Keanu
...
Keanu **blocks** Control UI connections without device identity.
