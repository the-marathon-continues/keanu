---
summary: "Agent tool surface for Keanu (browser, canvas, nodes, message, cron) replacing legacy `keanu-*` skills"
read_when:
  - Adding or modifying agent tools
  - Retiring or changing `keanu-*` skills
title: "Tools"
---

# Tools (Keanu)

Keanu exposes **first-class agent tools** for browser, canvas, nodes, and cron.
These replace the old `keanu-*` skills: the tools are typed, no shelling,
and the agent should rely on them directly.

## Disabling tools

You can globally allow/deny tools via `tools.allow` / `tools.deny` in `keanu.json`
...
- If `tools.allow` only references unknown or unloaded plugin tool names, Keanu logs a warning and ignores the allowlist so core tools stay available.
...
- `group:nodes`: `nodes`
- `group:keanu`: all built-in Keanu tools (excludes provider plugins)
...
- `host=node` can target a macOS companion app or a headless node host (`keanu node run`).
...
Keanu tracks recent tool-call history and blocks or warns when it detects repetitive no-progress loops.
...
- Requires a Brave API key (recommended: `keanu configure --section web`, or set `BRAVE_API_KEY`).
...
Control the dedicated Keanu-managed browser.
...
- Quick smoke: `keanu nodes canvas a2ui push --node <id> --text "Hello from A2UI"`.
...
- `restart` (authorizes + sends `SIGUSR1` for in-process restart; `keanu gateway` restart in-place)
...
- `sessions_spawn` starts a sub-agent run and posts an announce reply back to the requester chat.
  ...
  - If `runTimeoutSeconds` is omitted, Keanu uses `agents.defaults.subagents.runTimeoutSeconds` when set; otherwise timeout defaults to `0` (no timeout).
...
- Sandbox clamp: when the current session is sandboxed and `agents.defaults.sandbox.sessionToolsVisibility: "spawned"`, Keanu clamps `tools.sessions.visibility` to `tree`.
