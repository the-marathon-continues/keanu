...
- `gateway`/`node` approvals are controlled by `~/.keanu/exec-approvals.json`.
...
- `host=sandbox`: runs `sh -lc` (login shell) inside the container, so `/etc/profile` may reset `PATH`.
  Keanu prepends `env.PATH` after profile sourcing via an internal env var (no shell interpolation);
...
`keanu security audit` warns when interpreter/runtime `safeBins` entries are missing explicit profiles, and `keanu doctor --fix` can scaffold missing custom `safeBinProfiles` entries.
