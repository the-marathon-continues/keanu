...
For debugging “why is this blocked?”, see [Sandbox vs Tool Policy vs Elevated](/gateway/sandbox-vs-tool-policy-vs-elevated) and `keanu sandbox explain`.
...
Legacy `agent.*` configs are migrated by `keanu doctor`; prefer `agents.defaults` + `agents.list` going forward.
...
2. **Verify sandbox containers:**
   ...
   ```exec
   docker ps --filter "name=keanu-sbx-"
   ```
...
4. **Monitor logs:**
   ...
   ```exec
   tail -f "${KEANU_STATE_DIR:-$HOME/.keanu}/logs/gateway.log" | grep -E "routing|sandbox|tools"
   ```
