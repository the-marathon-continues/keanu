...
- **gateway host** → `keanu` process on the gateway machine
...
```json
{
  "version": 1,
  "socket": {
    "path": "~/.keanu/exec-approvals.sock",
...
- allowlist entries live in host-local `~/.keanu/exec-approvals.json` under `agents.<id>.allowlist` (or via Control UI / `keanu approvals allowlist ...`).
- `keanu security audit` warns with `tools.exec.safe_bins_interpreter_unprofiled` when interpreter/runtime bins appear in `safeBins` without explicit profiles.
- `keanu doctor --fix` can scaffold missing custom `safeBinProfiles.<bin>` entries as `{}` (review and tighten afterward). Interpreter/runtime bins are not auto-scaffolded.
...
`~/.keanu/exec-approvals.json` directly.

CLI: `keanu approvals` supports gateway or node editing (see [Approvals CLI](/cli/approvals)).
