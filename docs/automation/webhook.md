...
read_when:
  ...
  - Wiring external systems into Keanu
title: "Webhooks"
---
...
- `hooks.transformsDir` + `transform.module` loads a JS/TS module for custom logic.
  - `hooks.transformsDir` (if set) must stay within the transforms root under your Keanu config directory (typically `~/.keanu/hooks/transforms`).
...
- `keanu webhooks gmail setup` writes `hooks.gmail` config for `keanu webhooks gmail run`.
