...
function resolveGatewayPort(cfg: KeanuPluginApi["config"]): number {
  const envPort =
    parsePositiveInteger(process.env.KEANU_GATEWAY_PORT?.trim()) ??
    parsePositiveInteger(process.env.KEANU_GATEWAY_PORT?.trim());
  if (envPort) {
    return envPort;
  }
...
function resolveAuth(cfg: KeanuPluginApi["config"]): ResolveAuthResult {
...
async function resolveGatewayUrl(api: KeanuPluginApi): Promise<ResolveUrlResult> {
  ...
  api.registerCommand({
    ...
    handler: async (ctx) => {
      ...
      if (action === "qr") {
        ...
        if (channel === "telegram" && target) {
          try {
            ...
            if (send) {
              await send(
                target,
                ["Scan this QR code with the Keanu iOS app:", "", "```", qrAscii, "```"].join("\n"),
                {
        ...
        return {
          text: [
            "Scan this QR code with the Keanu iOS app:",
            "",
            "```",
            qrAscii,
            "```",
