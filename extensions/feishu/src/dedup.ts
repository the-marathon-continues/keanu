...
import { createDedupeCache, createPersistentDedupe } from "keanu/plugin-sdk";
...
function resolveStateDirFromEnv(env: NodeJS.ProcessEnv = process.env): string {
  const stateOverride = env.KEANU_STATE_DIR?.trim() || env.KEANU_STATE_DIR?.trim();
  if (stateOverride) {
    return stateOverride;
  }
  if (env.VITEST || env.NODE_ENV === "test") {
    return path.join(os.tmpdir(), ["keanu-vitest", String(process.pid)].join("-"));
  }
  return path.join(os.homedir(), ".keanu");
}
