...
import type { KeanuPluginApi, KeanuPluginService } from "keanu/plugin-sdk";
...
function normalizeDenyList(cfg: KeanuPluginApi["config"]): string[] {
  return uniqSorted([...(cfg.gateway?.nodes?.denyCommands ?? [])]);
}

function normalizeAllowList(cfg: KeanuPluginApi["config"]): string[] {
...
export default function register(api: KeanuPluginApi) {
  ...
  const timerService: KeanuPluginService = {
