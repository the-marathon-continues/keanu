...
export function extractMSTeamsPollVote(
  ...
  const pollId =
    readNestedString(value, ["keanuPollId"]) ??
    readNestedString(value, ["pollId"]) ??
    readNestedString(value, ["keanu", "pollId"]) ??
    readNestedString(value, ["keanu", "poll", "id"]) ??
    readNestedString(value, ["data", "keanuPollId"]) ??
    readNestedString(value, ["data", "pollId"]) ??
    readNestedString(value, ["data", "keanu", "pollId"]);
  if (!pollId) {
    return null;
  }
    ...
    actions: [
      {
        ...
        msteams: {
          type: "messageBack",
          text: "keanu poll vote",
          displayText: "Vote recorded",
          value: { keanuPollId: pollId, pollId },
        },
      },
    ],
  };
