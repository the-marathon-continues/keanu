 ...
 */
function setTwitchAccount(cfg: KeanuConfig, account: Partial<TwitchAccountConfig>): KeanuConfig {
...
async function noteTwitchSetupHelp(prompter: WizardPrompter): Promise<void> {
  await prompter.note(
    [
      ...
      "Env vars supported: KEANU_TWITCH_ACCESS_TOKEN",
...
): Promise<{ cfg: KeanuConfig } | null> {
  const useEnv = await prompter.confirm({
    message: "Twitch env var KEANU_TWITCH_ACCESS_TOKEN detected. Use env token?",
    initialValue: true,
  });
  if (!useEnv) {
    return null;
...
export const twitchOnboardingAdapter: ChannelOnboardingAdapter = {
  ...
  configure: async ({ cfg, prompter, forceAllowFrom }) => {
    ...
    const envToken = process.env.KEANU_TWITCH_ACCESS_TOKEN?.trim();
