/**
 * Twitch channel plugin for Keanu.
 *
...
import { buildChannelConfigSchema } from "keanu/plugin-sdk";
 ...
 * for Keanu. Supports message sending, receiving, access control, and
 * status monitoring.
 */
export const twitchPlugin: ChannelPlugin<TwitchAccountConfig> = {
  ...
  config: {
    ...
    listAccountIds: (cfg: KeanuConfig): string[] => listAccountIds(cfg),

    /** Resolve an account config by ID */
    resolveAccount: (cfg: KeanuConfig, accountId?: string | null): TwitchAccountConfig => {
    ...
    isConfigured: (_account: unknown, cfg: KeanuConfig): boolean => {
