...
import { recordBreathe } from "./breathe.js";
import { analyzeChain, type ChainAnalysis, type ChainTrigger } from "./chain.js";
...
import { checkHealth, type HealthReading, type HealthStatus } from "./health.js";
import type { MismatchReading } from "./mismatch.js";
import type { SeasonReading } from "./seasons.js";
import type {
  AliveState,
  PulseReading,
  HumanReading,
...
function determineTrigger(pulse: PulseReading, consecutiveGrey: number): GreyTrigger {
  if (pulse.state === "black") return "black_state";
  if (consecutiveGrey >= 3) return "consecutive_grey";

  // Check bullshit
  const bsScore = pulse.bullshitReadings?.reduce((sum, b) => sum + b.score, 0) ?? 0;
  if (bsScore > 0.5) return "high_bullshit";
...
export function advanceEpisode(ctx: AdvanceContext): AdvanceResult {
  if (!currentEpisode) {
    return {
      episode: null as unknown as UnifiedEpisode,
      shouldBreathe: false,
      completed: false,
      marker: null,
    };
...
export function completeEpisode(): CompleteResult {
  if (!currentEpisode) {
    return { marker: null, episode: null as unknown as UnifiedEpisode };
  }
