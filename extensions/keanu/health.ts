...
export function checkHealth(
  ...
  if (contextAge > 40) score += 2;
  else if (contextAge > 20) score += 1;

  // Bullshit trend climbing
  if (bullshitRate > 0.5) score += 2;
  else if (bullshitRate > 0.3) score += 1;

  // Prompt bloat
  if (avgPromptSize > 80000) score += 2;
  else if (avgPromptSize > 50000) score += 1;

  // Tools breaking
  if (toolErrorRate > 0.3) score += 2;
  else if (toolErrorRate > 0.15) score += 1;

  // Grey streak
  if (consecutiveGrey >= 5) score += 2;
  else if (consecutiveGrey >= 3) score += 1;
