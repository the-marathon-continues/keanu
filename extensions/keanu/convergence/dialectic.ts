...
import { GradientMachine, MachineOutput } from "./firmware.js";
import { Signal, clamp } from "./gradient.js";
import { Duality, DualityGraph, ConvergenceOps } from "./graph.js";
...
export class DialecticalEngine {
  ...
  private measureTension(thesis: string, antithesis: string, synthesis: string): number {
    ...
    let taOverlap = 0;
    for (const w of tWords) if (aWords.has(w)) taOverlap++;

    let tsOverlap = 0;
    for (const w of tWords) if (sWords.has(w)) tsOverlap++;
