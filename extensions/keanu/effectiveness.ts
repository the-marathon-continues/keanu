...
import type { CorrectionCategory, Correction } from "./mastery.js";
...
export function measurePendingInterventions(currentTurn: number): Intervention[] {
  ...
  for (const intervention of interventions) {
    if (intervention.measured) continue;
    if (currentTurn < intervention.windowEnd) continue;
...
export function formatInjection(): string | null {
  ...
  if (report.measuredCount < 3) return null;
