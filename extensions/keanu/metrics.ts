...
import type { BlindSpot, Correction } from "./mastery.js";
import type { TurnSnapshot } from "./state.js";
...
export function computeMetrics(ctx: {
  ...
  for (const s of ctx.snapshots) {
    if (s.pulse === "grey" || s.pulse === "black") {
      ...
      if (currentStreak > 0) greyStreaks.push(currentStreak);
      currentStreak = 0;
    }
  }
  if (currentStreak > 0) greyStreaks.push(currentStreak);
