...
function detectComplexity(
  ...
  if (score >= 4) return { level: "high", signals };
  if (score >= 2) return { level: "mid", signals };
...
function selectModules(msg: string, complexity: "low" | "mid" | "high"): ReasoningModule[] {
  ...
  if (COMPARISON.test(msg) || DECISION.test(msg)) modules.push("tradeoff");

  // Multi-part -> decompose
  if (MULTI_CLAUSE.test(msg) || MULTI_TOPIC.test(msg)) modules.push("decompose");
  ...
  if (/first|then|after|before|order|steps/i.test(lower)) modules.push("sequence");
  ...
  if (/team|user|customer|drew|people|affect|impact/i.test(lower)) modules.push("stakeholder");

  // "Like" or "similar" -> analogize
  if (/like|similar|same as|reminds me|pattern/i.test(lower)) modules.push("analogize");
  ...
  if (complexity === "high" && unique.length < 2) {
    if (!unique.includes("decompose")) unique.push("decompose");
    if (!unique.includes("tradeoff")) unique.push("tradeoff");
  }
