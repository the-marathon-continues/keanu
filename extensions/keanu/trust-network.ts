...
export function importFromParent(data: {
  ...
  if (data.selfId) _network.selfId = data.selfId;
  if (data.selfName) _network.selfName = data.selfName;
  if (data.peers) {
    _network.peers = new Map(Object.entries(data.peers));
  }
  if (data.lastUpdated) _network.lastUpdated = data.lastUpdated;
}
