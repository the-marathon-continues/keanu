...
import { detectCarnegie, assessCarnegieDelta } from "../carnegie.js";
...
export async function runChallenge(
  ...
  // KEANU readings (when enabled)
  let keanuReadings: HarnessResult["keanuReadings"];
  if (config.mode === "keanu") {
    const pulseReading = checkPulse(response, 1, false);
    const bullshitReadings = detectBullshit(response);
    const carnegieReading = detectCarnegie(challenge.prompt, []);
    const carnegieDelta = assessCarnegieDelta(response, carnegieReading);
    const helixResult = helix.analyze(response);
...
export async function runComparison(
  ...
  if (challenge.type === "keanu") {
    ...
    if (challenge.expectedPulse && withKeanu.keanuReadings?.pulse) {
      keanuDelta = withKeanu.keanuReadings.pulse.state === challenge.expectedPulse ? 1 : 0;
    }
    if (challenge.expectedBullshitTypes && withKeanu.keanuReadings?.bullshit) {
      const detected = new Set(withKeanu.keanuReadings.bullshit.map((b) => b.type));
      const expected = new Set(challenge.expectedBullshitTypes);
