...
- Publish as `keanu/plugin-sdk` (or export from core under `keanu/plugin-sdk`).
- Semver with explicit stability guarantees.

### 2) Plugin Runtime (execution surface, injected)

Scope: everything that touches core runtime behavior.
Accessed via `KeanuPluginApi.runtime` so plugins never import `src/**`.
...
export type PluginRuntime = {
  channel: {
    text: {
      ...
      resolveTextChunkLimit(cfg: KeanuConfig, channel: string, accountId?: string): number;
      hasControlCommand(text: string, cfg: KeanuConfig): boolean;
    };
    reply: {
    ...
    mentions: {
      buildMentionRegexes(cfg: KeanuConfig, agentId?: string): RegExp[];
    ...
    debounce: {
      ...
      resolveInboundDebounceMs(cfg: KeanuConfig, channel: string): number;
    };
    commands: {
  ...
  state: {
    resolveStateDir(cfg: KeanuConfig): string;
  };
};
```
...
- Introduce `keanu/plugin-sdk`.
- Add `api.runtime` to `KeanuPluginApi` with the surface above.
...
- Plugins declare a required runtime range (e.g., `keanuRuntime: ">=2026.2.0"`).
