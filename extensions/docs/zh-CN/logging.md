...
所有日志配置都在 `~/.keanu/keanu.json` 的 `logging` 下。
...
- Keanu 目前通过 **OTLP/HTTP（protobuf）** 导出。
...
- 你也可以使用 `keanu plugins enable diagnostics-otel` 启用插件。
...
- `keanu.tokens`（计数器，属性：`keanu.token`、`keanu.channel`、`keanu.provider`、`keanu.model`）
- `keanu.cost.usd`（计数器，属性：`keanu.channel`、`keanu.provider`、`keanu.model`）
- `keanu.run.duration_ms`（直方图，属性：`keanu.channel`、`keanu.provider`、`keanu.model`）
- `keanu.context.tokens`（直方图，属性：`keanu.context`、`keanu.channel`、`keanu.provider`、`keanu.model`）

消息流：

- `keanu.webhook.received`（计数器，属性：`keanu.channel`、`keanu.webhook`）
- `keanu.webhook.error`（计数器，属性：`keanu.channel`、`keanu.webhook`）
- `keanu.webhook.duration_ms`（直方图，属性：`keanu.channel`、`keanu.webhook`）
- `keanu.message.queued`（计数器，属性：`keanu.channel`、`keanu.source`）
- `keanu.message.processed`（计数器，属性：`keanu.channel`、`keanu.outcome`）
- `keanu.message.duration_ms`（直方图，属性：`keanu.channel`、`keanu.outcome`）

队列 + 会话：

- `keanu.queue.lane.enqueue`（计数器，属性：`keanu.lane`）
- `keanu.queue.lane.dequeue`（计数器，属性：`keanu.lane`）
- `keanu.queue.depth`（直方图，属性：`keanu.lane` 或 `keanu.channel=heartbeat`）
- `keanu.queue.wait_ms`（直方图，属性：`keanu.lane`）
- `keanu.session.state`（计数器，属性：`keanu.state`、`keanu.reason`）
- `keanu.session.stuck`（计数器，属性：`keanu.state`）
- `keanu.session.stuck_age_ms`（直方图，属性：`keanu.state`）
- `keanu.run.attempt`（计数器，属性：`keanu.attempt`）
...
- `keanu.webhook.error`
  - `keanu.channel`、`keanu.webhook`、`keanu.chatId`、`keanu.error`
- `keanu.message.processed`
  - `keanu.channel`、`keanu.outcome`、`keanu.chatId`、`keanu.messageId`、`keanu.sessionKey`、`keanu.sessionId`、`keanu.reason`
- `keanu.session.stuck`
  - `keanu.state`、`keanu.ageMs`、`keanu.queueDepth`、`keanu.sessionKey`、`keanu.sessionId`
...
- **Gateway 网关无法访问？** 先运行 `keanu doctor`。
