...
- With Anthropic API-key auth profiles, Keanu seeds `cacheRetention: "short"` for Anthropic model refs when unset.
...
For `openrouter/anthropic/*` model refs, Keanu injects Anthropic `cache_control` on system/developer prompt blocks to improve prompt-cache reuse.
...
Keanu exposes dedicated cache-trace diagnostics for embedded agent runs.
...
diagnostics:
  cacheTrace:
    enabled: true
    filePath: "~/.keanu/logs/cache-trace.jsonl" # optional
...
- `filePath`: `$KEANU_STATE_DIR/logs/cache-trace.jsonl`
...
- `KEANU_CACHE_TRACE=1` enables cache tracing.
- `KEANU_CACHE_TRACE_FILE=/path/to/cache-trace.jsonl` overrides output path.
- `KEANU_CACHE_TRACE_MESSAGES=0|1` toggles full message payload capture.
- `KEANU_CACHE_TRACE_PROMPT=0|1` toggles prompt text capture.
- `KEANU_CACHE_TRACE_SYSTEM=0|1` toggles system prompt capture.
