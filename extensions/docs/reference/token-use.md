---
summary: "How Keanu builds prompt context and reports token usage + costs"
read_when:
...
Keanu tracks **tokens**, not characters. Tokens are model-specific, but most
OpenAI-style models average ~4 characters per token for English text.

## How the system prompt is built

Keanu assembles its own system prompt on every run. It includes:
...
For images, Keanu downscales transcript/tool image payloads before provider calls.
...
- `/usage cost` → shows a local cost summary from Keanu session logs.

Other surfaces:

- **TUI/Web TUI:** `/status` + `/usage` are supported.
- **CLI:** `keanu status --usage` and `keanu channels list` show
...
`cacheWrite`. If pricing is missing, Keanu shows tokens only. OAuth tokens
never show dollar cost.

## Cache TTL and pruning impact

Provider prompt caching only applies within the cache TTL window. Keanu can
...
Anthropic's 1M context window is currently beta-gated. Keanu can inject the
...
Keanu skips the `context-1m-*` beta header because Anthropic currently
