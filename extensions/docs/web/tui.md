...
```bash
keanu tui --url ws://<host>:<port> --token <gateway-token>
```
...
- Turn delivery on:
  ...
  - or start with `keanu tui --deliver`
...
- Check the Gateway logs: `keanu logs --follow`.
- Confirm the agent can run: `keanu status` and `keanu models status`.
...
- No agents in picker: check `keanu agents list` and your routing config.
