...
- `/usage cost` in chats: local cost summary aggregated from Keanu session logs.
- CLI: `keanu status --usage` prints a full per-provider breakdown.
- CLI: `keanu channels list` prints the same usage snapshot alongside provider config (use `--no-usage` to skip).
