...
- Jobs persist under `~/.keanu/cron/` so restarts don’t lose schedules.
...
keanu cron list
keanu cron run <job-id>
keanu cron runs --id <job-id>
```
...
Cron jobs are persisted on the Gateway host at `~/.keanu/cron/jobs.json` by default.
The Gateway loads the file into memory and writes it back on changes, so manual edits
are only safe when the Gateway is stopped. Prefer `keanu cron add/edit` or the cron
...
To reduce top-of-hour load spikes across many gateways, Keanu applies a
...
If `delivery` is omitted for isolated jobs, Keanu defaults to `announce`.
...
- Job store: `~/.keanu/cron/jobs.json` (Gateway-managed JSON).
- Run history: `~/.keanu/cron/runs/<jobId>.jsonl` (JSONL, auto-pruned by size and line count).
...
- For removed run sessions no longer referenced by the session store, Keanu archives transcript files and purges old deleted archives on the same retention window.
...
- review growth periodically with `keanu cron runs` and adjust retention before logs become large
...
keanu cron add --name "Ops sweep" --cron "0 6 * * *" --session isolated --message "Check ops queue" --agent ops

# Switch or clear the agent on an existing job
keanu cron edit <jobId> --agent ops
keanu cron edit <jobId> --clear-agent
```

Manual run (force is the default, use `--due` to only run when due):

```bash
keanu cron run <jobId>
keanu cron run <jobId> --due
```

Edit an existing job (patch fields):

```bash
keanu cron edit <jobId> \
...
```bash
keanu system event --mode now --text "Next heartbeat: check battery."
```
...
- `cron.run` (force or due), `cron.runs`
  For immediate system events without a job, use [`keanu system event`](/cli/system).

## Troubleshooting

### “Nothing runs”

- Check cron is enabled: `cron.enabled` and `KEANU_SKIP_CRON`.
- Check the Gateway is running continuously (cron runs inside the Gateway process).
- For `cron` schedules: confirm timezone (`--tz`) vs the host timezone.

### A recurring job keeps delaying after failures

- Keanu applies exponential retry backoff for recurring jobs after consecutive errors:
