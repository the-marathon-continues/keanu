---
summary: "Use Venice AI privacy-focused models in Keanu"
read_when:
  - You want privacy-focused inference in Keanu
...
```bash
keanu onboard --auth-choice venice-api-key
```
...
```bash
keanu agent --model venice/llama-3.3-70b --message "Hello, are you working?"
```

## Model Selection

After setup, Keanu shows all available Venice models. Pick based on your needs:
...
```bash
keanu models set venice/claude-opus-45
keanu models set venice/llama-3.3-70b
```

List all available models:

```bash
keanu models list | grep venice
```

## Configure via `keanu configure`

1. Run `keanu configure`
2. Select **Model/auth**
3. Choose **Venice AI**
...
Keanu automatically discovers models from the Venice API when `VENICE_API_KEY` is set. If the API is unreachable, it falls back to a static catalog.
...
keanu agent --model venice/llama-3.3-70b --message "Quick health check"

# Use Claude via Venice (anonymized)
keanu agent --model venice/claude-opus-45 --message "Summarize this task"

# Use uncensored model
keanu agent --model venice/venice-uncensored --message "Draft options"

# Use vision model with image
keanu agent --model venice/qwen3-vl-235b-a22b --message "Review attached image"

# Use coding model
keanu agent --model venice/qwen3-coder-480b-a35b-instruct --message "Refactor this function"
```
...
The Venice model catalog updates dynamically. Run `keanu models list` to see currently available models. Some models may be temporarily offline.
