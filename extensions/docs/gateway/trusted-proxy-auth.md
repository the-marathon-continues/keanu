...
read_when:
  - Running Keanu behind an identity-aware proxy
  - Setting up Pomerium, Caddy, or nginx with OAuth in front of Keanu
...
- You run Keanu behind an **identity-aware proxy** (Pomerium, Caddy + OAuth, nginx + oauth2-proxy, Traefik + forward auth)
...
3. Keanu checks that the request came from a **trusted proxy IP** (configured in `gateway.trustedProxies`)
4. Keanu extracts the user identity from the configured header
...
- Keanu can stay on loopback HTTP behind the proxy.
...
If Keanu itself serves HTTPS directly (no TLS-terminating proxy), set:
...
```yaml
routes:
  - from: https://keanu.example.com
    to: http://keanu-gateway:18789
    policy:
      - allow:
          or:
            - email:
...
`keanu security audit` will flag trusted-proxy auth with a **critical** severity finding. This is intentional — it's a reminder that you're delegating security to your proxy setup.
...
3. Update Keanu config with trusted-proxy auth
4. Restart the Gateway
5. Test WebSocket connections from the Control UI
6. Run `keanu security audit` and review findings
