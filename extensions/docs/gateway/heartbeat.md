...
- During heartbeat runs, Keanu treats `HEARTBEAT_OK` as an ack when it appears
...
- `session`: optional session key for heartbeat runs.
  ...
  - Explicit session key (copy from `keanu sessions --json` or the [sessions CLI](/cli/sessions)).
...
If **all three** are false, Keanu skips the heartbeat run entirely (no model call).
...
```bash
keanu system event --text "Check for urgent follow-ups" --mode now
```
