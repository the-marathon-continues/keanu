...
description: "Complete field-by-field reference for ~/.keanu/keanu.json"
summary: "Complete reference for every Keanu config key, defaults, and channel settings"
read_when:
...
Every field available in `~/.keanu/keanu.json`. For a task-oriented overview, see [Configuration](/gateway/configuration).

Config format is **JSON5** (comments + trailing commas allowed). All fields are optional — Keanu uses safe defaults when omitted.
...
- Legacy single-account Baileys auth dir is migrated by `keanu doctor` into `whatsapp/default`.
...
```json5
{
  channels: {
    discord: {
      ...
      dm: { enabled: true, groupEnabled: false, groupChannels: ["keanu-dm"] },
      guilds: {
        "123456789012345678": {
...
- Keanu additionally attempts voice receive recovery by leaving/rejoining a voice session after repeated decrypt failures.
...
Mattermost ships as a plugin: `keanu plugins install @keanu/mattermost`.
...
Keanu spawns `imsg rpc` (JSON-RPC over stdio). No daemon or port required.
...
```json5
{
  ...
  agents: {
    list: [
      {
        id: "main",
        groupChat: { mentionPatterns: ["reisponde", "@keanu"] },
      },
    ],
  },
}
...
- `config: true` enables `/config` (reads/writes `keanu.json`).
...
Optional repository root shown in the system prompt's Runtime line. If unset, Keanu auto-detects by walking upward from the workspace.
...
- `model.primary`: format `provider/model` (e.g. `anthropic/claude-opus-4-6`). If you omit the provider, Keanu assumes `anthropic` (deprecated).
...
- `none`: per-scope sandbox workspace under `~/.keanu/sandboxes`
...
noVNC observer access uses VNC auth by default and Keanu emits a short-lived token URL (instead of exposing the password in the shared URL).

- `allowHostControl: false` (default) blocks sandboxed sessions from targeting the host browser.
- `network` defaults to `keanu-sandbox-browser` (dedicated bridge network). Set to `bridge` only when you explicitly want global bridge connectivity.
    ...
    list: [
      {
        ...
        default: true,
        name: "Main Agent",
        workspace: "~/.keanu/workspace",
        agentDir: "~/.keanu/agents/main/agent",
        ...
        groupChat: { mentionPatterns: ["@keanu"] },
        sandbox: { mode: "off" },
...
| `group:nodes`      | `nodes`                                                                                  |
| `group:keanu`      | All built-in tools (excludes provider plugins)                                           |
...
Keanu uses the pi-coding-agent model catalog. Add custom providers via `models.providers` in config or `~/.keanu/agents/<agentId>/agent/models.json`.
...
- Override agent config root with `KEANU_AGENT_DIR` (or `PI_CODING_AGENT_DIR`).
...
Set `OPENCODE_API_KEY` (or `OPENCODE_ZEN_API_KEY`). Shortcut: `keanu onboard --auth-choice opencode-zen`.
...
Set `ZAI_API_KEY`. `z.ai/*` and `z-ai/*` are accepted aliases. Shortcut: `keanu onboard --auth-choice zai-api-key`.
...
For the China endpoint: `baseUrl: "https://api.moonshot.cn/v1"` or `keanu onboard --auth-choice moonshot-api-key-cn`.
...
Anthropic-compatible, built-in provider. Shortcut: `keanu onboard --auth-choice kimi-code-api-key`.
...
Base URL should omit `/v1` (Anthropic client appends it). Shortcut: `keanu onboard --auth-choice synthetic-api-key`.
...
Set `MINIMAX_API_KEY`. Shortcut: `keanu onboard --auth-choice minimax-api`.
...
- Loaded from `~/.keanu/extensions`, `<workspace>/.keanu/extensions`, plus `plugins.load.paths`.
...
```json5
{
  gateway: {
    ...
    auth: {
      ...
      token: "your-token",
      // password: "your-password", // or KEANU_GATEWAY_PASSWORD
...
- `port`: single multiplexed port for WS + HTTP. Precedence: `--port` > `KEANU_GATEWAY_PORT` > `gateway.port` > `18789`.
...
Convenience flags: `--dev` (uses `~/.keanu-dev` + port `19001`), `--profile <name>` (uses `~/.keanu-<name>`).
...
```json5
{
  hooks: {
    ...
    presets: ["gmail"],
    transformsDir: "~/.keanu/hooks/transforms",
    mappings: [
      {
        match: { path: "gmail" },
        action: "agent",
...
Auth: `Authorization: Bearer <token>` or `x-keanu-token: <token>`.
...
- Gateway auto-starts `gog gmail watch serve` on boot when configured. Set `KEANU_SKIP_GMAIL_WATCHER=1` to disable.
...
```json5
{
  canvasHost: {
    ...
    liveReload: true,
    // enabled: false, // or KEANU_SKIP_CANVAS_HOST=1
  },
}
```

- Serves agent-editable HTML/CSS/JS and A2UI over HTTP under the Gateway port:
  - `http://<gateway-host>:<gateway.port>/__keanu__/canvas/`
  - `http://<gateway-host>:<gateway.port>/__keanu__/a2ui/`
...
- Also serves A2UI at `/__keanu__/a2ui/`.
...
- Hostname defaults to `keanu`. Override with `KEANU_MDNS_HOSTNAME`.
...
Writes a unicast DNS-SD zone under `~/.keanu/dns/`. For cross-network discovery, pair with a DNS server (CoreDNS recommended) + Tailscale split DNS.

Setup: `keanu dns setup --apply`.
...
- `.env` files: CWD `.env` + `~/.keanu/.env` (neither overrides existing vars).
...
{
  gateway: {
    auth: { token: "${KEANU_GATEWAY_TOKEN}" },
  },
}
```
...
- Legacy OAuth imports from `~/.keanu/credentials/oauth.json`.
...
- Default log file: `/tmp/keanu/keanu-YYYY-MM-DD.log`.
...
Current builds no longer include the TCP bridge. Nodes connect over the Gateway WebSocket. `bridge.*` keys are no longer part of the config schema (validation fails until removed; `keanu doctor --fix` can strip unknown keys).
