...
Keanu ships two lightweight web tools:
...
If no `provider` is explicitly set, Keanu auto-detects which provider to use based on available API keys, checking in this order:
...
3. Run `keanu configure --section web` to store the key in config (recommended), or set `BRAVE_API_KEY` in your environment.
...
**Recommended:** run `keanu configure --section web`. It stores the key in
`~/.keanu/keanu.json` under `tools.web.search.apiKey`.

**Environment alternative:** set `BRAVE_API_KEY` in the Gateway process
environment. For a gateway install, put it in `~/.keanu/.env` (or your
service environment). See [Env vars](/help/faq#how-does-keanu-load-environment-variables).
...
environment. For a gateway install, put it in `~/.keanu/.env`.

If no base URL is set, Keanu chooses a default based on the API key source:
...
For a gateway install, put it in `~/.keanu/.env`.
