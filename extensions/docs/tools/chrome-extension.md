---
summary: "Chrome extension: let Keanu drive your existing Chrome tab"
read_when:
...
The Keanu Chrome extension lets the agent control your **existing Chrome tabs** (your normal Chrome window) instead of launching a separate keanu-managed Chrome profile.
...
Keanu then controls the attached tab through the normal `browser` tool surface (selecting the right profile).
...
The extension ships inside the Keanu release (npm package) as static files. There is no separate “build” step.

After upgrading Keanu:

- Re-run `keanu browser extension install` to refresh the installed files under your Keanu state directory.
- Chrome → `chrome://extensions` → click “Reload” on the extension.

## Use it (set gateway token once)

Keanu ships with a built-in browser profile named `chrome` that targets the extension relay on the default port.

Before first attach, open extension Options and set:

- `Port` (default `18792`)
- `Gateway token` (must match `gateway.auth.token` / `KEANU_GATEWAY_TOKEN`)
...
- Open the tab you want Keanu to control.
...
- `ON`: attached; Keanu can drive that tab.
...
Debugging: `keanu sandbox explain`
...
`keanu browser extension path` prints the **installed** on-disk directory containing the extension files.

The CLI intentionally does **not** print a `node_modules` path. Always run `keanu browser extension install` first to copy the extension to a stable location under your Keanu state directory.
...
- **This is not isolated** like the dedicated keanu-managed profile.
