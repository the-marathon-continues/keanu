...
read_when:
  - Designing workspace memory (~/.keanu/workspace) beyond daily Markdown logs
  - Deciding: standalone CLI vs deep Keanu integration
...
Target: Clawd-style workspace (`agents.defaults.workspace`, default `~/.keanu/workspace`) where “memory” is stored as one Markdown file per day (`memory/YYYY-MM-DD.md`) plus a small set of stable files (e.g. `memory.md`, `SOUL.md`).
...
Keep `~/.keanu/workspace` as canonical human-readable memory.
...
```
~/.keanu/workspace/.memory/index.sqlite
```
...
Recommendation: **deep integration in Keanu**, but keep a separable core library.

### Why integrate into Keanu?
...
- You want the agent itself to call the tools:
  - `keanu memory recall "…" --k 25 --since 30d`
  - `keanu memory reflect --since 7d`
...
Pragmatic take for `~/.keanu/workspace`:
