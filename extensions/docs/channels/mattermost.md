---
summary: "Mattermost bot setup and Keanu config"
read_when:
...
```bash
keanu plugins install @keanu/mattermost
```

Local checkout (when running from a git repo):

```bash
keanu plugins install ./extensions/mattermost
```
...
4. Configure Keanu and start the gateway.
...
- Approve via:
  - `keanu pairing list mattermost`
  - `keanu pairing approve mattermost <CODE>`
...
Use these target formats with `keanu message send` or cron/webhooks:
