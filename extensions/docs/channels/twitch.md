...
read_when:
  - Setting up Twitch chat integration for Keanu
title: "Twitch"
---

# Twitch (plugin)

Twitch chat support via IRC connection. Keanu connects as a Twitch user (bot account) to receive and send messages in channels.
...
```bash
keanu plugins install @keanu/twitch
```

Local checkout (when running from a git repo):

```bash
keanu plugins install ./extensions/twitch
```
...
4. Configure the token:
   - Env: `KEANU_TWITCH_ACCESS_TOKEN=...` (default account only)
...
```json5
{
  channels: {
    twitch: {
      enabled: true,
      username: "keanu", // Bot's Twitch account
      accessToken: "oauth:abc123...", // OAuth Access Token (or use KEANU_TWITCH_ACCESS_TOKEN env var)
...
```bash
KEANU_TWITCH_ACCESS_TOKEN=oauth:abc123...
```
