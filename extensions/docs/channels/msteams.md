...
Keanu will offer the local install path automatically.
...
3. Configure Keanu with those credentials.
...
- Talk to Keanu via Teams DMs, group chats, or channels.
...
- On startup, Keanu resolves team/channel and user allowlist names to IDs (when Graph permissions allow)
...
5. Configure `msteams` in `~/.keanu/keanu.json` (or env vars) and start the gateway.
6. The gateway listens for Bot Framework webhook traffic on `/api/messages` by default.

## Azure Bot Setup (Prerequisites)

Before configuring Keanu, you need to create an Azure Bot resource.
...
2. Fill in the **Basics** tab:
   ...
   | Field              | Value                                                 |
   | ------------------ | ----------------------------------------------------- |
   | **Bot handle**     | Your bot name, e.g., `keanu-msteams` (must be unique) |
...
1. **Install the Microsoft Teams plugin**
   - From npm: `keanu plugins install @keanu/msteams`
   - From a local checkout: `keanu plugins install ./extensions/msteams`
...
Keanu handles this by returning quickly and sending replies proactively, but very slow responses may still cause issues.
...
By default, Keanu only downloads media from Microsoft/Teams hostnames. Override with `channels.msteams.mediaAllowHosts` (use `["*"]` to allow any host).
...
Uploaded files are stored in a `/KeanuShared/` folder in the configured SharePoint site's default document library.

## Polls (Adaptive Cards)

Keanu sends Teams polls as Adaptive Cards (there is no native Teams poll API).

- CLI: `keanu message poll --channel msteams --target conversation:<id> ...`
- Votes are recorded by the gateway in `~/.keanu/msteams-polls.json`.
