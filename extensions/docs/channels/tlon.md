...
Tlon is a decentralized messenger built on Urbit. Keanu connects to your Urbit ship and can
...
```bash
keanu plugins install @keanu/tlon
```

Local checkout (when running from a git repo):

```bash
keanu plugins install ./extensions/tlon
```
...
By default, Keanu blocks private/internal hostnames and IP ranges for this plugin (SSRF hardening).
...
Use these with `keanu message send` or cron delivery:
...
- Thread replies: if the inbound message is in a thread, Keanu replies in-thread.
