...
```bash
keanu plugins install @keanu/nextcloud-talk
```

Local checkout (when running from a git repo):

```bash
keanu plugins install ./extensions/nextcloud-talk
```
...
2. On your Nextcloud server, create a bot:
   ...
   ```bash
   ./occ talk:bot:install "Keanu" "<shared-secret>" "<webhook-url>" --feature reaction
   ```
...
- Approve via:
  - `keanu pairing list nextcloud-talk`
  - `keanu pairing approve nextcloud-talk <CODE>`
