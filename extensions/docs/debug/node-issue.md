...
read_when:
  ...
  - Investigating tsx/esbuild loader crashes in Keanu
...
Running Keanu via Node with `tsx` fails at startup with:

```
[keanu] Failed to start CLI: TypeError: __name is not a function
...
- Before that (Bun path), `keanu status` and `gateway:watch` worked.
...
- Use Node + tsc watch, then run compiled output:
  ...
  node --watch keanu.mjs status
  ```

- Confirmed locally: `pnpm exec tsc -p tsconfig.json` + `node keanu.mjs status` works on Node 25.
