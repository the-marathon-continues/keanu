...
function resolveKeanuRoot(): string {
  if (coreRootCache) {
    return coreRootCache;
  }
  const override = process.env.KEANU_ROOT?.trim();
  if (override) {
  ...
  throw new Error("Unable to resolve core root. Set KEANU_ROOT to the package root.");
}
...
async function importCoreExtensionAPI(): Promise<{
  ...
  const distPath = path.join(resolveKeanuRoot(), "dist", "extensionAPI.js");
