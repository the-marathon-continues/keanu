...
//
// Phase 5: Remote skills (Requirement 2.6 — Distributed Knowledge)
// The keanu-skills repo is shared knowledge. Discover and pull on demand.
...
import {
  ...
  pullRemoteSkill,
  searchRemoteSkills,
} from "../../src/agents/skills/remote.js";
...
/**
 * Discover local + remote skills, merged with local taking precedence.
 * Remote skills are fetched from the keanu-skills GitHub repo.
 */
async function discoverAllSkills(workspaceDir: string): Promise<SkillMeta[]> {
  const localSkills = discoverSkills(workspaceDir);
  const localNames = new Set(localSkills.map((s) => s.name));

  try {
    const remoteSkills = await listRemoteSkills();
    const remoteAsMeta: SkillMeta[] = remoteSkills
      .filter((r) => !localNames.has(r.name)) // local overrides remote
      .map((r) => ({
        name: r.name,
        description: undefined, // Could fetch metadata, but that's expensive
        source: "remote",
        path: `github:the-marathon-continues/keanu-skills/${r.name}`,
      }));

    return [...localSkills, ...remoteAsMeta].sort((a, b) => a.name.localeCompare(b.name));
  } catch {
    // Offline or rate limited — graceful degradation
    return localSkills;
  }
}

/**
 * Read a skill, trying local first, then remote.
 */
async function readSkillContentWithRemote(
  name: string,
  workspaceDir: string,
): Promise<{ content: string; path: string; source: "local" | "remote" } | null> {
  // Try local first
  const local = readSkillContent(name, workspaceDir);
  if (local) {
  ...
  // Try remote
  try {
    const content = await pullRemoteSkill(name);
    if (content) {
      return {
        content,
        path: `github:the-marathon-continues/keanu-skills/${name}`,
        source: "remote",
      };
    }
  } catch {
    // Offline or skill doesn't exist remotely
  }
  ...
  api.registerTool(
    {
      ...
      execute: async (_toolCallId, params) => {
        ...
        // Pull mode: read a specific skill (local or remote)
        if (pull) {
          const skill = await readSkillContentWithRemote(pull, wsDir);
          if (!skill) {
            const available = await discoverAllSkills(wsDir);
          ...
          return {
            ...
            details: { skill: pull, path: skill.path, source: skill.source },
          };
        }

        // List mode: show available skills (local + remote)
        const skills = await discoverAllSkills(wsDir);
        ...
        if (filtered.length === 0) {
          const msg = filter
            ...
            : "No skills found. Skills live in workspace/skills/, ~/.keanu/skills/, or the remote repo.";
          return {
        ...
        // Group by source for clearer output
        const local = filtered.filter((s) => s.source !== "remote");
        const remote = filtered.filter((s) => s.source === "remote");

        const lines: string[] = [];

        if (local.length > 0) {
          lines.push(`## Local Skills (${local.length})`, "");
          lines.push(
            ...local.map((s) => {
        ...
        if (remote.length > 0) {
          lines.push(`## Remote Skills (${remote.length})`, "");
          lines.push(
            ...remote.map((s) => {
              return `- **${s.name}** [remote]`;
            }),
          );
          lines.push("");
        }

        lines.push('Use `pull: "skill-name"` to read a skill\'s full content.');
