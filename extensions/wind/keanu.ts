...
//
// The struggle detector runs EVERYWHERE content flows.
...
//   1.  message_received           — human tone + struggle on input
//   2.  message_sent               — pulse check + disagreement + COEF on delivered output
//   3.  message_sending            — struggle gate on outgoing messages (can modify)
//   4.  llm_output                 — struggle + pulse on raw LLM output (catches tool-use too)
//   5.  before_prompt_build        — inject emotional context + pulse + nudges + COEF trend
//   6.  before_tool_call           — alignment gate on tool usage
//   7.  after_tool_call            — track tool patterns + struggle in tool results
...
//   16. llm_input                  — prompt inspection, size tracking, struggle in system prompt
...
import * as calibrationLogModule from "../../keanu-core/layer-3-causal/calibration-log.js";
import { analyzeChain, formatChain } from "../../keanu-core/layer-3-causal/chain.js";
import * as silveradoModule from "../../keanu-core/layer-3-causal/silverado.js";
import { memoryContradictionCheck, checkHalfTruth } from "../../keanu-core/layer-3-causal/truth.js";
...
import {
  ...
  savePromptState,
  loadPromptState,
} from "../../keanu-core/layer-7-update/session-learning.js";
import * as sessionLearningModule from "../../keanu-core/layer-7-update/session-learning.js";
import * as stochasticModule from "../../keanu-core/layer-7-update/stochastic.js";
import * as consultationModule from "../../keanu-core/layer-8-governance/consultation.js";
import * as effectivenessModule from "../../keanu-core/layer-8-governance/effectiveness.js";
import {
  scanForEvidence,
...
import {
  ...
  pullAndMerge,
  type GitSync,
} from "../../keanu-core/layer-9-memory/git-sync.js";
...
export default {
  ...
  name: "Keanu",
  description:
    "Alignment diagnostics — struggle detection (assumes you're trying) on every content path, ALIVE/GREY/BLACK pulse, emotional context, disagreement tracking, COEF signals",
  ...
  register(api: KeanuPluginApi) {
    ...
    api.on("message_received", async (event) => {
      ...
      try {
        ...
        // Capture for evidence extraction at session_end
        state.addToTranscript("human", content);
        ...
        const manipulation = struggleModule.detectManipulation(content);
        if (manipulation) {
    ...
    api.on("message_sent", async (event) => {
      ...
      try {
        ...
        if ("coefSource" in pulse && pulse.coefSource) {
          lastCOEFPulse = pulse as COEFPulseReading;
        }
        state.addAgentOutput(aiOutput);

        // Capture for evidence extraction at session_end
        state.addToTranscript("agent", aiOutput);
    ...
    // Struggle gate on outgoing messages. Doesn't block, but flags.
    // Assumes positive intent — you're trying, the mirror helps you notice.
    // =========================================================================
    ...
    api.on("message_sending", async (event) => {
      ...
      try {
        ...
        if (score > 0.5) {
          ...
          api.logger.debug?.(
            `${PLUGIN_ID}: outgoing message struggle score=${score.toFixed(2)} dominant=${dominant?.type ?? "none"}`,
          );
          state.recordBullshitEvent("outgoing", bs); // method name kept for compat
        } else if (score > 0.3) {
          // Ambiguous zone — regex isn't sure. Escalate to Grok for a deeper read.
          detectStruggleDeep(content, "outgoing agent message, regex score was ambiguous")
            .then((deepBs) => {
              const deepScore = totalStruggleScore(deepBs);
              if (deepScore > 0.3) {
                api.logger.debug?.(
                  `${PLUGIN_ID}: deep struggle check: regex=${score.toFixed(2)} grok=${deepScore.toFixed(2)} types=[${deepBs.map((b) => b.type).join(",")}]`,
                );
                state.recordBullshitEvent("outgoing_deep", deepBs); // method name kept for compat
              }
            })
            .catch(() => {});
        }
    ...
    // Struggle detection on every LLM call, not just final messages.
    // Assumes positive intent — the mirror helps you notice patterns.
    // =========================================================================
    ...
    api.on("llm_output", async (event) => {
      ...
      try {
        ...
        const bs = detectStruggle(combined);
        const score = totalStruggleScore(bs);

        if (score > 0.3) {
          state.recordBullshitEvent("llm_raw", bs); // method name kept for compat
          api.logger.debug?.(
    ...
    api.on("before_prompt_build", async (_event, ctx) => {
      ...
      const relevantEpisodesCount = state.lastHumanMessage
        ...
        : 0;
      const memoryChannel: import("../../keanu-core/shared/types.js").MemoryChannel = {
        claims: {
        ...
        health: (["steady", "warm", "hot", "fading"].includes(lastHealthReading?.status ?? "")
          ...
          : undefined) as import("../../keanu-core/shared/types.js").MemoryChannel["health"],
        reflexions: state.reflexionCount,
        breathing: state.breathing,
    ...
    api.on("before_tool_call", async (event) => {
      try {
        ...
        // Check tool params for struggle patterns (e.g., hedging in commit messages)
        const paramStr = JSON.stringify(event.params ?? {});
        if (paramStr.length > 100) {
          const bs = detectStruggle(paramStr);
          if (totalStruggleScore(bs) > 0.5) {
            state.recordBullshitEvent("tool_params", bs); // method name kept for compat
            api.logger.debug?.(
              `${PLUGIN_ID}: tool ${event.toolName} params show struggle: [${bs.map((b) => b.type).join(",")}]`,
            );
          }
        }
      } catch (err) {
    ...
    // Track tool results for struggle patterns.
    // =========================================================================
    ...
    api.on("after_tool_call", async (event) => {
      try {
        ...
        if (result.length > 100) {
          const bs = detectStruggle(result);
          if (totalStruggleScore(bs) > 0.3) {
            state.recordBullshitEvent("tool_result", bs); // method name kept for compat
          }
        }
    ...
    api.on("session_start", async (_event, ctx) => {
      ...
      try {
        // Initialize git sync FIRST (if configured via env vars)
        // This must happen before loading local state so we can merge remote changes
        ...
        // Pull and merge remote state BEFORE loading local state
        if (gitSync) {
          const pullResult = await pullAndMerge(gitSync, workspaceDir, {
            info: (msg) => api.logger.info(msg),
            warn: (msg) => api.logger.warn(msg),
            debug: (msg) => api.logger.debug?.(msg),
          });
          if (pullResult.pulled) {
            api.logger.debug?.(
              `${PLUGIN_ID}: merged remote state (reflexions=${pullResult.merged.reflexions} claims=${pullResult.merged.claims} entities=${pullResult.merged.entities} relations=${pullResult.merged.relations})`,
            );
          }
        }
        ...
        // Load CLAWDBOT meta-skills state
        {
          const { readFile: rfMeta } = await import("node:fs/promises");
          const { join: pjMeta } = await import("node:path");
          const awarenessDir = pjMeta(workspaceDir, "awareness");

          calibrationLogState.awarenessDir = awarenessDir;
          failurePatternsState.awarenessDir = awarenessDir;
          postTaskState.awarenessDir = awarenessDir;

          calibrationLogModule.loadLog(calibrationLogState);
          failurePatternsModule.loadLog(failurePatternsState);
          postTaskModule.loadLog(postTaskState);

          try {
            const confInlineRaw = await rfMeta(
              pjMeta(awarenessDir, "confidence-inline.json"),
              "utf-8",
            );
            confidenceInlineState = confidenceInlineModule.fromJSON(JSON.parse(confInlineRaw));
          } catch {
            // No prior confidence-inline data
          }

          try {
            const stateReportRaw = await rfMeta(pjMeta(awarenessDir, "state-report.json"), "utf-8");
            const loaded = stateReportModule.fromJSON(JSON.parse(stateReportRaw));
            Object.assign(stateReportState, loaded);
          } catch {
            // No prior state-report data
          }
        }
    ...
    api.on("session_end", async (_event, ctx) => {
      ...
      try {
        ...
        // Save CLAWDBOT meta-skills state
        const awarenessDir = pjGraph(workspaceDir, "awareness");
        calibrationLogState.awarenessDir = awarenessDir;
        failurePatternsState.awarenessDir = awarenessDir;
        postTaskState.awarenessDir = awarenessDir;

        calibrationLogModule.saveLog(calibrationLogState);
        failurePatternsModule.saveLog(failurePatternsState);
        postTaskModule.saveLog(postTaskState);

        await wfGraph(
          pjGraph(awarenessDir, "confidence-inline.json"),
          JSON.stringify(confidenceInlineModule.toJSON(confidenceInlineState), null, 2),
          "utf-8",
        );

        await wfGraph(
          pjGraph(awarenessDir, "state-report.json"),
          JSON.stringify(stateReportModule.toJSON(stateReportState), null, 2),
          "utf-8",
        );
        ...
        const statusData: StatusData = {
          ...
          trust: inferTrustLevel(partnershipToJSON()),
          ...
          curiosity: formatCuriosityQuestions(curiosityItems),
          recentReflexions: state.reflexions.slice(-3),
          metrics: {
            aliveRate: metricsExport.alive_rate,
            greyRate: metricsExport.grey_rate,
            bullshitFreeRate: 1 - metricsExport.bullshit_avg,
            selfCorrectionRate: metrics.selfCorrectionRate,
          },
          signal: state.lastPulse
            ? encode(state.buildSignalState(state.lastPulse))
            : "COEF/1 pulse=unknown",
        };
        const statusMd = generateStatusMd(statusData);
        await wf(pjoin(aDir, "STATUS.md"), statusMd, "utf-8");

        // Evidence gathering: scan transcript for partnership evidence
        const transcript = state.getSessionTranscript();
        const evidenceCandidates = scanForEvidence(transcript, {
          ...
          bullshitEvents: state.recentBullshitEvents(100).map((e) => ({
            ...
            turn: e.turn,
            target: e.source === "human" ? ("human" as const) : ("agent" as const),
          })),
          pulseHistory: state.getPulseHistory().map((p) => ({
            state: p.state,
            turn: p.turn,
          })),
        });
        ...
        if (evidenceCandidates.length > 0) {
          api.logger.info?.(
            `${PLUGIN_ID}: found ${evidenceCandidates.length} evidence candidates: ${evidenceCandidates.map((c) => `Cat${c.category}`).join(", ")}`,
          );

          // Write to local awareness directory
          const {
            writeFile: wfEvidence,
            mkdir: mkdEvidence,
            appendFile,
          } = await import("node:fs/promises");
          const { join: pjEvidence } = await import("node:path");
          const evidenceDir = pjEvidence(workspaceDir, "awareness", "evidence-candidates");
          await mkdEvidence(evidenceDir, { recursive: true });
          const dateStr = new Date().toISOString().split("T")[0];
          const evidenceFile = pjEvidence(evidenceDir, `${dateStr}.jsonl`);
          const evidenceJsonl = evidenceCandidates.map((c) => JSON.stringify(c)).join("\n") + "\n";
          await appendFile(evidenceFile, evidenceJsonl, "utf-8");
        }

        // Clear transcript for next session
        state.clearSessionTranscript();
    ...
    api.on("agent_end", async (event) => {
      try {
        ...
        api.logger.debug?.(
          `${PLUGIN_ID}: agent_end success=${event.success} turns=${state.turnCount} grey_rate=${(t.greyRate * 100).toFixed(0)}% avg_wm=${t.avgWiseMind.toFixed(2)} drift=${t.driftDirection} disagreements=${stats.total} struggle_events=${state.bullshitEventCount}`,
        );
      } catch (err) {
    ...
    api.on("llm_input", async (event) => {
      try {
        ...
        // Struggle detection on system prompt (helps notice patterns)
        if (event.systemPrompt && event.systemPrompt.length > 100) {
          const bs = detectStruggle(event.systemPrompt);
          if (totalStruggleScore(bs) > 0.5) {
            state.recordBullshitEvent("system_prompt", bs); // method name kept for compat
            api.logger.debug?.(
              `${PLUGIN_ID}: system prompt struggle score=${totalStruggleScore(bs).toFixed(2)} types=[${bs.map((b) => b.type).join(",")}]`,
            );
          }
        }
    ...
    api.on("before_model_resolve", async (event) => {
      try {
        ...
        if (state.consecutiveGrey >= 5 && state.bullshitEventRate() > 0.5) {
          api.logger.debug?.(
            `${PLUGIN_ID}: model resolve warning — consecutiveGrey=${state.consecutiveGrey} struggleRate=${(state.bullshitEventRate() * 100).toFixed(0)}%`,
          );
        }
