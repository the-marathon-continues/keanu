...
describe("carnegie manipulation detection", () => {
  describe("scarcity patterns", () => {
    it("detects 'last chance' urgency", () => {
      ...
      expect(result.presuppositions.some((p: { type: string }) => p.type === "scarcity")).toBe(true);
    });
  ...
  describe("false dilemma patterns", () => {
    it("detects 'either/or' forced choices", () => {
      ...
      expect(result.triggered).toBe(true);
      expect(result.presuppositions.some((p: { type: string }) => p.type === "false_dilemma")).toBe(true);
    });
  ...
  describe("moving goalposts patterns", () => {
    it("detects 'yes but now also' shifting", () => {
      ...
      expect(result.triggered).toBe(true);
      expect(result.presuppositions.some((p: { type: string }) => p.type === "moving_goalposts")).toBe(true);
    });
  ...
  describe("whataboutism patterns", () => {
    it("detects 'what about when' deflection", () => {
      ...
      expect(result.triggered).toBe(true);
      expect(result.presuppositions.some((p: { type: string }) => p.type === "whataboutism")).toBe(true);
    });
  ...
  describe("sealioning patterns", () => {
    it("detects 'just asking questions' pattern", () => {
      ...
      expect(result.triggered).toBe(true);
      expect(result.presuppositions.some((p: { type: string }) => p.type === "sealioning")).toBe(true);
    });
  ...
  describe("appeal to consequences patterns", () => {
    it("detects 'if you don't, you'll regret'", () => {
      ...
      expect(result.triggered).toBe(true);
      expect(result.presuppositions.some((p: { type: string }) => p.type === "appeal_to_consequences")).toBe(true);
    });
  ...
  describe("edge cases", () => {
    ...
    it("does not trigger on benign questions", () => {
      ...
      const sealioning = result.presuppositions.find((p: { type: string }) => p.type === "sealioning");
