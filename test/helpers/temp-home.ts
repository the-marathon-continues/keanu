...
function snapshotEnv(): EnvSnapshot {
  return {
    ...
    keanuHome: process.env.KEANU_HOME,
...
function restoreEnv(snapshot: EnvSnapshot) {
  ...
  restoreKey("KEANU_HOME", snapshot.keanuHome);
  restoreKey("KEANU_STATE_DIR", snapshot.stateDir);
}
...
function setTempHome(base: string) {
  ...
  // Ensure tests using HOME isolation aren't affected by leaked KEANU_HOME.
  delete process.env.KEANU_HOME;
  process.env.KEANU_STATE_DIR = path.join(base, ".keanu");
...
export async function withTempHome<T>(
  ...
  const base = await fs.mkdtemp(path.join(os.tmpdir(), opts.prefix ?? "keanu-test-home-"));
  ...
  setTempHome(base);
  await fs.mkdir(path.join(base, ".keanu", "agents", "main", "sessions"), { recursive: true });
  if (opts.env) {
