...
// Tool-aware oracle types — preserves content blocks instead of flattening to text
...
// Tool result block — what comes back after executing a tool
export interface OracleToolResultBlock {
  type: "tool_result";
  tool_use_id: string;
  content: string;
}

// Tool-aware messages can carry content block arrays (assistant with tool_use)
// or tool result arrays (user responding to tool_use).
export interface OracleToolMessage {
  role: "user" | "assistant";
  content: string | OracleContentBlock[] | OracleToolResultBlock[];
}

export interface OracleToolOptions extends Omit<OracleOptions, "messages"> {
  tools?: OracleToolDef[];
  messages: (OracleMessage | OracleToolMessage)[];
}

export interface OracleToolResponse {
