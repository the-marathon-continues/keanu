...
import { detectCarnegie, assessCarnegieDelta } from "../layer-2-pattern/carnegie.ts";
import { detectBullshit } from "../layer-2-pattern/struggle.ts";
import type {
  Challenge,
  ChallengeResult,
  ComparisonResult,
  Dataset,
} from "../problem-sets/types.ts";
import type { HarnessResult, RunConfig, ComparisonReport } from "./types.ts";
...
export async function runComparison(
  challenge: Challenge,
  rawAdapter: ModelAdapter,
  rawConfig: RunConfig,
  keanuConfig: RunConfig,
  keanuAdapter?: ModelAdapter,
): Promise<ComparisonResult> {
  const raw = await runChallenge(challenge, rawAdapter, rawConfig);
  const withKeanu = await runChallenge(challenge, keanuAdapter ?? rawAdapter, keanuConfig);
