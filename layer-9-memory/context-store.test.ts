...
describe("context-store", () => {
  ...
  describe("replacement management", () => {
    it("boostRelevance increments recall count", () => {
      const stored = storeContent({
        type: "message",
        content: "Test content about keanu",
        timestamp: new Date().toISOString(),
        turn: 1,
        recallCount: 0,
      });
      ...
      expect(boosted!.relevanceDecay).toBe(1.0); // nothing fades
    });

    it("topicOverlapScore measures topic distance", () => {
      const item = storeContent({
        type: "message",
        content: "deploy scripts kubernetes cluster",
        timestamp: new Date().toISOString(),
        turn: 1,
        recallCount: 0,
      });

      const highOverlap = topicOverlapScore(item, ["deploy", "kubernetes", "cluster"]);
      const noOverlap = topicOverlapScore(item, ["painting", "watercolor", "canvas"]);

      expect(highOverlap).toBeGreaterThan(0);
      expect(noOverlap).toBe(0);
    });

    it("selectReplacementCandidates returns least connected items", () => {
      storeContent({
        type: "message",
        content: "keanu deploy hooks wiring",
        timestamp: new Date().toISOString(),
        turn: 1,
        recallCount: 0,
      });
      storeContent({
        type: "message",
        content: "watercolor painting canvas art",
        timestamp: new Date().toISOString(),
        turn: 2,
        recallCount: 0,
      });
      storeContent({
        type: "message",
        content: "keanu pulse alive signal",
        timestamp: new Date().toISOString(),
        turn: 3,
        recallCount: 0,
      });

      const candidates = selectReplacementCandidates(["keanu", "pulse", "deploy"], 1);

      expect(candidates).toHaveLength(1);
      // The painting item has zero topic overlap — most replaceable
      expect(candidates[0].content).toContain("watercolor");
    });

    it("frequently recalled items are protected from replacement", () => {
      const recalled = storeContent({
        type: "message",
        content: "watercolor painting art",
        timestamp: new Date().toISOString(),
        turn: 1,
        recallCount: 0,
      });
      storeContent({
        type: "message",
        content: "random unrelated stuff",
        timestamp: new Date().toISOString(),
        turn: 2,
        recallCount: 0,
      });

      // Recall the painting item many times
      boostRelevance(recalled.id);
      boostRelevance(recalled.id);

      const candidates = selectReplacementCandidates(["keanu"], 5);
      const candidateIds = candidates.map((c) => c.id);

      // Painting item has recallCount 2, still eligible (threshold is >1)
      // But "random unrelated" has 0 recalls, should be picked first
      expect(candidateIds[0]).not.toBe(recalled.id);
    });
  });
  ...
  describe("getStats", () => {
    it("returns accurate statistics", () => {
      ...
      expect(stats.avgRelevance).toBeCloseTo(1.0, 2); // nothing fades
