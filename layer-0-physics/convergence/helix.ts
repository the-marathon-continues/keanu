...
export class Helix {
  ...
  private classifyState(
    ...
    // Check before grey — black is stricter and worse
  ...
  private stateToColor(state: AliveState, s: StrandScore): string {
    switch (state) {
      case "alive": {
        ...
        if (s.factual > s.felt + 0.15) {
          return "#4169E1";
        } // blue (fact-heavy alive)
        if (s.felt > s.factual + 0.15) {
          return "#DC143C";
        } // red (feeling-heavy alive)
        if (s.convergence > 0.8) {
          return "#9370DB";
        } // purple (deep convergence)
