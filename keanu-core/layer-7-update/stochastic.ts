...
import type { ReasoningModule } from "../layer-2-pattern/discover.js";
...
export function shouldExplore(
  ...
  if (turnsSinceExplore > 5) {
    rate += 0.1;
  }
  if (turnsSinceExplore > 10) {
    rate += 0.15;
  }

  // Low health suggests current approach isn't working
  // HealthReading.status: "steady" | "warm" | "hot" | "fading"
  if (health && (health.status === "hot" || health.status === "fading")) {
    rate += 0.15;
  }
  ...
  if (humanTone === "confused" || humanTone === "uncertain") {
    rate += 0.1;
  }

  // Decisive human doesn't want alternatives
  if (humanTone === "decisive" || humanTone === "impatient") {
    rate -= 0.15;
  }
