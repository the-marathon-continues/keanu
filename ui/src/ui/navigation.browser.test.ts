...
describe("control UI routing", () => {
  ...
  it("infers nested base paths", async () => {
    const app = mountApp("/apps/keanu/cron");
    await app.updateComplete;

    expect(app.basePath).toBe("/apps/keanu");
    expect(app.tab).toBe("cron");
    expect(window.location.pathname).toBe("/apps/keanu/cron");
  });

  it("honors explicit base path overrides", async () => {
    window.__KEANU_CONTROL_UI_BASE_PATH__ = "/keanu";
    const app = mountApp("/keanu/sessions");
    await app.updateComplete;

    expect(app.basePath).toBe("/keanu");
    expect(app.tab).toBe("sessions");
    expect(window.location.pathname).toBe("/keanu/sessions");
  });
  ...
  it("hydrates token from URL params even when settings already set", async () => {
    localStorage.setItem("keanu.control.settings.v1", JSON.stringify({ token: "existing-token" }));
    const app = mountApp("/ui/overview?token=abc123");
    await app.updateComplete;
