...
export async function handleWhatsAppStart(host: KeanuApp, force: boolean) {
  await startWhatsAppLogin(host, force);
  await loadChannels(host, true);
}

export async function handleWhatsAppWait(host: KeanuApp) {
  await waitWhatsAppLogin(host);
  await loadChannels(host, true);
}

export async function handleWhatsAppLogout(host: KeanuApp) {
  await logoutWhatsApp(host);
  await loadChannels(host, true);
}

export async function handleChannelConfigSave(host: KeanuApp) {
  await saveConfig(host);
  await loadConfig(host);
  await loadChannels(host, true);
}

export async function handleChannelConfigReload(host: KeanuApp) {
  await loadConfig(host);
  await loadChannels(host, true);
}
...
function resolveNostrAccountId(host: KeanuApp): string {
...
function resolveGatewayHttpAuthHeader(host: KeanuApp): string | null {
...
function buildGatewayHttpHeaders(host: KeanuApp): Record<string, string> {
...
export function handleNostrProfileCancel(host: KeanuApp) {
...
export function handleNostrProfileToggleAdvanced(host: KeanuApp) {
  const state = host.nostrProfileFormState;
  if (!state) {
    return;
  }
...
export async function handleNostrProfileSave(host: KeanuApp) {
  const state = host.nostrProfileFormState;
...
export async function handleNostrProfileImport(host: KeanuApp) {
  const state = host.nostrProfileFormState;
